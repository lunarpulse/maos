...
**Story 13.6b — the sign-side widening is now SHIPPED, and the weld is what carries the binding.** The paragraphs above wrote the seed widening in the future tense ("the crossing *requires* every emitter to hold the seed"). It is present tense now: the production crossing emitter reads `MAOS_CROSS_TEAM_BASE_SEED` on the **sign** side (`build_replication_bundle_v2` → `derive_team_signing_seed`), where before 13.6b production read it only to derive *public* verification keys. `env_contract.rs` is corrected accordingly — its purpose text used to claim *"the store never receives the seed"* about a verify-only input.

And 13.6b closed the gap the 13.6a/13.6b split left behind. 13.6a bound the **envelope** (`request.cohort_source_team`); the crossing decides consent from the **payload** (`bundle.source_team` → `is_granted`); **nothing bound them**. A host could stamp a truthful envelope — it cannot lie there — and sign a **lying payload** under any team's derived key, landing an impersonated row with transport ✓, `-32010` ✓, signature ✓, consent ✓. The applier now refuses that before any apply, under `CODE_CROSSING_SOURCE_TEAM_UNBOUND` (-32011), distinct from both `-32010` (a lying envelope) and `-32012` (an honest but ungranted crossing). ⚠ **13.6a's impersonation leg is NOT evidence for this control**: different site, different assertion, and its forger is seed-less while this one holds the seed and signs correctly. The 13.6b leg runs the real attack through the real applier.

A **boot reconcile** (`MAOS_LOOM_HOME_TEAM` must equal the signed `CohortMember.team`, or the process does not start) sits alongside it, and the two must never be conflated: an attacker owns their own boot and will set the environment correctly and lie in the payload. The reconcile is the correctness control against misconfiguration; the weld is the security control against a peer. **The surviving limit above is unchanged** — a seed-holder still forges within its own team, and an operator who controls a host controls that host's team.
