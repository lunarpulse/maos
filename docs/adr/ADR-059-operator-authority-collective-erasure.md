...
The independent stale-spill/duplicate-scan defect remains open under Story `13-5j`: overwriting a spilled value does not unlink the superseded file. Story 13.5i deliberately does not alter `write` or `scan`.
...
- Shared-tier principal state is closed by Decision 11's authorized partition, and private-tier filesystem erasure is closed by Decision 10's authorized Shape A4 repair. Cross-team fan-out, team-scoped holds, cross-shard correlation, and the separate `13-5j` stale-spill/duplicate-scan defect remain named limits; no proof or operator document may claim them closed.
