...
| 4 | Erasure correlation IDs and a multi-shard reconciliation reader | OPEN. **Same design as residual #7 and as decision D5.4** — scope them together or neither; three homes for one item is the drift the Epic-14 register exists to stop | `v25-erasure-crash-reconciliation` — owner assigned 2026-08-26 by Story `14-0` AC4.2/AC4.3, replacing *Ownerless and open* |
...
| 7 | Crash atomicity between store mutation and audit append | OPEN; fail-fast and detected by one-sided reconciliation. **DUPLICATES residual #4 and decision D5.4** (`deferred-work.md:547` was its third home) — recorded 2026-08-26 by Story `14-0` AC4.2 so the three cannot drift apart | `v25-erasure-crash-reconciliation` — owner assigned 2026-08-26 by Story `14-0`, replacing *Ownerless and open* |
...
| 9 | Private-tier forget is not atomic across its two stores, and a failed attempt is unaccounted | OPEN — **and this row's stated MECHANISM was INVERTED; corrected 2026-08-26 by Story `14-0` AC4.3, verified at HEAD.** The row asserted that *"the in-memory removal completes before the first fallible filesystem operation"*. It does not. `forget_principal_unix` (`crates/maos-kernel-core/src/memory/private.rs:827`) takes a READ lock and snapshots the matching keys into `to_remove` (`:830-845`), then walks and unlinks the filesystem (`:847-908`), and only **afterwards** takes the WRITE lock and removes from the map (`:910-919`) before returning `Ok(erased.len())`. So an `Err` from the filesystem walk returns via `?` with the in-memory map **still intact** — the RAM-only value is NOT destroyed, and the row described the opposite hazard for the whole time it was open. **What survives is the real residual and it is unchanged in severity:** the two stores are still not atomic, so a mid-walk failure leaves the filesystem partially purged with no reconciliation record and no accounting of what the failed attempt destroyed. That is the limit; the RAM-loss consequence is withdrawn. Raised by the 13.5i code review; not a spec violation, and deliberately not absorbed | Ownerless and open — the general non-atomicity is carried by `v25-private-tier-erase-atomicity` (14-0 AC4.3, D6's surviving residual) |
