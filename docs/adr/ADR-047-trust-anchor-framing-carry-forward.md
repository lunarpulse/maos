...
- **Operator-local signing seed.** The Transparency Log signing key is derived from the operator's seed via HKDF-SHA256 (`hkdf` crate, RustCrypto). The seed is the operator's secret, stored locally. No network dependency.
  **CORRECTED 2026-08-26 by Story `14-0` AC4.4 — this line named the wrong variable, and it is the likely origin of the whole `MAOS_REGION_HOME` confusion.** The seed is NOT in `MAOS_REGION_HOME`. It resolves, in precedence order, from `--audit-key`, then the **`MAOS_AUDIT_KEY`** env var, then `~/.config/maos/audit-signing.key` (`crates/maos-domain/src/audit_key.rs:25-32`, `load_audit_key_seed`), and it fails loudly rather than silently generating a key. `MAOS_REGION_HOME` is a different thing entirely: a non-secret **region tag** that is bound into the HKDF `info` parameter (see the next bullet) and welded again per team by `derive_team_signing_seed` (`sealed_export.rs:303-315`). It is a **key-derivation INPUT, not key material** — which is precisely why it must stay echoable (`operator_config.rs:227` prints it) and must NOT be classified `EnvStability::Secret`. That an unregistered env var therefore selects WHICH Ed25519 key signs an audit bundle is decision **D4c**, open at `14-9-secret-var-governance-provider-keys`.
