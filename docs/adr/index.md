...
| [ADR-049](ADR-049-cross-region-collective-memory-consensus.md) | Cross-region convergent replication with enforced re-attestation — TL-anchored CRDT (formerly "consensus") | binding-v2.0 (Story 11.2a) | Story 11.2a — mediated re-attestation of KV rows via `canonical_kv_leaf` + region-signed `CrossRegionReplicationBundle` (no transparent replication, NOT TL-frame-crossing; "TL-anchored" = the 9.4b-derived signing key); independent per-region convergence oracle over collective_memory rows; region-identity reflex; dropped-audit-orphan surfaced; AP-local-degrade proven-red; kernel-core = one bounded `WriteEntryPoint::CrossRegionReadmit` re-pin 22964→~22980 (`maos-loom-lite` + `…
