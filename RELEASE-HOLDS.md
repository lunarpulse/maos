...
| 9 | `j1-crosshost-2c`: **two signatures prove two keyed identities signed — not two machines, two processes, or two operators.** The host discriminator on `AuditBundle` defeats a forger who does not hold the other host's key; it does not prove physical separation, and under a shared base seed it proves nothing at all, which is why `reconcile_two_host_bundles` refuses two halves attested by one root. `region` cannot separate two hosts in one jurisdiction (same derived key), `boot_nonce` is per-boot and one `--range 1d` export swept eight, and `attester_pubkey` is bundle-supplied so R-RG1 forbids trusting it. The mTLS trust anchor between two hosts is established **out of band by a human operator**, not by the protocol; host B's audit signing key is **provisioned separately, by hand**. Both are capture fields whose overclaim direction is refused (`two_host_trust_anchor`, `two_host_host_b_audit_key`). | That a two-host bundle proves two machines, two processes or two operators; that the pairing or the key provisioning is automated; that `region`, `boot_nonce` or `attester_pubkey` can discriminate the halves. | **CORRECTED 2026-08-26 (Story `14-0` AC4.4): "None … not debt" was true of the pairing path and FALSE of the region half.** The manual pairing channel and the artifact format's limits are indeed permanent properties, and an automated pairing channel would be its own story. But this row rests on *"`region` cannot separate two hosts in one jurisdiction (same derived key)"*, and that shared derived key is selected by **`MAOS_REGION_HOME`, which is unregistered in the env contract** (absent from `env_contract.rs`'s 67 entries; `check_env_contract.rs:119` scans only `crates/maos-bin/src` while both primitive reads live in `maos-kernel-core` and `maos-domain`). It is an HKDF input to the region seed (`sealed_export.rs:303-315`) and then to `derive_team_signing_seed`, so it silently selects which Ed25519 key signs an audit bundle — and no verifier can catch a wrong value, because `resolve_verify_key` derives the EXPECTED key from the bundle's own claimed region, so a wrong-but-self-consistent region verifies GREEN and `key_a == key_b` never fires. That IS debt, it is decision **D4** (split into D4a/D4b/D4c), and it is open at `14-d4a-region-home-boot-reconciliation`, `14-8-register-classify-full-workspace-env-surface` and `14-9-secret-var-governance-provider-keys`. **No fix is claimed here — only the boundary is stated correctly.** |
