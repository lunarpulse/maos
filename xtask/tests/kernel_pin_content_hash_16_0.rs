...
fn the_three_other_readers_still_resolve_the_pin() {
    ...
    assert_eq!(a2a, 24474, "the a2a-tcp reader");
    ...
    let oracle: usize = text
        ...
        .parse()
        .expect("parses");
    assert_eq!(oracle, 24474, "the fkcs oracle reader");
}
...
fn the_pin_is_readable_by_a_real_toml_parser() {
    ...
    // line-count-neutral: `src_lines` is unmoved at line 481.
    let text = std::fs::read_to_string(baseline_toml()).expect("read baseline");
    let parsed: toml::Value = toml::from_str(&text).expect("the baseline file must be valid TOML");
    assert_eq!(parsed["src_lines"].as_integer(), Some(24474));
    ...
    assert_eq!(
        numbered + 1,
        481,
        "`src_lines` moved off line 481 — every `kernel-core-baseline.toml:481` citation \
