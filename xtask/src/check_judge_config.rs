...
pub fn run(config_path: &str, identifiers_path: &str, json: bool) -> Result<(), String> {
    ...
    if json {
        ...
        println!(
            "check-judge-config: PASSED ({} entries checked)",
            report.checked
        );
    } else {
    ...
    if !report.passed {
        return Err("check-judge-config failed".into());
    }
    Ok(())
}
...
fn check_judge_config(config_path: &Path, identifiers_path: &Path) -> Result<Report, String> {
    ...
    for (name, entry) in &config.judge {
        ...
        if entry.temperature != 0.0 {
            push("temperature", &entry.temperature.to_string());
        }
        if entry.top_p != 1.0 {
            push("top_p", &entry.top_p.to_string());
        }
        if entry.retry_budget != 1 {
            push("retry_budget", &entry.retry_budget.to_string());
        }
        if !is_hex_64(&entry.prompt_version_hash) {
            push("prompt_version_hash", &entry.prompt_version_hash);
        }
        if !is_model_format(&entry.model) {
    ...
    if !call_set.is_empty() {
        ...
        if tests_dir.exists() {
            fs_walk::collect_rs_files(tests_dir, &mut test_files);
        }
        let crates_tests = Path::new("crates");
        if crates_tests.exists() {
            for entry in
                fs::read_dir(crates_tests).map_err(|e| format!("cannot read crates dir: {e}"))?
            {
        ...
        if node.attrs.iter().any(|attr| {
            attr.path().is_ident("cfg")
                && attr
                    .meta
                    .require_list()
                    .map(|m| m.tokens.to_string().contains("test"))
                    .unwrap_or(false)
        }) {
...
#[cfg(test)]
mod tests {
    include!("tests/check_judge_config_tests.rs");
}
