...
fn ac5_morning_digest_composes_cites_and_persists_within_budget() {
    ...
    std::fs::write(
        &journal,
        format!(
            "{{\"timestamp\":{},\"lifecycle_event\":\"Pause\",\"spirit_id\":\"butler\"}}\n",
            now - 5_000
        ),
    )
    .unwrap();
    ...
    let anomalies = vec![
        ...
        .unwrap(),
        NotificationEvent::anomaly_flagged("observer", "butler", "low-confidence blip", 0.40)
            .unwrap(),
    ];
    ...
    let cited: std::collections::BTreeSet<String> = digest
        .completed
        .iter()
        .map(|c| c.source_log_ref.clone())
        .collect();
    assert!(
        cited.contains(&hex(&c1)) && cited.contains(&hex(&c2)),
        "both completions cite their frame id"
    );
    ...
    assert!(
        digest
            .completed
            .iter()
            .all(|c| c.source_log_ref.len() == 32),
        "every cite is a 16-byte frame id"
    );
    ...
    assert_eq!(
        digest.anomalies.len(),
        1,
        "only the ≥0.6-confidence anomaly is included"
    );
    ...
    let request = butler
        .digest_to_distillation_request(&digest)
    ...
    let receipt = writer
        .write_distillate(1, request)
        .expect("write succeeds — all refs resolve");
    assert!(receipt.effective_distillation_depth >= 1);
    assert!(
        !receipt.effective_source_log_ref.is_empty(),
        "kernel computed a non-empty audit chain"
    );
    assert!(
        !receipt.digest_frame_id.iter().all(|b| *b == 0),
        "digest frame id minted"
    );
}
