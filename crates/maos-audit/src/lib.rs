...
// ─────────────────────────────────────────────────────────────────────────
// Story 11.4c — out-of-kernel `identity.asserted` write (kind = 30)
// ─────────────────────────────────────────────────────────────────────────
//
// `maos-audit` is read-only by design: every OTHER row in the Transparency
// Log is written by `maos-kernel-core::iac::transparency_log`
// (`TransparencyLogAdapter::insert_frame_event`), which takes the kernel-core
// `FrameKind` enum. `IdentityAsserted` is NOT a kernel-core `FrameKind` —
// adding it would be an L1 kernel-core delta (FORBIDDEN) — so Story 11.4c
// AC2's requirement that the `maos-sso` adapter EMIT an out-of-kernel
// `identity.asserted` audit row is satisfied by THIS single sanctioned
// production write path. The read-side kind map already round-trips it
// (`kind_to_string(30) == "identity.asserted"`); this helper is the write
// complement.

/// Append ONE out-of-kernel `identity.asserted` (kind = 30) row to the
/// Transparency Log.
///
/// This is the ONLY sanctioned production write path in `maos-audit`. It
/// exists because `IdentityAsserted` is not a kernel-core `FrameKind`
/// (adding it would be an L1 kernel-core delta); the `maos-sso` adapter's
/// verified-principal authorization flow consumes it to emit an audit row
/// (Story 11.4c AC2).
///
/// # Row shape
///
/// - `frame_id`         — 16 cryptographically-random bytes (`getrandom`);
///                        a PK collision (vanishingly unlikely at 128 bits)
///                        surfaces as `AuditError::Query`, NEVER swallowed.
...
/// - `origin`           — `FrameOrigin::HumanAuthored = 0`: the identity was
///                        human-asserted at an external IdP — a NON-kernel
///                        origin (`Kernel = 3` is reserved for kernel-mediated
///                        frames). Reuses the existing domain enum so the
///                        discriminator cannot drift from `i3`.
///
/// Fail-closed: any SQLite, serialization, or entropy error returns
/// `AuditError` — no silent `INSERT OR IGNORE`. The `transparency_log` table
/// is created by the kernel; this helper does NOT create it and returns an
/// error if it is absent (single-sourced schema, no drift).
pub fn append_identity_asserted(
    ...
    // 16-byte random frame_id (128-bit collision space — the same width the
    // kernel uses). `getrandom` is the OS CSPRNG on every supported target;
    // an entropy failure is an I/O-level fault mapped onto `AuditError::Io`.
    let mut frame_id = [0u8; 16];
    getrandom::getrandom(&mut frame_id)
        .map_err(|e| AuditError::Io(std::io::Error::new(std::io::ErrorKind::Other, e)))?;

    // Compact JSON payload. Key order is irrelevant on read (parsed as an
    // object); serde_json's default Map backend yields a stable lexicographic
    // order on disk, so the serialized bytes are reproducible.
    ...
    // Narrow the u32/u64 inputs to the i64 SQLite INTEGER columns; fail-closed
    // on overflow — mirrors `query`'s `ValueOverflow` discipline.
    ...
    // Plain INSERT (NOT `INSERT OR IGNORE`): a frame_id PK collision surfaces
    // as SQLITE_CONSTRAINT_PRIMARYKEY → AuditError::Query rather than being
    // silently swallowed (Story 11.4c "fail closed" requirement).
    conn.execute(
