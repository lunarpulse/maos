...
fn openai_round_trip_with_fixture_response() {
    let fixture_bytes = std::fs::read("tests/fixtures/openai_success_response.json").unwrap();
...
fn openai_round_trip_with_max_tokens_fixture() {
    let fixture_bytes = std::fs::read("tests/fixtures/openai_max_tokens_response.json").unwrap();
    let transport = std::sync::Arc::new(MockTransport(fixture_bytes));
    let provider = OpenAiProvider::with_api_key(
        transport,
        "https://api.openai.com".into(),
...
fn openai_round_trip_with_error_fixture() {
    let fixture_bytes = std::fs::read("tests/fixtures/openai_error_response.json").unwrap();
    let transport = std::sync::Arc::new(MockTransport(fixture_bytes));
    let provider = OpenAiProvider::with_api_key(
        transport,
        "https://api.openai.com".into(),
