...
fn three_entry_paths_all_land_pending_never_auto_admitted() {
    ...
    // (1) package-shipped
    q.enqueue_skill(skill("skill.pkg"), SkillEntryPath::PackageShipped, "package").unwrap();
    // (2) skill.author.self (dynamic write)
    q.enqueue_skill(skill("skill.self"), SkillEntryPath::AuthorSelf, "spirit:42").unwrap();
    ...
    assert_eq!(labels, ["package_shipped", "author_self", "revision_proposal"]);
}

#[test]
fn approve_transitions_pending_to_admitted_and_journals() {
    let mut q = SkillAdmissionQueue::new();
    let id = q.enqueue_skill(skill("skill.approve"), SkillEntryPath::AuthorSelf, "spirit:7").unwrap();
    ...
    assert!(!audit[0].decision, "enqueue row is pending (decision=false)");
...
fn reject_transitions_pending_to_rejected() {
    let mut q = SkillAdmissionQueue::new();
    let id = q.enqueue_skill(skill("skill.reject"), SkillEntryPath::PackageShipped, "package").unwrap();
    assert!(q.reject(&id));
...
fn already_resolved_entry_does_not_transition_again() {
    let mut q = SkillAdmissionQueue::new();
    let id = q.enqueue_skill(skill("skill.once"), SkillEntryPath::AuthorSelf, "spirit:1").unwrap();
    assert!(q.approve(&id));
...
fn duplicate_skill_id_rejected_on_enqueue() {
    let mut q = SkillAdmissionQueue::new();
    q.enqueue_skill(skill("skill.dup"), SkillEntryPath::PackageShipped, "package").unwrap();
    let err = q.enqueue_skill(skill("skill.dup"), SkillEntryPath::AuthorSelf, "spirit:1").unwrap_err();
    assert!(matches!(err, ESkillQueue::DuplicateSkillId(ref id) if id == "skill.dup"));
    assert_eq!(q.entries().len(), 1, "only the first entry should be in the queue");
}

#[test]
fn same_id_after_approve_allows_re_enqueue() {
    let mut q = SkillAdmissionQueue::new();
    let id = q.enqueue_skill(skill("skill.re"), SkillEntryPath::PackageShipped, "package").unwrap();
    q.approve(&id);
    let id2 = q.enqueue_skill(skill("skill.re"), SkillEntryPath::AuthorSelf, "spirit:1").unwrap();
    assert_eq!(id, id2);
