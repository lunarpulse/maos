...
fn cap_token_verify_p99_under_5us_hot_path() {
    ...
    eprintln!(
        "cap_token_verify P99 = {}ns (ADR-030 budget: 5000ns)",
        p99_ns
    );
    assert!(
        p99_ns < 5_000,
        "ADR-030 binding broken: cap_token_verify P99 = {}ns, budget = 5000ns",
        p99_ns
    );
}
...
fn cap_token_verify_p99_under_100us_overall() {
    ...
    assert!(
        p99_us < 100,
        "NFR-Perf-3 binding broken: P99 = {}µs, budget = 100µs",
        p99_us
    );
}
