...
on_value_above = { threshold = 0.7 }"#);
    ...
    assert!(matches!(outcome, Some(PolicyEvaluationOutcome::VerbalizeOnly)));
}
...
on_value_within = { lower = 0.3, upper = 0.6 }"#);
    let adapter = make_adapter();
    let outcome = evaluate_after_set_scalar(
        "spirit-1", 1, 0xCAFE, "uncertainty", 0.45, "frame-001",
        &policy, &adapter as &dyn CapabilityRegistryPort,
    ).unwrap();
    assert!(matches!(outcome, Some(PolicyEvaluationOutcome::Flag(_))));
}
...
on_value_below = { threshold = 0.3 }"#);
    let adapter = make_adapter();
    let outcome = evaluate_after_set_scalar(
        "spirit-1", 1, 0xCAFE, "uncertainty", 0.1, "frame-001",
        &policy, &adapter as &dyn CapabilityRegistryPort,
    ).unwrap();
    assert!(matches!(outcome, Some(PolicyEvaluationOutcome::Flag(_))));
}
...
on_value_below = { threshold = 0.5 }"#);
    let adapter = make_adapter();
    let outcome = evaluate_after_set_scalar(
        "spirit-1", 1, 0xCAFE, "uncertainty", 0.3, "frame-001",
        &policy, &adapter as &dyn CapabilityRegistryPort,
    ).unwrap();
    assert!(matches!(outcome, Some(PolicyEvaluationOutcome::VerbalizeOnly)));
}
