...
use maos_kernel_core::capability::{
    cap_audit, cap_policy, cap_quota, cap_tokens, CapabilityRegistryAdapter, CapabilityRegistryPort,
};
...
fn fr4_1000_call_mediation() {
    ...
    for spirit_offset in 0..5u32 {
        ...
        for _ in 0..200 {
            let token = adapter
                ...
                .unwrap();
            assert_eq!(
                token.spirit_pid, spirit_pid,
                "token spirit_pid must match requested PID"
            );
            tokens.push(token);
    ...
    for token in &tokens {
        adapter
            .record_invocation(token, "fr4-test".into(), b"test-payload")
            .unwrap();
    }
    ...
    while let Ok(event) = audit_rx.try_recv() {
        match &event {
            cap_audit::CapAuditEvent::Issue { spirit_pid, .. } => {
                assert!(
                    *spirit_pid >= 1 && *spirit_pid <= 5,
                    "issue spirit_pid must be 1-5"
                );
                issue_count += 1;
            }
            cap_audit::CapAuditEvent::Verify {
                ...
                assert!(
                    *spirit_pid >= 1 && *spirit_pid <= 5,
                    "verify spirit_pid must be 1-5"
                );
                assert_eq!(
                    *outcome,
                    cap_audit::VerifyOutcome::Ok,
                    "all verifies should succeed"
                );
                verify_count += 1;
            }
            cap_audit::CapAuditEvent::Invocation { spirit_pid, .. } => {
                assert!(
                    *spirit_pid >= 1 && *spirit_pid <= 5,
                    "invocation spirit_pid must be 1-5"
                );
                invocation_count += 1;
            }
            _ => {}
    ...
    assert!(
        issue_count >= 1000,
        "FR4: expected >=1000 issue events, got {}",
        issue_count
    );
    assert!(
        verify_count >= 1000,
        "FR4: expected >=1000 verify events, got {}",
        verify_count
    );
    assert!(
        invocation_count >= 1000,
        "FR4: expected >=1000 invocation events, got {}",
        invocation_count
    );
    ...
    for (i, &count) in per_spirit_counts.iter().enumerate() {
        assert_eq!(
            count,
            200,
            "Spirit {} should have 200 tokens, got {}",
            i + 1,
            count
        );
    }
