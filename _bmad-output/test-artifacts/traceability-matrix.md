...
| **AC5.3 — beat flipped by an EXECUTED leg** | 2c | `demo_j1.rs::apply_two_host_signed_run` — in-process judge, never the published-ledger route (structurally dead twice). Owner re-pointed to `j1-crosshost-2d-paid-two-host-run` (RF-0; the "verified, not edited" record was corrected in §A6 review) | Unit | ✅ PROVEN |
