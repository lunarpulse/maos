...
> **📍 Phasing reconciliation banner (2026-05-06).** This document's §13 Phased Roadmap reflects the original v0.1 = Bootstrap with Architect Spirit driving a coding task framing. **The PRD's Step 8 supersedes that phasing** with a more honest 8-phase structure: v0.1 Foundational (placeholder Spirit only, no multi-Spirit, no A2A) → v0.3 Butler → v0.5 Researcher + Observer → v0.8 Founder Loop wedge demo (the original v0.1 Architect demo lives here) → v1.0 Team-ready → v1.5 Diagnostic-Architect → v2.0 (technical) → v2.5 (ecosystem-adoption). For canonical phase boundaries, ship-gate floors, and journey-to-phase mapping, see PRD Step 8. This document remains canonical for ADRs (now 27), invariants (now 14), kernel internal architecture, and architectural decision rationale. Doc-set propagation to update §13 + reference-Spirit version commitments is tracked as a follow-up.
...
| **Digest-hedge-preservation** | ≥ 0.95 | Hedged statements in raw ("possibly," "60% confident," "needs verification") survive into the digest, not flattened to certainty. |
...
*Winston signing off. Architecture is the practice of arranging trade-offs so future-you can change your mind without burning everything down. Twenty-seven ADRs, fourteen invariants, fourteen open questions. ADRs 012–015 anchor the distillation pattern. ADRs 016–017 anchor the kernel's role in long-running aggregating Spirits. ADRs 018–019 close two operational gaps that emerged when innovations interact (intent provenance, halt continuity). ADRs 020–021 supply the developer-tool contracts the substrate ships under. ADR-022 supplies the tagged-scalar primitive that lets Spirit cognition bind to the kernel's halt mechanism without the kernel adopting a probabilistic model class — under §4.0.7's principle that the kernel computes no Spirit-specific cognitive measure. ADRs 023–027 are the full Mary-suggested ADR set, all approved through the §4.0.7 lens: ADR-023 Trajectory Export Format (raw audit-and-training-data export), ADR-024 Spirit-Authored Skill Packages (actors-learn-from-each-performance with operator-admission gating), ADR-025 Proactive Scheduling Hook `on_schedule` (time-triggered Spirit invocations with manifest-declared side-effect allowlists), ADR-026 Principal Memory Namespace with Redaction-Aware Operations (substrate-side complement to Spirit-side user-modeling — kernel provides namespace + GDPR-class operations; Spirit defines schema), ADR-027 Skill Package Format and External-Standard Interop via Spirit-Authored Adapter (kernel hosts the format-discovery mechanism; agentskills.io interop pushed to user-space adapter Spirits). Together they keep the kernel small while making the substrate's promises mechanically defensible across Spirit-class evolution, operational change, the cognitive-frameworks ambition that single-Spirit Butler (v0.3) and Researcher (v0.5) need to succeed, and the v1.0 substrate-positioning claim that **MAOS at v1.0 can host a hermes-class Spirit as a tenant, with full audit, revocation, and substrate-uninstall guarantees that hermes-as-application cannot itself provide.** That sentence is the load-bearing positioning claim; if it survives the v1.0 black-box external-author trial (per Murat's gate), the substrate has earned its theater.*
