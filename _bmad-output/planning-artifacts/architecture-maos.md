...
**Acceptance criteria for distillation-shipping Spirits.** Aggregating Spirits MUST meet, across a corpus appropriate to the Spirit's domain (with at least 10 hedge-preservation cases and 10 contradiction cases):
...
*Winston signing off. Architecture is the practice of arranging trade-offs so future-you can change your mind without burning everything down. Fifteen ADRs, twelve invariants, fourteen open questions. The latest four ADRs (012–015) and two invariants (I11, I12) anchor the distillation pattern that emerged from the Orchestrator–Worker design discussions: typed-intent consent, kernel-mediated `log.recall`, digest audit-chain enforcement, and decision-context recording. They keep the kernel small while making aggregation across many peers — Orchestrator, Mira, Cortex, Loom — auditable from raw evidence to final decision.*
