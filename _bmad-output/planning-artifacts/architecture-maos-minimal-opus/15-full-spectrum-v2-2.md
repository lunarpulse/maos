...
2. **Per-crate ceiling discipline, as actually lived (ADV-055-2):** ceilings move only **(a) downward at any time — including when measured lines physically move out of a crate, or when a budget names a crate that does not exist (both previously forbidden by the `kloc.toml` MIGRATION RULE); (b) at epic retrospective under the CEILING RULE formula; (c) under an explicitly authorized measured grant; or (d) by an operator-ratified epic re-base story until Story 21-4 lands, when this temporary allowance expires.** Lowering is monotone — it can only make the gate stricter, and no threat model exists for an unlisted lowering — so (a) carries no condition and the two cases named above are clarifications, not a closed list. Raising is the direction that needs sanctioning, and (b)–(d) are its only sources. Every change records the measured value and driver in the same commit; planned-growth reservations remain forbidden. A crate minted by extraction gets its initial ceiling from measured LOC plus the CEILING RULE reserve, recorded in the extraction commit. (Amended by Story 15-2, 2026-09-06: the pre-amendment retro allowance of *"the tight measured residual (+≤1% slack)"* is superseded under (b) by the CEILING RULE's `max(100, ceil(0.02 × measured))` — a substantive loosening, disclosed here rather than left for a reader to diff.) These comments are governance prose; no gate parses them.
