...
**v1.0 binding — End-to-end publish/install/yank/air-gap (Story 7.2):** The producer side of the v1.0 registry quartet (FR35 publish + FR36 install + FR59 yank + FR60 air-gap import) lands here. `maos-spirit publish --tier=<tier>` (`crates/maos-spirit-cli/`) signs `sha256(manifest_toml || artifact_bytes)` with the publisher's Ed25519 key, auto-populates a self-attested ComplianceClaim envelope from the manifest (or accepts `--compliance-claim <path>` for externally-baked envelopes), and dispatches `registry.publish` through `SpiritRegistryClient`. `maosctl import --offline <signed-bundle.tar>` (`crates/maos-cli/src/cli.rs::Subcommand::Import` + `crates/maos-registry/src/import.rs`) opens an uncompressed tar carrying `signed-package.json` (authoritative) + `manifest.toml` + `artifact.bin` (operator-inspection extracts) + optional `vetter-attestations/` + `compliance-claims/` directories, verifies per-file consistency, and admits the bundle through the SAME `admit_spirit` code path network installs use — `registry_origin_tier = Local` (operator-controlled offline media), `FrameKind::SpiritImported = 26` audit-distinguishes the import from registry-served admissions (`SpiritAdmitted = 19`). FR59 5-min yank-propagation latency is mechanically asserted by `crates/maos-registry/tests/fr59_yank_propagation_within_5min_test.rs` driving the `YankPoller` against a virtual clock. Consumer-side trust-tier cross-verification (`SignedManifest.server_reported_tier` + `server_signature_on_tier`, both additive `Option` with `#[serde(default)]`) closes the Story 5.5d High `[edge]` carry-forward. `ABI_VERSION` stays at `1` — every new surface is additive (`cargo public-api --diff` reports `Added` only).

**v1.0 binding — Semantic evaluator + CCAC N=600 ship gate (Story 7.3):** The consumer side promotes the v0.5-α structural floor to a v0.9-binding semantic evaluator. `maos-compliance` (previously a placeholder; workspace count UNCHANGED at 29) is now the SINGLE home of envelope verification — `maos-registry::compliance_verify` is a thin re-export shim delegating to `maos_compliance::evaluator::evaluate_envelope`, which runs four steps: (1) Ed25519 verify over `claim_bytes`; (2) strict canonical-CBOR decode where an unknown enum value is `MalformedClaim`, never a silent default; (3) recompute the fingerprint from the **runtime** `RuntimeExecutionContext` (NOT the manifest alone) via the shared `canonical_cbor` encoder; (4) conjunctive comparison naming the FIRST divergent field via `DriftField`. The v1.0 upgrade is runtime-over-manifest: `effective_trust_tier` is the admission strictest-of result and `runtime_provider_endpoint`/`runtime_crypto_provider` come from the operator config + composition root, so a claim attesting `trust_tier=local` admitted under a policy forcing `public_untrusted` (attack table row 3), or attesting `crypto_provider="ring"` on a kernel composed with a different provider (row 7), is rejected with a field-named `EComplianceContextDrift`. NFR-Aud-9 ships the CCAC N=600 corpus (200 well-formed + 400 malformed; exactly 100 context-drift) via `maos-corpus-gen::ccac` (SHA-pinned seeds + committed JSONL) and the `ccac-n600-ship-gate` P0 ship-blocker: per-class floor ≥27/30, 100/100 context-drift rejected naming the divergent field, ±2% cross-validation across 3 reference Spirit contexts. The seven-field `ExecutionContextFingerprint` ABI is UNCHANGED — `ABI_VERSION` stays `1` (`abi-diff` clean; `cargo public-api --diff` reports `Added`-only for the new `maos-compliance` types).
