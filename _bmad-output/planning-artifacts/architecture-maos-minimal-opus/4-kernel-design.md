...
**Workspace member count (post Story 12.1 Task 1):**<!-- workspace-count-authoritative --> reconciles the stale post-11.4b baseline to the actual post-append count of **54 workspace members** (was stale at 50: 11.4c added `crates/maos-siem` + `crates/maos-sso` and 11.5 added `crates/maos-fkcs` without bumping the sentinel; Story 12.1 Task 1 appends ONE lib crate `crates/maos-cohort/` — the out-of-kernel signed cohort-manifest schema v1, RR9). The `check-workspace-count` hard-equality gate floor moves to 54. History: 46 (Story 10.4a, maos-loom-lite) → 48 (Story 11.1a, maos-host + maos-wasm-host) → 49 (Story 11.4a, maos-pdp) → 50 (Story 11.4b, maos-escape-detector) → 51/52 (Story 11.4c, maos-siem + maos-sso) → 53 (Story 11.5, maos-fkcs) → 54 (Story 12.1, maos-cohort).
