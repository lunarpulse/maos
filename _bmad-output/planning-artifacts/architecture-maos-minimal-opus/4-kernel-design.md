...
**Workspace member count (post Story 8.3):**<!-- workspace-count-authoritative --> Story 8.3 adds `spirits/observer/` (the Observer v0.5 reference Spirit — broad `scalar.tap` Telemetry-Stream subscriber + pre-halt scalar-drift watchdog + structural-anomaly suspect surfaced via `NotificationEvent::AnomalyFlagged`; the third crate under the epic-8 `spirits/` mandate, Decision A), moving the count to **33 workspace members** (post-Story-8.2 baseline 32 + `spirits/observer` = 33). The `check-workspace-count` gate floor moves to 33. (History: 27 pre-7.1 → 28 with `examples/example-spirit` → 29 with `maos-spirit-cli` (Story 7.2) → 30 with `maos-skill` (Story 7.4) → 31 with `spirits/butler` (Story 8.1) → 32 with `spirits/researcher` (Story 8.2) → 33 with `spirits/observer` (Story 8.3).)
