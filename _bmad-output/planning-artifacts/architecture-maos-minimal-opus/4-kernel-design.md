...
**Workspace member count (post Story 10.4a):**<!-- workspace-count-authoritative --> Story 10.4a adds ONE lib crate `crates/maos-loom-lite/` (Postgres+pgvector Loom-lite collective memory tier — user-space, ADR-006), moving the count from 45 to **46 workspace members** (45 + 1 = 46). The `check-workspace-count` gate floor moves to 46.
