...
**Workspace member count (post Story 7.4):**<!-- workspace-count-authoritative --> Story 7.4 adds `crates/maos-skill/` (the kernel-mediated skill ecosystem: the `maos.skill.v1` schema, filesystem discovery, the FR39 operator-admission queue, and FR57 revision proposals), moving the count to **30 workspace members** (post-Story-7.2 baseline 29 + `maos-skill` = 30; Story 7.3 added no new crate). The `check-workspace-count` gate floor moves to 30. (History: 27 pre-7.1 → 28 with `examples/example-spirit` → 29 with `maos-spirit-cli` (Story 7.2) → 30 with `maos-skill` (Story 7.4).)
