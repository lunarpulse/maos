...
**Workspace member count (post Story 8.2):**<!-- workspace-count-authoritative --> Story 8.2 adds `spirits/researcher/` (the Researcher v0.5 reference Spirit — participant-scoped `log.recall` walker + the §9.5 distillation pattern + the five-metric NFR-Aud-7 reference producer; the second crate under the epic-8 `spirits/` mandate, Decision A), moving the count to **32 workspace members** (post-Story-8.1 baseline 31 + `spirits/researcher` = 32). The `check-workspace-count` gate floor moves to 32. (History: 27 pre-7.1 → 28 with `examples/example-spirit` → 29 with `maos-spirit-cli` (Story 7.2) → 30 with `maos-skill` (Story 7.4) → 31 with `spirits/butler` (Story 8.1) → 32 with `spirits/researcher` (Story 8.2).)
