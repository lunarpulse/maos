...
**Workspace member count (post Story 8.1):**<!-- workspace-count-authoritative --> Story 8.1 adds `spirits/butler/` (the Butler v0.3 reference Spirit — `on_idle` anticipatory reasoning + FR17 morning digest; the first crate under the epic-8 `spirits/` mandate, Decision A), moving the count to **31 workspace members** (post-Story-7.4 baseline 30 + `spirits/butler` = 31). The `check-workspace-count` gate floor moves to 31. (History: 27 pre-7.1 → 28 with `examples/example-spirit` → 29 with `maos-spirit-cli` (Story 7.2) → 30 with `maos-skill` (Story 7.4) → 31 with `spirits/butler` (Story 8.1).)
