...
**Workspace member count (post Story 11.4b):**<!-- workspace-count-authoritative --> Story 11.4b adds ONE lib crate `crates/maos-escape-detector/` (out-of-kernel sandbox-escape structural anomaly detector — read-only Transparency-Log consumer, ADR-024 / NFR-Sec-3 signal 1 of 3 @v2.0; ZERO kernel-core dependency at the library level, the `maos-audit` template), moving the count from 49 to **50 workspace members** (49 + 1 = 50). The `check-workspace-count` gate floor moves to 50. History: 46 (Story 10.4a, maos-loom-lite) → 48 (Story 11.1a, maos-host + maos-wasm-host) → 49 (Story 11.4a, maos-pdp) → 50 (Story 11.4b, maos-escape-detector).
