...
**Workspace member count (post Story 11.4a):**<!-- workspace-count-authoritative --> Story 11.4a adds ONE lib crate `crates/maos-pdp/` (out-of-kernel Cedar enterprise Policy Decision Point adapter — user-space, ADR-050 / NFR-Sec-17), moving the count from 48 to **49 workspace members** (48 + 1 = 49). The `check-workspace-count` gate floor moves to 49. NOTE: the prior authoritative sentinel declared 46 (post-Story-10.4a); Story 11.1a added TWO lib crates `crates/maos-host/` + `crates/maos-wasm-host/` (ADR-031 WASM Spirit form host) WITHOUT updating this sentinel — a gate drift (46 actual → 48) corrected here in ink alongside the 11.4a bump (48 → 49). History: 46 (Story 10.4a, maos-loom-lite) → 48 (Story 11.1a, maos-host + maos-wasm-host) → 49 (Story 11.4a, maos-pdp).
