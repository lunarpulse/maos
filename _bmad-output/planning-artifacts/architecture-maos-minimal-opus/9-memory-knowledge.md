...
### 9.5.1 v0.5 surface — Story 4.4 distillate audit chain

Story 4.4 lands the kernel-side substrate for the distillation pattern: `DistillateWriter` (I11 enforcement), `LogRecallAdapter` (participant-scoped read), and the five-metric measurement harness (NFR-Aud-7). `DistillateWriter::write_distillate` validates non-empty `source_log_ref`, `distillation_depth ≥ 1`, and kernel-computed `intent_lineage` (I13 — union of input frame intents, NEVER Spirit-self-reported). Transitive flattening resolves digests-of-digests to original raw frames with cycle detection (`HashSet` seen-set). The serialized receipt carries `digest_payload` and `segment_hint` for round-trip integrity. `FrameKind::Distillate = 11` is the audit-log discriminator; `SelfTelemetryAggregator` counts precise `Distillate` frames (replacing the Story 4.3 `Decision` proxy). The five-metric gate (recall ≥0.90, faithfulness ≥0.98, hedge-preservation ≥0.95 at IAA ≥0.85, traceability 100%, secret-leakage 0%) is asserted by the `maos-eval` corpus harness against N=100 synthetic-v0 scenarios. See Table 9.5-1 and Appendix F.5 for floor derivation.
