...
**Fail-closed on unclassified (Story 8.8, closes audit G7).** The cross-Host A2A router (`A2ARouterCore`) is **unconditionally fail-closed**: a cross-Host frame whose consent is *unclassified* — no `consent_envelope`, no `intent_class`, a non-canonical `intent_class`, or one over `MAX_CANONICAL_INTENT_LEN` (128) — is **DENIED** at both the send seam (`prepare_outbound` → `A2AError::ConsentUnclassified`, frame never leaves) and the accept seam (`handle_intake` → `CODE_CONSENT_UNCLASSIFIED` -32009), never silently downgraded to the coarse 3-band projection. The deny code is distinct from `EIntentDenied` (-32001, classified-but-not-allowlisted), so the Transparency Log shows *why* a frame was rejected. There is **no band-fallback opt-in** — the 3-band-downgrade mechanism G7 closed is removed entirely, not toggled off (team consensus 2026-06-07, Winston + Murat + security red-team, under the most-spec-faithful + long-term-correct criterion): a fail-open opt-in serving a population of zero shipped legacy peers is a loaded gun, and any genuine future unmigrated-peer requirement is a new, threat-modeled, sunset-gated story. Same-Host process-internal IAC routes through `iac_bus.rs`, never `A2ARouterCore`, so same-Host trust is untouched by this posture.
