...
**Story 8.1 (2026-06-02) — Butler v0.3 Decision A & B ratification (Winston).**

Story 8.1 shipped the first reference cognitive Spirit and flagged two LOCKED decisions for architect ratification. Both are ratified below; the relevant ADRs are clarified in place rather than amended (no invariant I1–I14 surface changes, so `invariant-lock` does not fire).

**Decision A — Butler lives at `spirits/butler/` as a workspace member; count 30 → 31. RATIFIED.**
- *Home.* The reference Spirits are sibling workspace crates that compile against the published Spirit ABI, exactly like `examples/example-spirit` — they are NOT compiled into the `maos-bin` kernel binary. This is what the epic-8 "zero kernel KLOC — all Spirit code under `spirits/`" mandate requires, and what the 7.5b resolver already assumed (`BUTLER_CORPUS_REL = "spirits/butler/tests/fixtures/…"`). §6 chapter intro wording ("ship in the kernel binary") is corrected accordingly — see §6 edit.
- *Workspace count.* `check-workspace-count` floor moves to **31** (post-7.4 baseline 30 + `spirits/butler`). Authoritative sentinel is the `<!-- workspace-count-authoritative -->` line in §4 (4-kernel-design.md).
- *ADR-038 (KLOC ceiling).* `spirits/butler` is Spirit-side, not a kernel crate; it does **not** consume the ≤20 KLOC trusted-core budget and gets no `xtask/kloc.toml` entry. (The pre-existing `maos-kernel-core` overshoot alarm is unrelated and Butler-neutral.)
- *ADR-010 / Story 0.2 (empty-kernel + service boundary).* Butler adds zero public symbols to `maos-kernel-core`. Its `dev-dependencies` on kernel crates — used only to drive the real halt path (`WorkingMemoryOrchestrator::process_scalar_write`) and the I11 distillation chain in tests — do not violate `check-service-boundary`, which inspects the kernel public surface + the spirit-ABI hook count, not the dependency graph. The operational reading of the Story 0.2 invariant is "add no public symbol to `maos-kernel-core`," which holds.
- *ADR-002 (subprocess-only at v0.1; rust-inproc gated).* Butler's manifest declares `forms = ["rust-inproc"]`. **This is NOT a production-runtime unlock of rust-inproc.** It is the reference/measurement form — the form §13's harness measures and `LocalRunner`/`spirit-test` exercises — which ADR-002 already contemplates. ADR-002's three-condition gate for promoting rust-inproc to a *production deployment form* (sustained §13 threshold breach + J-Butler-not-fixable-code confirmation + superseding ADR) remains in force and **unfired**. Butler-as-reference-Spirit therefore sits inside ADR-002, not athwart it. Trade-off accepted: the substrate gains a measured in-proc reference implementation now, while the production-form commitment stays subprocess-only until the gate trips on its own evidence.

**Decision B — v0.3 "MCP tool integrations" satisfied by capability-scope mediation + fixture-replay, not live drivers. RATIFIED.**
- The §13 v0.3 milestone phrase "MCP tool integrations (Calendar / Slack / Linear / Figma)" is satisfied at the **substrate layer** by (1) Butler declaring `[[capabilities.required.mcp.servers]]` → `Scope::McpCall` capability scopes (the kernel-mediated, audited capability path — I1), and (2) fixture-replay of per-scenario inputs for validation (the J-Butler / Sandra-scene replay).
- Live Google Calendar / Slack / Linear / Figma client drivers are **application-layer integrations, not substrate code.** Deferring them is consistent with ADR-005 ("providers/drivers are independent crates; new drivers ship without kernel changes") and the substrate-not-product framing. They land with v0.5's "broad MCP capabilities" row / Epic 9.
- *Trade-off named (carry-forward).* Fixture-replay gives no live-network conformance at v0.3. The first live MCP driver (v0.5) MUST ship its own driver-conformance corpus; the v0.3 fixtures validate the kernel capability path and Butler's cognition, not provider wire behavior. Flagged for the v0.5 Researcher/Observer phase.
- This closes the ADR-025 gate ("Butler `on_idle` Sandra-scene replay") at v0.3 via the corpus replay, and confirms ADR-022's `[epistemic_policy]` four-predicate halt path on a real cognitive Spirit.
