...
- **NFR-Rel-7:** A2A trust establishment under churn — 100-host Cortex (or compressed 30-host scale per Murat's cost-compression), 10–20% host turnover/week for 4 weeks, 3 planted adversarial hosts. Floor: detection latency ≤ 1h median, blast radius ≤ 5 peers, recovery ≤ 24h. v2.0 (compressed — delivered by Story 11.3 at 25/30-host) / **v2.2 (full 100-host)** [PHASE-MOVE per John] **[DELTA-2026-07-06: full 100-host retagged v2.5 → v2.2 — engineering-ownable, belongs in the functional-completeness wave, not the adoption phase]**
...
- **NFR-Sec-18** [NEW, Story 11.4c / ADR-051]: **Enterprise identity assertion (OIDC)** — capability-authorization requests may be governed by a cryptographically verified principal behind out-of-kernel `IdentityAssertionPort` (`maos-domain`) with reference `maos-sso` OIDC static-JWKS verifier. Required guarantees: explicit RS256/ES256 algorithm allowlist; reject `alg:none`; reject HS256-with-RS256-public-key confusion; enforce signature, issuer, audience, `exp`, and `nbf`; configured-but-down SSO denies issuance rather than falling open to `spirit_pid`. Honesty clause: this delivers one reference OIDC path; SAML/social IdPs and production discovery/caching adapters are additive future work. Gate: `check-enterprise-identity`; anchor: ADR-051.
- **NFR-Sec-19** [NEW, Story 11.4c / ADR-051]: **Opt-in adapter-store at-rest AEAD** — adapter stores can seal rows via out-of-kernel `KeyManagementPort` and reference `maos-secrets` envelope helper; ciphertext must differ from plaintext, right key opens, wrong key fails, and configured-but-down KMS refuses sealed writes rather than silently writing plaintext. Honesty clause: encryption is opt-in and scoped to adapter-store rows (loom-lite Collective / audit-style stores); Option-A plaintext remains the default when KMS is unconfigured; kernel-core Private/Shared at-rest encryption, Vault/cloud KMS, and OS keyrings are deferred additive adapters. Gate: `check-enterprise-identity`; anchor: ADR-051.
...
- **NFR-Aud-11:** SIEM export at v2.0; OpenTelemetry adapter at v1.0. Story 11.4c delivers the SIEM export half via `maos-siem`: read-only Transparency Log export through `query_with_redaction` before NDJSON + CEF projection and RFC5424 framing, with non-vacuous count reporting (empty TL = N/A, not green zero). Honesty clause: network sinks beyond the reference projection are additive; HTTPS/network sinks must be TLS-only, plaintext/file sinks localhost-only. Gate: `check-enterprise-identity`; anchor: ADR-051.
...
- **NFR-Test-8** [PHASE-SPLIT per Story 11.7 / ADR-053]: **Black-box third-party trial v1.0 — N=12 stratified** (≥4 with no prior MAOS contribution; ≥3 who've never written Rust Spirit; ≥2 who've never written Rust at all; ≥2 non-English-native; ≥1 working offline-only). 14-day no-DM-support window. Floor: **≥10/12 produce working signed Spirit binary** that loads on fresh Host VM, runs ≥1000 frames, halt-recall ≥0.85 on the class-appropriate subset. Wilson CI [0.552, 0.962] meaningful at N=12; meaningless at N=5. Auditable via SBOM + signing chain re-loaded on clean VM by CI bot. **v2.0 delivers the infrastructure:** `check-trial-attestation` derives clean-reload/SBOM/signing/halt facts, uses an in-house Chinese-wall proxy as advisory proof-of-mechanism, asserts SBOM/signing at v2.0, and defers CycloneDX/SBOM emission to v2.5. **v2.5/Epic-14 owns the genuine external N=12 trial.** Minor releases use NFR-Onb-1 (12-author onboarding) as proxy [COST-COMPRESSION per Murat].
...
- **NFR-Doc-6** [PHASE-MOVE per John]: Localization v1.0 = **Korean only** (shipped); ~~Japanese + Chinese-simplified at v1.5~~. `LOCALES.md` with glossary lock — terms NEVER translated: Spirit, Worker, kernel, ADR identifiers, error codes. **[DELTA-2026-07-06, recording the Epic 11 §8 decision of 2026-06-29: ja + zh-Hans deferred INDEFINITELY after the Epic-10 fabricated-translation incident (§A2: "Japanese" was Korean/machine-fabricated). en + ko are the supported locales. Re-introduction requires real human translation + Unicode-script-identity gate (ja = kana + 0 Hangul; zh-Hans = simplified-Han + 0 kana + 0 Hangul) + native-reviewer runbook. "MT + script-pass" must NEVER be claimed as real translation. No committed target version.]**
...
- **NFR-Scale-2** [PHASE-SPLIT per John + cost-compression per Murat]: **25-host churn test at v2.0** (delivered by Story 11.3); **100-host churn at v2.2** (cost compression: 100→30 hosts at v2.0 same churn-events-per-week). **[DELTA-2026-07-06: second half retagged v2.5 → v2.2 — engineering-ownable scale closer, moved into the functional-completeness wave]**
...
- **NFR-Ops-11** [NEW, Mary]: Multi-operator tenancy isolation — primitive-reservation only at v1.0 (declared as primitive-reserved in namespace grammar so v0.5 grammar lock doesn't paint us into a corner; full implementation v1.5+). Per-operator namespace, per-operator transparency-log shard, per-operator capability-token signing key, per-operator GDPR-erasure scope. v1.0 (reserved — shipped); **v2.2 (full implementation)**. **[DELTA-2026-07-06: "v1.5+ (implemented)" did not happen at v1.5; full multi-operator/multi-tenant implementation is retagged to the v2.2 functional-completeness wave — it is a Reza-journey prerequisite (per-team data residency), not an adoption artifact.]**
...
- **NFR-Tenancy-1:** Explicit single-tenant per kernel instance commitment through v2.0; multi-tenant primitive-reserved at v1.0 per NFR-Ops-11; ~~full multi-tenant out of scope before v2.5~~ **full multi-tenant lands at v2.2 [DELTA-2026-07-06: retagged — Reza-journey prerequisite in the functional-completeness wave; the single-tenant guarantee holds through v2.0 exactly as declared, and the boundary stays loud: multi-tenancy arrives as an explicit v2.2 capability, never as a silent assumption]**. v0.1 (declared); v2.0 (single-tenant guaranteed); v2.2 (multi-tenant implemented).
...
**v2.2 (functional completeness) [DELTA-2026-07-06 per Lunarpulse (operator) + John]:** J3 Team Nexus journey closure. Reza Cortex journey closure (NFR-Scale-5 capacity envelope). 100-host Cortex churn (NFR-Scale-2 second half — retagged from v2.5). Multi-operator / multi-tenant full implementation (NFR-Ops-11 / NFR-Tenancy-1 — retagged from v2.5). FR37 vetting machinery with internal vetter keys. v2.0 remainder sweep.

**v2.5 (ecosystem, non-gating):** FKCS-populated by genuine external authors (NFR-Test-5 second half). External N=12 trial cohort. RTL layout support (NFR-Doc-7 follow-on). Vetter ecosystem maturity (accredited external vetters issuing FR37 attestations). Adoption outcomes (cert bodies, external Spirits, consortium case study) — measured, never scheduled.
