...
**[DELTA-2026-09-04 per Lunarpulse (operator), bmad-correct-course]** The phases above stand unchanged. The 2026-09-04 review (`planning-artifacts/review-2026-09-04/`) found 22 of 66 FRs live from a product binary and no release tag after seven "shipped" phases; the sprint tracker's `done` was true at the harness layer and false at the operator layer. Execution is therefore re-sequenced as six epics — **15 (W0 green at HEAD + first tag), 16 (W1 ⇒ v0.1/J0), 17 (W2 ⇒ v0.3/v0.5), 18 (W3 ⇒ v0.8/J1), 19 (W4 ⇒ v1.0), 20 (W5 ⇒ v1.5–v2.2 reality)** — each closing on one operator-runnable exit command recorded in its epic file. No FR is added, removed or weakened; phase milestones are re-anchored to those commands.

**[DELTA-2026-09-04 R2 per Lunarpulse (operator), bmad-correct-course Round 2]** The preflight of the six epics found that the v0.1 "single-Spirit subprocess form" was never built (all reference Spirits are in-process; the CliWrapper Worker and the WASM host are the only out-of-process forms). Ratified **Fork C — Spirit forms by trust tier (ADR-060)**: `rust-inproc` is the first-party form (kernel-distribution code, never admitted from a registry); the **WASM component is the third-party and polyglot form** (isolated by construction, as this document already says for v2.0); `cli_wrapper` hosts agent CLIs under a T3 profile with an egress allowlist and a kernel-minted credential (ADR-061). Consequences, stated rather than implied: **FR5 holds by form** (Workers by profile, third-party by construction, first-party trusted); **FR33 is served by WASM toolchains** (TypeScript via componentize-js now; Python via componentize-py later), not by per-language subprocess SDKs. The lane is seven epics (15 Foundations → 21 Multi-host), 28–40 measured weeks, each closing on a hermetic exit command that runs in CI; paid, human and external legs live in an operator lane. No other FR changes.
...
**[DELTA-2026-09-05 R3]** The R2 preflight (`epics/epic-15-21-preflight-r2-2026-09-05.md`) found all seven recovery epics `needs-rework` (none blocked) and Round 3 ratified ten implementation decisions by default: the `maos run`/`maos shell` daemon process binds the mutating loopback surface and `maosctl` discovers it via `MAOS_HOME/control.json` (D-A); Worker egress is enforced by a host-side proxy that also holds the vendor key, so no raw key reaches a Worker (D-B); WASM `log.recall` rides a maos-bin unix-socket side-channel, not the kernel bridge (D-C); one replay selector, `MAOS_INFERENCE_MODE` + `MAOS_REPLAY_CASSETTE` (D-D); the J1 digest is daemon-rendered (D-E); kernel headroom for the FLAG-Winston stories is granted per story in 15-2 (D-F); the absent-pass gate list is the 19 named in the note (D-G); I9 exemptions use the attribute path (D-H); the env registry stays a data file (D-I); the orchestrator move targets a named leaf crate (D-J). Scope, phases and journeys are unchanged; the lane's measured duration is 34–54 weeks serial / 30–48 parallel.
