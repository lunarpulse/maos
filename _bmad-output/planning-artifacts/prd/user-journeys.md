...
**Eight journeys, ordered by ship phase:** J0 Evaluator (cross-cutting; applies from v0.1 minimum-viable install onward); J-Butler (v0.3 anchor — anticipatory single-Spirit); J-Researcher (v0.5 anchor — exploratory single-Spirit); J1 Founder's Loop (v0.8 wedge demo — compositional multi-Spirit, the demo that announces the substrate); J3 Marcus Team Nexus (v2.2 — peer mesh; **[DELTA-2026-07-06]** original v1.0 tag corrected: architecture §10.7.1 deferred J3 beyond v1.5 and no epic through Epic 11 served it); J6 Diego Spirit-author (v1.0 — third-party builder; final `public-vetted` promotion leg = FR37, v2.2 machinery / v2.5 accredited vetters); J4 Mira-Nash 2 AM (v1.5 — diagnostic-architect pair); Reza single-org cross-team Cortex (v2.2 — **[DELTA-2026-07-06]** promoted from "v2.0/2.5 candidate" to committed target; Epic 11 shipped its enablers, the journey itself lands in the v2.2 functional-completeness wave per operator directive — see `prd-delta-full-spectrum-2026-07-06.md`).
...
> **[DELTA-2026-07-06]** Phase tag corrected v1.0 → **v2.2 (functional-completeness wave)**. The v1.0 tag became dishonest when the minimal architecture deferred J3 (§10.7.1) and Epics 1–11 never picked it up; Story 11.3 proved a 30-host churn *envelope*, not this journey. J3 is now a first-class planning anchor for the post-Epic-11 wave: cohort-topology operator UX, per-(peer,role) consent tuples, cohort hot-swap choreography, cross-agent halt-on-conflict, narrative team digest.
...
> **[DELTA-2026-07-06]** Promoted from Tier-3 *candidate* to **committed v2.2 target** per operator directive (functionality first). Epic 11 shipped the §10.7.2 enablers — WASM Spirit form, multi-region convergent-replication Loom, enterprise PDP/identity — but not the journey itself. Outstanding for v2.2: cross-team asymmetric consent envelopes, multi-hop distillation provenance (digest→raw across Cortex hops), multi-tenant Loom with per-team data residency, FR37 vetting machinery, NFR-Scale-5 capacity envelope.
