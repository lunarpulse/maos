...
| 4-crate vs 14-crate v0.1 scope decision | 💻 Amelia (Round 2) | Step 8 (Scoping) — resolved: v0.1 ships **kernel skeleton + Spirit ABI v0.1 + single-Spirit subprocess + placeholder Spirit + maosctl basic + SQLite persistence + Anthropic provider + T0/T1 sandbox** as a *reduced crate set* (between Amelia's 4-crate lean position and the kernel implementation guide's 14-crate full topology), sized to the 6–8 week solo phase per John's hard cap. Final crate boundary decision is implementation-team prerogative within the v0.1 capability envelope. |
