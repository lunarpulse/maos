...
| **D3** · OPEN | Fallible `record_invocation` can drop an audit event while returning `Ok` (`crates/maos-kernel-core/src/capability/mod.rs:332-351`). | Kernel-core fallible-path repair requires a **new FLAG-Winston decision**. Approve the kernel-Δ, or ratify the drop with a stated boundary. | `14-d3-audit-drop-and-legal-hold-serialization` | UNQUERYABLE — "before any Epic 14 kernel-core edit" is a code event, not a `sprint-status.yaml` transition, and the ruling is ZERO kernel-Δ so the trigger can no longer fire; RE-ANCHORED: before `14-d3-audit-drop-and-legal-hold-serialization` leaves `backlog` | Winston (FLAG-Winston) |
