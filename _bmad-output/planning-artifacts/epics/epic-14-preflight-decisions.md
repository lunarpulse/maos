...
| **D20** · CLOSED | **E12-B1 was six of eight, and it was recorded `done`.** B1's ratified text was *"decouple blocking-disposition from `CURRENT_PHASE`"*. Eight gates carried a private `const CURRENT_PHASE`; six also adopted `BindingClass`, so theirs was vestigial. **Two adopted nothing** — `check_escape_detector.rs` and `check_cohort_mesh.rs`. OBSERVED, not derived: `cargo run -p xtask -- check-escape-detector` exited 0 while emitting `::warning::Escape-detector oracle RED — would block ship at v2.0`, and reported `"passed": true`. | **RESOLVED 2026-09-07 by Story 15-3 — BOTH gates, per its decision F3. See the D20 ruling below.** `check_cohort_mesh` now carries `BindingClass::Blocking` and its Story-12.1 hand-rolled carve-out is retired into the shared predicate. `check_escape_detector` adopts **mixed per-leg** binding: legs 1–7 `Blocking`, the two seccomp legs `AdvisorySubstrate` with a **real substrate probe**, because the collapse of *"seccomp is unavailable here"* and *"the detector regressed"* into one `green=false` WAS the defect this row names. Proven both ways at HEAD. | `15-3-single-phase-source-and-exit-command-check` | Met 2026-09-07 (was: before `20-3a-gate-honesty` leaves `backlog`) | Murat + Winston |


### Re-homing 2026-09-04 (sprint-change-proposal-2026-09-04.md §6/§7)

Epic 14 closed by re-scope after `14-2c` landed. Target cells above were re-pointed mechanically to the recovery-lane vehicles; the old keys remain in the rulings below as history. ⚠ **CORRECTED 2026-09-17 (Story 16-5 preflight, §12 row 10): this prose said `16-6` in three places where every Target CELL above says `16-5`.** It was written in the round-1 numbering (`sprint-change-proposal-2026-09-04.md:68` filed the debt slot as `16-6-audit-drop-…`), superseded the same week by round 2 (`sprint-change-proposal-2026-09-04-round2.md:86`, *"Decision-register re-points: D3/D5/D7/D12/D18 → 16-5"*) — and since 2026-09-13 `16-6` has been a real and entirely different story, `16-6-maosctl-load-over-the-door`. The gate parses table cells only, so it could never see this; a human following the prose was sent to the wrong story. This is the register's own signature failure mode — *a row can be true when written and false when read* — firing on the register. Map: `14-d3`→`16-5`, `14-d4a`/`14-8`/`14-9`→`20-5`, `14-7` (D14)→`15-2`, `14-e1`/`e12-b1`/`spec-epic-5`/`14-3`→`19-4`, `14-6`→`20-4` (D17→`15-2`), `14-4`→`16-5` (D18) + `v25-canary-rollback-and-native-push`, `14-5`→`19-5`, `14-2d`→`20-3`. D3/D17/D18 deadlines re-anchored to queryable transitions; D19 left as CLOSED. Deadline re-anchor rationale (kept out of the cells so the clause parser sees one clause): **D3** — the earlier "before any Epic 14 kernel-core edit" was a code event, not a tracker transition; **D17** — "before the v2.2 wave closes" named no transition and the retro was `optional`; Story `15-2` re-derives the aggregate, so D17 targets that row and closes with its measured re-base; **D18** — anchored off 14-4, which was split; the deny-vocabulary fix is named in the `16-5` row. ⚠ **D3, D7 and D18 re-anchored again 2026-09-17** (Story 16-5 preflight, D-16-5-H) from `` leaves `backlog` `` to `` reaches `done` ``: all three fired on 16-5's own `backlog` → `ready-for-dev` transition, which is the moment the story that must discharge them BEGINS, not ends — a row that must red on its vehicle starting is the calendar tripwire `14-1` named, not a control. Each clause was REPLACED, never supplemented, because `fn deadline_clauses` splits on `;` and evaluates every clause independently.

**Re-homing amendment 2026-09-07 (Story 15-3).** **D20** was re-pointed from
`20-3a-gate-honesty` to `15-3-single-phase-source-and-exit-command-check` and CLOSED
there. Reason: 15-3 consolidates the phase source, so it is the story that necessarily
opens all eight `CURRENT_PHASE` holders — including D20's two non-adopters — and a row
whose subject is edited by an earlier story cannot lawfully wait for a later one. Leaving
...
**Decision:** finish E12-B1 on **both** gates that never adopted `BindingClass`, and do it
with the binding class applied **per leg** rather than per gate. Ratified under Story 15-3
decision **F3**.

**Why whole-gate `Blocking` was refused.** Applying the majority pattern
`dev_enforced_red_blocks(BindingClass::Blocking, true)` to `check-escape-detector` would red
CI on any host without usable seccomp. `gate_common`'s own `AdvisorySubstrate` doc comment
names this case explicitly — *"requires a substrate CI cannot always provision (Postgres,
multi-region geo, a measurement engagement, **a seccomp-capable kernel**)"* — so the shared
module was written with this gate in mind and the whole-gate adoption would have been the
wrong one.

**The defect this row names, precisely.** D20 observed that on a seccomp-blocked host the
live legs return `green=false`, so `passed == 0 && failed == 0` is false and the advisory
tail converts a RED oracle into `passed: true`. The root cause is that
`invoke_cargo_test_marker` **collapsed two different events into one `green=false`**:
*"this host cannot sandbox"* and *"the detector regressed"*. No aggregation over that single
boolean can tell them apart, which is why the fix is a third signal rather than a different
threshold.

**What shipped.**

- `check_cohort_mesh` → `BindingClass::Blocking`. Its Story-12.1 hand-rolled
  phase-independent carve-out is retired into the shared predicate — that carve-out is what
  `gate_common`'s `BindingClass` was promoted FROM, so the gate now uses the module written
  to replace it. Its two tautological asserts (comparing a local const to its own literal)
  were deleted rather than repointed at the shared const, which would have converted a
  sanctioned phase advance into a runtime panic (F4).
- `check_escape_detector` → **mixed per leg.** Legs 1–7 are `Blocking`. The two seccomp legs
  (`producer-wired-proven-red`, `detection-quality-live`) are `AdvisorySubstrate`, carrying
  a `substrate_present` fact ALONGSIDE their counts rather than folded into `green` — the
  shape of `check_multi_region_slo`'s `RawLeg`.
- **The real substrate probe.** `substrate_present` is decided by the harness's own
  genuine-unavailability signal, which is backed by a REAL sandboxed spawn against the real
  kernel (`maos-escape-detector`'s `tests/common::skip_if_sandbox_unavailable` prints its
  `SKIP` line only for `SandboxUnavailable`/`EPERM`/`ENOSYS`; every other error panics). A
  kernel-capability probe (`/proc/sys/kernel/seccomp/actions_avail`) is reported alongside as
  context. **Both are needed, and that was measured:** on the 15-3 dev host the kernel probe
  says seccomp filtering IS available (eight actions listed, `Seccomp_filters: 1`) while a
  real spawn still fails `PermissionDenied` — so a config read alone would have marked the
  substrate present and hard-failed a host that genuinely cannot sandbox.

**Proven both ways at HEAD, which is what makes this a closure rather than a claim.**
With the substrate genuinely absent the gate reports `passed: true`, `advisory: true`,
`substrate_absent: true` and a WOULD-HAVE-BLOCKED banner, with the two red legs labelled
`status: "substrate-absent"` instead of being anonymous reds. With the substrate probe
neutralised so the same two legs are seen as substrate-present, the gate reports
`passed: false`, names them in `blocking_legs`, and exits **1**. Before this story both
states produced `passed: true` — that difference IS D20.
...
- **Constraint carried to the vehicle:** `cap_audit_backpressure.rs:121-124` asserts `drops > 0`, so
  a "never drop" repair **reds a shipped test**. `14-d3-audit-drop-and-legal-hold-serialization` must rule on that
  test in the same breath.

**Vehicle:** `14-d3-audit-drop-and-legal-hold-serialization`. **Kernel-Δ: ZERO**, so D3's own deadline trigger
...
#### D3 — RE-RULED 2026-09-17 by Winston (FLAG-Winston, the row's owner of record), at the 16-5 round-table

**The ZERO-Δ half of the ruling above is AMENDED to BOTH; the class-wide half is UNCHANGED and is now the larger
half.** Recorded here, under the owner's name, rather than inside a story's decision table — a story does not get
to overturn a ruled row because its own circumstances changed, and a ruling that moves silently is how rulings rot
(Murat, same round-table).

- **What changed, and why it is not a reversal of reasoning.** The 2026-08-30 ruling chose out-of-kernel *because*
  the repair "costs zero kernel-core lines and zero re-pin." That premise is now false for the vehicle that
  actually carries it: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary` already buys an operator-authorized
  FLAG-Winston grant and a content-hash re-pin for its legal-hold serializer and its Lifecycle-Journal repair.
  Against a grant already spent, the propagate is **+1 kernel-core line and +3 `maos-domain`** — and without it
  the vehicle's own AC1 command ("a fault-injected sink makes the invocation refuse") **cannot pass**, because an
  out-of-kernel counter does not change `record_invocation`'s return value. The cost comparison the ruling rested
  on no longer describes the world; the ruling follows the measurement.
- **The class-wide half is reinforced, not weakened.** The propagate repairs **one** site. Measured at
  `df45c609`, the class is **NINE**, not the seven-plus-one this register recorded: site 8 is `fn revoke`
  (`crates/maos-capability/src/cap_tokens/mod.rs:294-296` — this register's `:280-282` is stale by 14), and
  **site 9 is new** — `crates/maos-acp/src/notification_channel.rs:59-64`, a production drop carrying its own
  unclosed `// TODO: integrate cap_audit::record_drop`.
- **This register's carried constraint is WITHDRAWN as measured-false.** It states that
  `cap_audit_backpressure.rs:121-124`'s `drops > 0` means *"a 'never drop' repair reds a shipped test."* That
  test (in `crates/maos-kernel-core/tests/`, not `crates/maos-capability/tests/`) exercises `issue`, `verify` and
  `revoke` at `:92-108` and **never calls `record_invocation`** — neither half of the repair touches it. The real
  tripwire is narrower and this register never stated it: `:106` is `adapter.revoke(…).unwrap()`, so extending
  the *propagate* discipline to site 8 panics under saturation. **Site 8 is repaired by counting, not
  propagating.**
- **Two obligations added to the vehicle, both ratified at the same round-table.** (a) The refusal's cause must
  be a **distinct** outcome — `crates/maos-bin/src/main.rs:1275` currently maps an audit failure to
  `ResearcherCollectiveError::Denied`, the same variant `:1286` uses for a real policy denial, which would
  reproduce **D18's own collapse inside D3's repair**. (b) A refusal caused by an unavailable sink **cannot be
  written to the sink**, so the class-wide counter is promoted to the *audit of last resort* and the process
  latches a named degraded state on the `Closed` path — writer-task death silences audit for the process's whole
  lifetime, which makes pressure on the audit channel a denial of service that reads as policy working normally.

**Status unchanged: `OPEN`.** The decision is re-recorded; the implementation has not landed. Per the `Status`
cell, a recorded ruling never makes an outstanding obligation invisible.
...
- **D5.1 — MERGED INTO `14-d3-audit-drop-and-legal-hold-serialization` (2026-08-30, Lunarpulse
  approved).** It was already *"routed through D3's ruling, not a second escalation: same crate, same
  ZERO-Δ fence, same moment"* — and the row's own next sentence, *"two rulings on one crate in one
  afternoon is the single-source defect this project has paid for three times,"* argued against the
  separate key it was written on. One fence, one moment, one vehicle. The former key
  `14-e2-legal-hold-erase-serialization` no longer exists; D5.1's scope is carried verbatim
  in the merged vehicle's sprint row, and the D3 row's Target and deadline now govern both halves.
...
(`14-0` AC4.2): `14-e1-erasure-attestation-honesty` (D5.2 + D5.3), `14-d3-audit-drop-and-legal-hold-serialization`
(D5.1, MERGED 2026-08-30 into D3's vehicle — it was already riding D3's ZERO-Δ ruling and a second key
for the same fence was the single-source defect the row itself named), `v25-erasure-crash-reconciliation`
(D5.4). All three targets are real `development_status` keys. Nothing substantive is owed and none is re-opened here.
