...
6. ZERO kernel-Δ @23141 — the gate is a **static scan** and a `maos-domain` registry needs **no `maos-kernel-core` source edit**; FLAG-Winston only if placement genuinely surfaces a kernel-crate-set seam.
...
6. ZERO kernel-Δ @23141 (enum variant + registry + xtask).
