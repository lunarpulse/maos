...
| Finding | Severity | Status | Resolution |
|---|---|---|---|
| `migrator.rs::run_migrator` populated `EMigratorMissing.predecessor_class` with the SUCCESSOR's class name (predecessor_class and successor_class both got `successor_manifest.class.name`) — operator-facing error messages would lie about which side caused the missing migrator | Critical | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `post_swap_monitor.rs::spawn` used `tokio::time::interval(Duration::from_secs(1))` regardless of window length — when `MAOS_AUTO_REVERT_FAST=1` shrinks the window to 300ms, the FIRST real check tick comes AT 1s which is AFTER the 300ms deadline → the fast-mode auto-revert path silently fired ZERO invariant checks per swap, making fast-mode tests structurally unable to exercise the auto-revert | Critical | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `state_codec.rs::decode` used `.unwrap_or_default()` on the `payload` field and `.unwrap_or(1)` on `envelope_version` — directly violates Story 5.2 Dev Notes carry-forward rule #1 and #7 ("No serde decode that returns a default on failure"; "No `.unwrap_or_default()` on serde failures"). Malformed payloads or missing envelope_version were silently swallowed | Critical | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `coordinator.rs` step-6 `decode` failure → coordinator always returned `HotSwapError::SchemaIncompatible` regardless of the actual decode error type (truncated CBOR / missing fields / hex-decode failure all map to a misleading "schema incompatible" verdict; the `\|e\|` binding was bound but ignored) | Critical | closed | **Patched**: match arm now distinguishes `StateCodecError::SchemaVersionMismatch` (legitimate `SchemaIncompatible` verdict) from other decode errors (mapped to `HotSwapError::Internal` with the underlying error in the message). Fixed in `crates/maos-domain/src/hot_swap.rs`. |
| `coordinator.rs` step-11 `pre_swap_halt_ids` snapshot for `PostSwapMonitor` was collected from `halt_registry.pending_halt_ids()` AFTER step 3's `validate_swap_halt_continuity` (which may have drained halts as a side effect per the existing Patch "may drain halts before swap commits"). Result: the monitor compared post-drain ids against post-swap ids → halt-set-loss invariant could never trigger because the baseline was already drained | Critical | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `coordinator.rs` step-7 `match compat { CrossMajor => …, _ => payload }` used a silent wildcard arm — violates Story 5.2 Dev Notes carry-forward rule #5 ("No silent fall-through wildcards on enum match arms"). `SchemaCompat::Breaking` (if reachable through any future decode path) would be silently treated as same-major and proceed to swap-in | Major | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `saga.rs::compensate` reported `DiscardSuccessor` compensation with `phase_str = "SnapshotTaken"` — but `DiscardSuccessor` fires from step 9 (`on_swap_in` failure) which is `SagaPhase::SwapInFired`, NOT `SnapshotTaken`. Operator journal entries would mis-attribute the failure phase | Major | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `precheck.rs::HotSwapPrecheck::check` always populated BOTH `drained_count` and `migrated_count` with the SAME value (`pending_halt_ids().len()`) regardless of verdict — `SafeDrained` verdict should report `drained=N, migrated=None`; `SafeMigrated` should report `drained=0, migrated=N`; error verdicts should report both `None`. The current behavior would confuse `maosctl spirit hot-swap-precheck` JSON consumers | Major | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `post_swap_monitor.rs::check_invariants_static` was dead code — never called from any path, unconditionally returned `None`. Likely a leftover from review patch iteration | Minor | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `post_swap_monitor.rs::tests::monitor_window_uses_env_var` test was a meaningless no-op — set the env-var then just constructed a snapshot and dropped it; never asserted the window duration logic it claimed to test → false confidence in CI | Minor | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `coordinator.rs` had three dead imports (`HaltContinuityError`, `validate_halt_set`, `SpiritArchive`) and unused `HaltId` | Minor | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `archive.rs` had unused `Path` import | Minor | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
| `precheck.rs::check` had unused `predecessor_pid: u32` parameter (signature-only, never read) | Minor | closed | fixed in `crates/maos-domain/src/hot_swap.rs` |
