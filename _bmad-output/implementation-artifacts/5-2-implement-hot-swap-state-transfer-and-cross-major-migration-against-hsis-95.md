...
**Backfill review findings from formal Claude-Opus-4.7 review pass (2026-05-25, Epic 5 retro §A2):**

This backfill audits the commit at SHA `78e0180` using the Acceptance Auditor / Blind Hunter / Edge Case Hunter triad. The earlier (in-line) review captured 30+ patches at landing; this pass surfaces additional findings that escaped triage. Apparent-clean inline patches were applied where they were closeable without restructuring; remaining items are tagged `deferred` with explicit forward references.

| Finding | Severity | Status | Resolution |
|---|---|---|---|
| `migrator.rs::run_migrator` populated `EMigratorMissing.predecessor_class` with the SUCCESSOR's class name (predecessor_class and successor_class both got `successor_manifest.class.name`) — operator-facing error messages would lie about which side caused the missing migrator | Critical | closed | **Patched**: predecessor_class now derived from `scb.manifest.class.name` (the predecessor SCB). Both class names and the successor version are computed once at function entry; all three `EMigratorMissing` construction sites use the shared values. |
| `post_swap_monitor.rs::spawn` used `tokio::time::interval(Duration::from_secs(1))` regardless of window length — when `MAOS_AUTO_REVERT_FAST=1` shrinks the window to 300ms, the FIRST real check tick comes AT 1s which is AFTER the 300ms deadline → the fast-mode auto-revert path silently fired ZERO invariant checks per swap, making fast-mode tests structurally unable to exercise the auto-revert | Critical | closed | **Patched**: cadence now scales with window — `Duration::from_millis(50)` when `window <= 1s`, `Duration::from_secs(1)` otherwise. Fast mode gets ~5–6 ticks within its 300ms window. |
| `state_codec.rs::decode` used `.unwrap_or_default()` on the `payload` field and `.unwrap_or(1)` on `envelope_version` — directly violates Story 5.2 Dev Notes carry-forward rule #1 and #7 ("No serde decode that returns a default on failure"; "No `.unwrap_or_default()` on serde failures"). Malformed payloads or missing envelope_version were silently swallowed | Critical | closed | **Patched**: both fields now propagate `StateCodecError::CborDecode(_)` on absence; payload hex-decode failures propagate explicit error; envelope_version range-checked against `u32::MAX`. |
| `coordinator.rs` step-6 `decode` failure → coordinator always returned `HotSwapError::SchemaIncompatible` regardless of the actual decode error type (truncated CBOR / missing fields / hex-decode failure all map to a misleading "schema incompatible" verdict; the `\|e\|` binding was bound but ignored) | Critical | closed | **Patched**: match arm now distinguishes `StateCodecError::SchemaVersionMismatch` (legitimate `SchemaIncompatible` verdict) from other decode errors (mapped to `HotSwapError::Internal` with the underlying error in the message). |
| `coordinator.rs` step-11 `pre_swap_halt_ids` snapshot for `PostSwapMonitor` was collected from `halt_registry.pending_halt_ids()` AFTER step 3's `validate_swap_halt_continuity` (which may have drained halts as a side effect per the existing Patch "may drain halts before swap commits"). Result: the monitor compared post-drain ids against post-swap ids → halt-set-loss invariant could never trigger because the baseline was already drained | Critical | closed | **Patched**: `pre_swap_halt_ids_baseline` captured BEFORE step 3 in `initiate_swap`; the post-swap monitor snapshot now uses that baseline instead of a fresh post-drain read. Loss detection is now structurally possible. (Note: this stacks with the existing deferred Patch — once Story 5.3 lands per-PID drain, the I14 wrapper itself stops draining and this baseline still holds.) |
| `coordinator.rs` step-7 `match compat { CrossMajor => …, _ => payload }` used a silent wildcard arm — violates Story 5.2 Dev Notes carry-forward rule #5 ("No silent fall-through wildcards on enum match arms"). `SchemaCompat::Breaking` (if reachable through any future decode path) would be silently treated as same-major and proceed to swap-in | Major | closed | **Patched**: exhaustive match — `SameMajor` passes payload through; `CrossMajor` invokes migrator; `Breaking` returns `HotSwapError::SchemaIncompatible`. Each arm explicit. |
| `saga.rs::compensate` reported `DiscardSuccessor` compensation with `phase_str = "SnapshotTaken"` — but `DiscardSuccessor` fires from step 9 (`on_swap_in` failure) which is `SagaPhase::SwapInFired`, NOT `SnapshotTaken`. Operator journal entries would mis-attribute the failure phase | Major | closed | **Patched**: `DiscardSuccessor` now reports `"SwapInFired"`. Also patched: the `RestorePredecessor` arm's wildcard `_ => "restore_predecessor"` replaced with an exhaustive 6-arm match over all `SagaPhase` variants (NotStarted, HaltContinuityChecked, SwapOutFired, SnapshotTaken, SwapInFired, Committed). |
| `precheck.rs::HotSwapPrecheck::check` always populated BOTH `drained_count` and `migrated_count` with the SAME value (`pending_halt_ids().len()`) regardless of verdict — `SafeDrained` verdict should report `drained=N, migrated=None`; `SafeMigrated` should report `drained=0, migrated=N`; error verdicts should report both `None`. The current behavior would confuse `maosctl spirit hot-swap-precheck` JSON consumers | Major | closed | **Patched**: verdict-dependent assignment — `SafeDrained → (Some(pending), None)`; `SafeMigrated → (Some(0), Some(pending))`; error verdicts → `(None, None)`. Also removed unused `remaining_halt_count` helper and unused `HaltContinuityError`/`Arc` imports. |
| `post_swap_monitor.rs::check_invariants_static` was dead code — never called from any path, unconditionally returned `None`. Likely a leftover from review patch iteration | Minor | closed | **Patched**: removed the dead function. |
| `post_swap_monitor.rs::tests::monitor_window_uses_env_var` test was a meaningless no-op — set the env-var then just constructed a snapshot and dropped it; never asserted the window duration logic it claimed to test → false confidence in CI | Minor | closed | **Patched**: replaced with `monitor_window_resolves_from_env_var` that exercises the actual resolution function and asserts `300ms` (fast) vs `30s` (production). Test is now meaningful. |
| `coordinator.rs` had three dead imports (`HaltContinuityError`, `validate_halt_set`, `SpiritArchive`) and unused `HaltId` | Minor | closed | **Patched**: all four imports removed. |
| `archive.rs` had unused `Path` import | Minor | closed | **Patched**: removed. |
| `precheck.rs::check` had unused `predecessor_pid: u32` parameter (signature-only, never read) | Minor | closed | **Patched**: renamed to `_predecessor_pid` to silence the warning; signature preserved for caller compatibility. Note: this parameter ought to be removed once the public API surface stabilizes — flagged for Story 5.4 cleanup. |
| `state_codec.rs::encode` and `decode` use `serde_json::json!` macro then CBOR-encode the resulting `Value` structure — this is a layering smell. Bytes are hex-encoded to a string before CBOR-encoding (2x wire bloat) because `serde_json::Value` can't represent CBOR byte-strings (major type 2). The architecturally correct approach is a real `#[derive(Serialize)]` struct with `serde_bytes` for the payload field — would halve the encoded size and remove the JSON→CBOR detour | Major | deferred → Story 5.5x | Story 5.5x ships the subprocess-form wire encoding and will reach for the same envelope; the codec is the natural place to converge on a proper `StateEnvelopeWire` struct then. Wire-format change is observable (existing tests would fail) so deferring to that boundary avoids two ABI-touching commits. |
| `coordinator.rs::initiate_swap` step-11 spawns `Arc::new(Self { ... })` each swap — constructs a fresh `HotSwapCoordinator` instance solely as a vehicle to give the `PostSwapMonitor` task an `Arc<Self>`. The inner adapters are all `Arc::clone`'d correctly so the §A5 "no two instances of shared-state" gate is not violated, but each swap leaks a small allocation that persists until the monitor task ends (30s normally, 300ms in fast mode) | Moderate | deferred → Story 5.5e | Cleanest fix is making `HotSwapCoordinator` always wrapped in `Arc<Self>` at construction and threading the Arc through `initiate_swap` via `&Arc<Self>`. Touches the composition-root signature in `maos-bin/src/main.rs` and the kernel API re-export; defer to §13.1 measurement-gate ADR when the broader rust-inproc latency budget is being audited. |
| `saga.rs::HotSwapSaga` exposes `pub` fields (`pre_swap_snapshot`, `predecessor_pid`, `started_at`, `current_phase`) without the Story 5.1 `check-pub-field-constructors` doc-attribute discipline. The struct has a `new()` constructor but the gate is keyed on the doc-attribute marker, not the constructor presence | Moderate | deferred → Story 5.3 | Saga is an INTERNAL helper of the coordinator (not re-exported through `api.rs`), so the gate's current allow-list classification handles this. If the gate is tightened in Story 5.3 (Epic 5 retro §A2 closure), revisit. |
| `coordinator.rs::initiate_swap` step-9 SystemTime → Duration uses `.unwrap_or_default()` for journal timestamps; same in `saga.rs::compensate` and `coordinator.rs::auto_revert` — clock-going-backwards on a host produces timestamp 0 for journal entries silently. Three call sites | Moderate | deferred → Story 5.3 | Story 5.3 already plans to centralize `wall_clock_now_ns()` helpers per Epic 5 retro action items; consolidate these three call sites into that helper. Until then, the value of "0 timestamp" is recognizable by audit tooling as a clock-skew marker. |
| `LifecycleEvent` enum in `maos-domain/src/invariants/i10.rs` is NOT `#[non_exhaustive]` despite Story 5.2 AC9 stating "additive on `#[non_exhaustive]` enums". Three new variants (`HotSwapAborted`, `HotSwapAutoReverted`, `HotSwap`) were added; current consumers use `matches!` only (no exhaustive `match`) so no break in tree, but a hostile downstream `cargo public-api` consumer doing `match` on this enum would observe a break | Moderate | deferred → Story 5.4 | Adding `#[non_exhaustive]` is itself ABI-affecting (forces consumers to add wildcard arms) and should land alongside the next planned ABI-fence. Story 5.4 is the FIRST production consumer of `LifecycleEvent::HotSwap` from `UpgradeOrchestrator` so it is the natural carrier. |
| `precheck.rs` defines a SECOND `PrecheckVerdict` struct alongside `maos_domain::hot_swap::PrecheckVerdict` (different module, same field set, no constructor on the kernel one). CLI serializes the kernel one. The duplication risks drift if either side adds a field | Moderate | deferred → Story 5.4 | Consolidate to ONE `PrecheckVerdict` (domain) at the same window when `LifecycleEvent` gets `#[non_exhaustive]` (related ABI-fence work). |
| `precheck.rs::check` does NOT detect or report `EMigratorMissing` verdict — only `SafeDrained`/`SafeMigrated`/`HaltContinuityViolation`/`SchemaIncompatible`. AC7 explicitly enumerates `EMigratorMissing` as a verdict variant; the precheck currently has no manifest-path argument so it cannot inspect `[migrates_from]` to detect a missing migrator. Operators get false greens on cross-major-without-migrator scenarios at precheck time | Major | deferred → Story 5.4 | The precheck signature needs `successor_manifest: &SpiritManifestBundle` (not just versions) to inspect `[migrates_from]`. Story 5.4's `maosctl spirit upgrade` calls this internally with the full successor manifest already loaded — extending the signature there is cheaper than retrofitting Story 5.2's CLI shell-out path. Track as a deferred Patch with the explicit forward reference. |
| `saga.rs::compensate` is NOT actually compensating — it only journals + emits IAC frames. The real SCB restore happens IN-LINE inside `coordinator.rs::initiate_swap` at the step-9 failure branch. Naming "saga.compensate(DiscardSuccessor)" suggests the saga performs the restore, but it doesn't. Misleading API | Moderate | deferred → Story 5.3 | Refactor saga to OWN the restore — pass the `spirits` Arc + pre_swap_snapshot at construction so `compensate(DiscardSuccessor)` actually does the SCB swap-back atomically. Story 5.3 wants this same shape for crash compensation. Defer to that consolidation. |
| `saga.rs::ensure_snapshot` panics via `assert!` on misuse — kernel correctness anti-pattern (panic-on-misuse). Should propagate `Result` and let the coordinator decide compensation | Minor | deferred → Story 5.3 | Same refactor window as the previous item — when saga owns the restore, it gains explicit `Result<(), SagaError>` return on compensation, and the pre-snapshot invariant becomes type-checked at construction (no panic possible). |
| HSIS 300-scenario corpus generation was deferred to a script-based generator (Task 10.1 dev notes: "Full 300 scenario generation deferred to `xtask/src/gen_hsis_corpus.rs`"). Each generated scenario JSON shape is uniform `~37-42 lines` (per `wc -l`) but the SEMANTIC content is templated — many scenarios within the same class are near-duplicates with only ID + a small variation differing. The `hsis_runner.rs` test only ASSERTS the loader (`pass rate ≥ 95%` test exists, but never wired through to actually exercise scenarios via `TestKernel`). The CI gate `nfr-rel-3-hsis-95pct` runs but trivially passes because the scenarios load without runtime execution | Major | deferred → Epic 6 | The substrate's first FUNCTIONAL HSIS run is blocked on `TestKernel::with_hot_swap_coordinator()` having full I14-aware setup AND `run_hsis_scenario` actually executing the swap. Both are 3–4-day expansions. Recommendation: Epic 6 opens with a "HSIS-300 first functional pass" story; Story 5.2's substrate (loader + corpus + gate scaffold) is the prerequisite that this story SHIPPED but the actual measurement gate is still informational. |
| `tests/integration/maosctl_hot_swap_precheck.sh` smoke test exists but is not gated by CI (no entry in `discipline.yml`); only the inline clap test exists | Moderate | deferred → Story 5.4 | Shell smoke tests for `maosctl spirit hot-swap-precheck` should join `maosctl spirit upgrade` (Story 5.4's new verb) in a single integration-shell suite under `tests/integration/`. Story 5.4 has natural reason to author that surface. |
| `coordinator.rs::resolve_pid` does a linear scan of the spirits map for each `initiate_swap` call. With ≥100 Spirits the spirit_id→pid lookup becomes O(N) on a hot path | Moderate | deferred → Epic 6 | Same lookup pattern as Story 5.1's `SpiritSchedulerAdapter::resolve_pid` (which has the same O(N) shape). Resolution requires a reverse-index `BTreeMap<spirit_id, pid>` maintained alongside the SCB map. Defer until Spirit count actually exceeds ~10 in any planned scenario. |

#### Pattern observations (Claude-Opus-4.7 review, 2026-05-25)

1. **deepseek-v4-pro substitution risks materialized as predicted.** Two patterns from `feedback_deepseek_v4_pro_patterns.md` recurred: (a) integration plumbing weakness — the `EMigratorMissing` field swap (predecessor_class=successor_class) is a classic "right shape, wrong wiring" bug; (b) env-var threading — the `MAOS_AUTO_REVERT_FAST` window collapse correctly shrunk the window but missed that the tick interval ALSO needed to shrink. Both bugs passed every existing unit test because the tests never asserted on the field semantics or on actual auto-revert firing in fast mode.

2. **Serde silent-fallback anti-pattern persisted despite explicit warning in Dev Notes.** The Story 5.2 Dev Notes section "Carryover from Epic 4 retro — patterns to specifically AVOID" lists `.unwrap_or_default()` on serde paths as rule #1 and rule #7 (separately). The state codec STILL contained both: payload and envelope_version. This suggests dev_record self-review checklists are not detecting CBOR-codec contexts; consider adding a `grep -n unwrap_or_default` walker to the per-story self-review gate.

3. **Wildcard match arms recurred in two places** (`coordinator.rs` step-7, `saga.rs::compensate`) despite explicit warning rule #5. The pattern is sticky for the same reason as #2 — review tooling doesn't enforce it. Consider an `xtask` `check-wildcard-arms` gate that flags `_ =>` arms on `HotSwapError`, `SagaPhase`, `PrecheckOutcome`, `SchemaCompat`, `HsisAttackCategory` enums (the explicit list from Dev Notes #5).

4. **Composition-root completeness gate passed, but the coordinator-as-Arc pattern (`Arc::new(Self { ... })` per swap) is a subtle drift.** The gate validates that adapters are constructed once; it does NOT validate that a coordinator never re-wraps itself. Consider extending `check-composition-root-completeness` to flag `Arc::new(Self {` patterns inside async methods on types that already have an existing composition-root `Arc<Self>`.

5. **Test quality regression: the `monitor_window_uses_env_var` test was a textbook no-op test** — it set env-vars, constructed a snapshot, then dropped it without asserting anything. This is the kind of pattern that lets bugs ride the green-CI signal. The `bmad-testarch-test-review` skill should be run on Story 5.2 + Story 5.3 + Story 5.4 deferred test files as a batch retro action.

#### Verification of inline fixes

- `cargo build -p maos-kernel-core`: PASS (0 errors, 51 warnings — all pre-existing or in other modules)
- `cargo test -p maos-spirit-abi --lib`: PASS (20/20)
- `cargo test -p maos-domain --lib hot_swap`: PASS (7/7)
- `cargo test -p maos-kernel-core --test hot_swap_cross_major_migration`: PASS (5/5) — verifies migrator fix didn't regress
- `cargo test -p maos-kernel-core --test hook_dispatch_swap_hooks`: PASS (3/3)
- `cargo test -p maos-kernel-core --test hot_swap_saga_compensation`: PASS (1/1)
- `cargo test -p maos-kernel-core --test hot_swap_halt_continuity_test`: PASS (1/1)
- `cargo test -p maos-kernel-core --test hot_swap_same_major_lifecycle`: PASS (1/1)
- `cargo run -p xtask -- check-pub-field-constructors`: BLOCKED at HEAD (post-5.5e: xtask has missing modules `check_review_findings_resolved` + `check_dev_record_completeness`; pre-existing). The gate was green at the Story 5.2 commit per dev record line 1224.
- `cargo run -p xtask -- check-composition-root-completeness`: BLOCKED at HEAD (same reason). Was green at 5.2 commit per dev record line 1225.
- `cargo build --workspace`: BLOCKED at HEAD (post-5.5d/5.5e: `maos_registry::fixture_replay`, `maos_providers::fixture_replay`, `maos_mcp::fixture_replay`, `maos_acp::fixture_replay`, `maos_bench::fixture_replay` all gate-cfg'd-out behind `feature = "fixture_replay"` that callers in `maos-bin/src/main.rs` don't request). NOT introduced by Story 5.2; tracked as Epic 5.5 carry-forward.

#### HSIS corpus gaps for Epic 6 prep
- The HSIS 300-corpus exists on disk (verified by `git show 78e0180 --stat` count of 300 scenario JSON files across 6 class directories) but the scenarios are templated near-duplicates — semantic coverage is not what 300 distinct hand-authored scenarios would yield. Epic 6 should open with hand-authored scenario diversification.
- The `hsis_runner.rs` test currently asserts on LOADER behavior only; the actual swap-execution path (`run_hsis_scenario(&kernel, scenario)`) is a stub. Epic 6 needs to wire the runner to `TestKernel::with_hot_swap_coordinator()` and actually fire `initiate_swap` per scenario.
- The `nfr-rel-3-hsis-95pct` CI gate is in `discipline.yml` but trivially passes today. Epic 6's "HSIS functional gate" story needs to make this gate actually validate the 95% floor.
- The `halt-continuity-corpus-v0/` 12 scenarios DO exercise actual swap behavior via the AC4 integration test — that gate is real. Use this as the template for the HSIS runner upgrade.

---

**Backfill review pass — Epic 6 retro §A2 closure (2026-05-28, calibrated post-hoc review)** | **Reviewers:** Blind Hunter + Edge Case Hunter + Acceptance Auditor

This pass re-audits Story 5.2 at HEAD (commit `5108e6e`) on the Epic 6 retro day. The 2026-05-21 inline review (30+ patches) and the 2026-05-25 Claude-Opus-4.7 backfill (23 rows) closed most surfaces. This pass surfaces residual findings that escaped both prior triages and verifies prior-deferred items are still tracked.

| # | Finding | Severity | Status | Resolution |
|---|---|---|---|---|
| 1 | `state_codec.rs::decode` rejects all single-major cross-major upgrades. Lines 152-158 use `pred_major.wrapping_sub(succ_major) >= 2` as the cross-major guard, but when `succ_major > pred_major` (the normal upgrade direction, e.g. pred=1 → succ=2) the wrapping subtraction yields `u32::MAX` which is `≥ 2` → returns `SchemaVersionMismatch` BEFORE the coordinator reaches the migrator path. Cross-major upgrades structurally cannot succeed against `crates/maos-kernel-core/src/hot_swap/state_codec.rs`. `hot_swap_cross_major_migration.rs` tests passed because the test fixtures use `encode(state, 1)` and `decode(blob, 1)` with same expected version, hitting the same-major branch. AC2's "cross-major swap with `[migrates_from]`" path is unreachable through the production coordinator. | Critical | open | Replace the wrapping-arithmetic check with `pred_major.abs_diff(succ_major) >= 2` (or `(pred_major as i64 - succ_major as i64).abs() >= 2`). File: `crates/maos-kernel-core/src/hot_swap/state_codec.rs:152-158`. Add a coordinator-path integration test that calls `initiate_swap` with predecessor schema_version=0x0001_0000 and successor schema_version=0x0002_0000 and asserts the migrator path fires (not decode error). |
| 2 | `post_swap_monitor.rs::check_invariants` produces guaranteed false-positive `HaltSetLoss` on any swap whose Step-3 `validate_swap_halt_continuity` legitimately drained halts. `pre_swap_halt_ids_baseline` is captured at `coordinator.rs:146-151` BEFORE Step 3 drains, so the baseline contains drained halts; `check_invariants` at `crates/maos-kernel-core/src/hot_swap/post_swap_monitor.rs:117-123` then asserts every baseline halt MUST appear in the post-swap registry, but those drained halts are intentionally gone. Every successful swap that drained ≥1 halt will auto-revert within 50ms of commit (fast mode) or 1s (production). The prior backfill row "pre_swap_halt_ids_baseline captured BEFORE step 3" addressed half the bug (the baseline IS captured before drain), but the invariant check semantics flipped: a baseline halt missing from the post-swap registry is supposed to indicate "successor silently lost it", not "the I14 wrapper legitimately drained it". | Critical | open | Distinguish "drained-by-I14" from "lost-by-successor". Either (a) subtract the set returned by `SwapVerdict::SafeDrained { drained_count }` from `pre_swap_halt_ids_baseline` so the monitor only checks for halts that should have persisted, or (b) capture `pre_swap_halt_ids` AFTER Step 3's drain (using the wrapper's typed verdict as authoritative). File: `crates/maos-kernel-core/src/hot_swap/coordinator.rs:141-151` + `crates/maos-kernel-core/src/hot_swap/post_swap_monitor.rs:99-149`. Add a regression test that exercises drained-and-still-passes vs successor-loses-halt. |
| 3 | `coordinator.rs::initiate_swap` Step 5 `snapshot()` hook failure returns `HotSwapError::SwapOutFailed { error: format!("snapshot failed: {outcome:?}") }` — wrong variant. Snapshot is between `on_swap_out` (Step 4) and `on_swap_in` (Step 9); a snapshot failure is a serialization/state-extraction failure, not a swap-out hook failure. The error variant misattributes the root cause to operators reading journal entries. AC1's typed-error contract enumerates seven `HotSwapError` variants — `SwapOutFailed` is one of them but the variant message format `"snapshot failed: ..."` makes the wrong variant ambiguous. | Major | open | Either (a) add a `HotSwapError::SnapshotFailed { spirit_id, error }` variant (additive on `#[non_exhaustive]`) and map Step 5 to it, or (b) use `HotSwapError::Internal(format!("snapshot failed: ..."))` for snapshot-specific failures. File: `crates/maos-kernel-core/src/hot_swap/coordinator.rs:228-237`. |
| 4 | `coordinator.rs::initiate_swap` Step 6 immediately re-encodes the snapshot blob via `state_codec::encode(&state_blob, predecessor_state_schema_version)` then `decode`s it back. The Spirit's `snapshot()` hook produces opaque bytes (per AC6, the trait method returns `Vec<u8>` with no kernel-side schema contract); the kernel then wraps these bytes in a CBOR envelope with a schema_version field, then immediately decodes the envelope it just produced. The encode/decode roundtrip serves only as a schema-version assertion path but adds 2x serialization cost on every swap and breaks the "snapshot is already CBOR" intent from AC1 step 5 ("snapshot() — produces CBOR-encoded state"). The state codec is being used as a schema-version validator, not a wire codec. | Major | open | Either (a) strip the redundant encode and make the codec a pure validator over the predecessor's raw `snapshot()` output assuming it conforms to `StateEnvelope`, or (b) commit to kernel-side envelope-wrapping (current design) but document that `snapshot()` returns RAW state, not CBOR, and update AC6 trait doc accordingly. Pick one. File: `crates/maos-kernel-core/src/hot_swap/coordinator.rs:254-271` + AC1 step 5 + `crates/maos-spirit-abi/src/lifecycle.rs::Spirit::snapshot`. |
| 5 | `state_codec.rs::decode` reports `expected_schema_version: 0` as the `expected` field on the `SchemaVersionMismatch` error when the CALLER passes 0 (lines 131-137). The blob's encoded `schema_version` IS valid (encode rejects 0); the error mislabels which side is wrong — operators reading the error see "expected 0, got 1" and chase the encoder when the caller is buggy. | Minor | open | Distinguish caller-validation errors from version-mismatch via a distinct variant (e.g. `StateCodecError::InvalidExpectedVersion`) or return `Err(CborEncode("expected_schema_version must be > 0"))` symmetrically with the encode path. File: `crates/maos-kernel-core/src/hot_swap/state_codec.rs:131-137`. |
| 6 | `coordinator.rs::initiate_swap` Step 8 replaces `*scb` with a `new_scb` Arc rather than mutating the existing SCB in-place. AC1 step 7 explicitly says "modify SCB in-place... so the captured Arc sees the update" (per the 2026-05-21 patch on line 1317-1318). The prior patch claim says "FIXED: Step 8 now returns error on missing SCB; step 9 fetches successor SCB from map after atomic swap" — but the implementation creates a fresh `Arc::new(SpiritControlBlock::new(...))` (coordinator.rs:308-320) rather than in-place mutation. The captured `predecessor_scb` Arc (line 127-135) still points to the OLD object. This is structurally fine because Step 9 fetches a fresh `successor_scb` from the map (line 329-334), but it means: (a) any external Arc holders (capability registry, halt registry write-side) keep references to the PREDECESSOR object even post-commit; (b) the assertion in the docstring that the swap is "atomic against the spirits lock so the predecessor SCB is recoverable" is true but for a different reason than stated. | Moderate | open | Either align the implementation with AC1's "modify in-place" promise (would require `SpiritControlBlock` interior-mutability for `spirit_obj` and `manifest` fields, likely via `Mutex` or `ArcSwap`), or update AC1 step 7's wording to match the current "replace Arc under write-lock" semantics. The current code works correctly for the swap path but creates a documentation/intent drift. File: `crates/maos-kernel-core/src/hot_swap/coordinator.rs:303-326` vs AC1 step 7 + AC6 dev-record line 1317-1318. |
| 7 | `migrator.rs::run_migrator` line 89 maps `MigratorError::NotImplemented` → `HotSwapError::EMigratorMissing`. This conflates two distinct failure modes: (a) the successor's manifest declares `[migrates_from] = ["0.3.x"]` but the Spirit author forgot to implement the `migrate()` hook (default returns `NotImplemented`), and (b) the manifest doesn't declare `[migrates_from]` at all (already handled at line 52-61). Both produce identical operator-facing error messages, but they have different remediation: (a) implement the hook in the Spirit's `migrate()` body; (b) add `[migrates_from]` to the manifest. The error variant should distinguish so operators know whether to file a Spirit-author bug or a manifest fix. | Moderate | open | Add `HotSwapError::MigratorNotImplemented { class, version }` variant and map `MigratorError::NotImplemented` to it; keep `EMigratorMissing` for the manifest-absent case. File: `crates/maos-kernel-core/src/hot_swap/migrator.rs:84-99` + `crates/maos-domain/src/hot_swap.rs::HotSwapError`. |
...
| 11 | `saga.rs::HotSwapSaga` exposes `pub` fields without the `check-pub-field-constructors` doc-attribute discipline (line 54-57). Prior 2026-05-25 backfill row deferred to Story 5.3 with "Saga is an INTERNAL helper". The gate did NOT tighten in Story 5.3 (per Epic 6 retro carry-forward §A2/§A5/§A6 — the §A5 gate `check-review-findings-resolved` wired but the `check-pub-field-constructors` gate was NOT extended). Still deferred. | Minor | deferred → Story 7.x | Carry forward the same target — once the gate is tightened to non-api-exposed internal helpers, add `/// allowed-pub-field: internal saga helper, kernel-only` doc attributes or convert to private fields with accessor methods. File: `crates/maos-kernel-core/src/hot_swap/saga.rs:53-58`. |
| 12 | `precheck.rs::HotSwapPrecheck::check` cannot produce the `EMigratorMissing` verdict (lines 53-72). Prior 2026-05-25 backfill row deferred to Story 5.4. Story 5.4 shipped (Epic 5 close per git log) but precheck.rs:77-83 still maps only `SafeDrained | SafeMigrated | HaltContinuityViolation | SchemaIncompatible | EMigratorMissing` — the `EMigratorMissing` arm exists in the match but no code path produces it (lines 53-72's match only emits the other four). Cross-major-without-migrator at precheck time silently reports as `SafeMigrated` or `SafeDrained` depending on halt set. | Major | deferred → Story 7.x | Extend `HotSwapPrecheck::check` signature to take the successor manifest's `migrates_from` section + predecessor version; emit `EMigratorMissing` when cross-major detected AND `migrates_from` absent or no version match. File: `crates/maos-kernel-core/src/hot_swap/precheck.rs:31-94` + AC7 verdict enumeration. |
| 13 | `post_swap_monitor.rs::check_invariants` (iii) output-shape contract check is still a placeholder comment (lines 144-146: "placeholder; full implementation requires sampling journaled frames... Deferred to Story 5.4"). Story 5.4 shipped; the check did NOT land. AC3 Arm 3 promises output-shape-regression auto-revert; the corpus scenario `auto_revert_window` (per AC5) has no implementation to exercise. | Moderate | deferred → Story 7.x (operator-visible auto-revert path) | Wire journal frame-shape sampler into the post-swap monitor. Should land before any production deployment exercises `[output_shape]` schema changes. File: `crates/maos-kernel-core/src/hot_swap/post_swap_monitor.rs:144-146`. |
| 14 | Task 3.3 in the task list is unchecked (`[ ] 3.3 Compose root at crates/maos-bin/src/main.rs: construct exactly ONE Arc<HotSwapCoordinator>`) yet the prior 2026-05-25 backfill row claims `FIXED: HotSwapCoordinator constructed exactly once after SpiritSchedulerAdapter wiring`. The task checkbox state contradicts the patch claim. Either the wiring was reverted, or the checkbox was never re-marked. AC9's `check-composition-root-completeness` gate would have flagged this if it had been wired in discipline.yml (per Epic 6 retro §A5/§A6 — the gate xtask exists but is not wired in discipline.yml). | Minor | open | Verify by inspecting `crates/maos-bin/src/main.rs` for the `HotSwapCoordinator::new(...)` call; if present, mark Task 3.3 as `[x]`; if absent, restore the wiring per the prior patch claim. Files: `_bmad-output/implementation-artifacts/5-2-...md` Task 3.3 checkbox + `crates/maos-bin/src/main.rs`. |
...
1. **Deferred items from the 2026-05-25 backfill compounded forward across Stories 5.3 / 5.4 / 5.5x / 5.5e / Epic 6 without any of them being closed at the original target window.** Findings 8, 9, 11, 12, 13, 15 above all redirect "deferred → Story 5.X" targets that came and went. The §A2/§A5/§A6 carry-forward pattern noted in the Epic 6 retro now applies to Story 5.2's individual deferred items too — every story that promised to absorb a deferred finding instead deferred it again. Consider a "defer-target landed" gate that flags deferred-to targets whose story closed without addressing the deferred finding.

2. **Cross-major path is structurally unreachable** (finding #1) despite full inline-patch density on the migrator module — the bug is in the codec's compatibility-detection arithmetic, not the migrator itself. The integration test `hot_swap_cross_major_migration.rs` exercises `matches_version_pattern` directly, never the coordinator-driven cross-major decode path. The HSIS `cross_major_migration` attack category is therefore unexercised even when the runner activates. This is a "test boundaries don't match production boundaries" gap.

3. **Halt-set-loss invariant always false-positives** (finding #2) — the prior 2026-05-25 backfill PATCHED `pre_swap_halt_ids_baseline` to capture before drain, which is correct for the spec's intent ("baseline halt set"), but the invariant check semantics weren't updated to account for "drained halts are legitimately absent post-swap". Without the integration test that exercises a non-empty-halt-set + successful-drain swap end-to-end with PostSwapMonitor wakeup observed, this regression rides green CI. The 12-scenario `halt-continuity-corpus-v0/` tests the I14 wrapper but not the monitor's invariant logic.

4. **The "deferred → Story X.Y" idiom decays unless mechanized.** Findings 8/9/10/11/12/13/15 ride the same decay pattern Epic 6 retro §A2/§A5/§A6 documented at the bridge-gate layer. The remediation is the same: a mechanical gate that surfaces "deferred-to target closed without resolution" as a CI failure on the closing story's PR. Stories carry the forward-debt structurally; absent mechanization, the only signal is human re-discovery in the next retro.

#### Verification of new findings

- `crates/maos-kernel-core/src/hot_swap/state_codec.rs:152` — verified by inspection; `pred_major.wrapping_sub(succ_major) >= 2` with succ > pred underflows.
- `crates/maos-kernel-core/src/hot_swap/post_swap_monitor.rs:117-123` — verified by inspection; loop asserts baseline halt-id ⊆ current registry, no exception for legitimately-drained halts.
- `crates/maos-kernel-core/src/hot_swap/coordinator.rs:228-237, 373-376, 424-436, 583-594` — verified by inspection; matches prior-backfill descriptions.
- `crates/maos-kernel-core/src/hot_swap/migrator.rs:89` — verified by inspection; `NotImplemented → EMigratorMissing` mapping confirmed.
- `crates/maos-kernel-core/src/hot_swap/precheck.rs:53-83` — verified by inspection; `EMigratorMissing` verdict variant exists in match but no producer.
- Task 3.3 checkbox `[ ]` at line 918 of the story file — verified by Read; the 2026-05-25 backfill claim of composition-root wiring is unverified without inspecting `crates/maos-bin/src/main.rs` (NOT inspected in this backfill pass).
