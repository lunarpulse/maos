...
- [x] **Task 3 — `HotSwapCoordinator` body + composition-root wiring** (AC: 1, 7)
  ...
  - [x] 3.3 Compose root at `crates/maos-bin/src/main.rs`: constructs exactly one `Arc<HotSwapCoordinator>` at `main.rs:3219-3233` with the shared scheduler SCB map and adapters.
...
- **Task 3**: Created `crates/maos-kernel-core/src/hot_swap/` with the 12-step `HotSwapCoordinator`, precheck, and auto-revert; `crates/maos-bin/src/main.rs:3219-3233` now constructs exactly one coordinator from the shared scheduler/adapters. `hook_dispatch_swap_hooks.rs` passes its three hook-surface cases.
...
3. **Historical halt-set false-positive, now closed** (finding #2) — the 2026-05-25 patch captured the baseline before drain but initially treated legitimately drained halts as post-swap loss. Current HEAD snapshots only post-drain survivors and tests both legitimate drains and genuine survivor loss.
...
- `crates/maos-kernel-core/src/hot_swap/coordinator.rs:379-390` and `post_swap_monitor.rs:113-126,213-254` — current-HEAD audit verified finding #2 closed: only post-drain survivors are monitored, with positive and negative coverage.
...
- Task 3.3 and the completion note were corrected in the 2026-08-12 audit after verifying `crates/maos-bin/src/main.rs:3219-3233` constructs exactly one production `Arc<HotSwapCoordinator>` from the shared scheduler/adapters.
