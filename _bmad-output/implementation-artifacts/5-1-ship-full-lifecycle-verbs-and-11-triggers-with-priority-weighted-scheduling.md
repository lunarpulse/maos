...
Current-HEAD closure files (2026-08-13):

- `crates/maos-spirit-abi/src/identity.rs`
...
| 4 | `KernelLifecycleResolver::resolve_verb` used an unconditional nested Tokio-runtime bridge from sync code. | Medium | **closed** | `crates/maos-kernel-core/src/scheduler/verb_resolver.rs` now runs the async operation on a dedicated thread with a fallible current-thread runtime and typed bridge errors. `crates/maos-kernel-core/tests/scheduler_five_verb_lifecycle.rs` proves calls without Tokio and inside a current-thread runtime return `LifecycleError` without unwinding. |
...
| 6 | Budget warning/exceeded discriminants were internal to the Transparency Log and absent from the canonical ABI `FrameKind`. | Medium | **closed** | `crates/maos-spirit-abi/src/identity.rs` preserves discriminants 12/13 as `BudgetWarning`/`BudgetExceeded`; `crates/maos-domain/src/frame.rs` carries typed budget envelopes, and `crates/maos-kernel-core/src/scheduler/hook_dispatch.rs` delivers them through the invoking Spirit's bounded mailbox before returning the hook outcome. |
...
| 8 | `five_verb_lifecycle_routes_through_scheduler` did not assert that any of the five hooks fired. | Medium | **closed** | `crates/maos-kernel-core/tests/scheduler_five_verb_lifecycle.rs` retains shared hook counters and asserts `on_load`, `on_start`, `on_pause`, `on_resume`, and `on_unload` each fire exactly once. |
| 9 | `hook_dispatch_budget_envelope.rs` covered only three of AC2's five scenarios. | Low | **closed** | `crates/maos-kernel-core/tests/hook_dispatch_budget_envelope.rs` now covers a 900ms hook crossing the 800ms warning boundary, typed mailbox delivery, and a panicking hook whose payload preview is preserved. |
| 10 | `KernelLifecycleResolver::director_identity` accepted an unvalidated `String`. | Low | **closed** | `crates/maos-kernel-core/src/scheduler/verb_resolver.rs` makes construction fallible and rejects blank/whitespace identities; `crates/maos-kernel-core/tests/scheduler_five_verb_lifecycle.rs` covers the typed `LifecycleError::Admission` result. |
...
| 12 | DRR selection incremented deficits without deducting a quantum from the chosen SCB. | Low | **closed** | `crates/maos-kernel-core/src/scheduler/scheduler_loop.rs` atomically deducts `SCHEDULER_QUANTUM` with `fetch_update` as part of selection; `crates/maos-kernel-core/tests/drr_priority_weighted_dispatch.rs` exercises repeated weighted and equal-weight dispatch. |
| 13 | The one-second hook cap scheduled its nominal 80% warning at the full one-second timeout. | Low | **closed** | `crates/maos-kernel-core/src/scheduler/hook_dispatch.rs` uses millisecond arithmetic (`cap_seconds * 800`), and `crates/maos-kernel-core/tests/hook_dispatch_budget_envelope.rs` proves a 900ms hook emits a warning before the one-second timeout. |
...
**2026-08-13 closure disposition:** all seven formerly open rows are closed with production behavior and focused executable evidence; zero current rows remain open or decision-needed.
