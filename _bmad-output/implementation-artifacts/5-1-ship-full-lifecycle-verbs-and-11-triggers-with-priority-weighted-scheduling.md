...
| 4 | `KernelLifecycleResolver::resolve_verb` still uses an unconditional nested Tokio-runtime bridge from sync code. | Medium | open | Patch required — `crates/maos-domain/src/lifecycle.rs:137-149` promises this synchronous trait never panics, but `crates/maos-kernel-core/src/scheduler/verb_resolver.rs:39-53` panics without a Tokio runtime and on a current-thread runtime. Convert the boundary to async or provide a bridge that returns typed errors in both contexts; add regressions for both. |
...
| 6 | Budget warning/exceeded discriminants remain internal to the Transparency Log and absent from the canonical ABI `FrameKind`. | Medium | open | Patch required — log recall now distinguishes both labels, but `maos_domain::frame::IacFrame` still uses `maos_spirit_abi::identity::FrameKind`, whose enum and `from_u8` omit discriminants 12/13. Add the ABI variants and consuming dispatch, with matching typed payload/routing if these events cross IAC. |
...
| 8 | `five_verb_lifecycle_routes_through_scheduler` still does not assert that any of the five hooks fired. | Medium | open | Patch required — `crates/maos-kernel-core/tests/scheduler_five_verb_lifecycle.rs:112-164` moves the counter-bearing Spirit into `load` and checks only successful returns plus SCB removal. Retain shared counter state and assert each hook fired exactly once on the dispatched object. |
| 9 | `hook_dispatch_budget_envelope.rs` still covers only three of AC2's five scenarios. | Low | open | Patch required — add an 80%-before-timeout case asserting `BudgetWarning80 { fired: true, .. }` and a panicking-hook case asserting `Panicked` with the captured payload preview; current lines 64-134 cover only skip, within-budget, and exceeded-budget. |
| 10 | `KernelLifecycleResolver::director_identity` is still accepted as an unvalidated `String`. | Low | open | Patch required — production passes `"director"`, but `verb_resolver.rs:25-35,104-118` permits and journals empty/whitespace input. Make construction fallible or require a validated director/principal identity type. |
...
| 12 | DRR selection still increments deficits without deducting a quantum from the chosen SCB. | Low | open | Patch required — `scheduler_loop.rs:34-66,431-439` returns the winner without deduction; tests manually compensate while the live dispatch path at `main.rs:4032-4054` does not. Deduct atomically inside the picker and update tests, or explicitly rename the no-deduct API and enforce deduction at every dispatch point. |
| 13 | The one-second hook cap still schedules its nominal 80% warning at the full one-second timeout. | Low | open | Patch required — `hook_dispatch.rs:513-551` changed the old zero-delay bug to `max(1, cap_seconds * 4 / 5)`, but accepted one-second caps still warn at 100%, not 800 ms. Use subsecond arithmetic and cover a hook completing between 800 ms and one second. |
...
**2026-08-12 current-HEAD disposition for the seven formerly open rows:** all 7 remain open.
