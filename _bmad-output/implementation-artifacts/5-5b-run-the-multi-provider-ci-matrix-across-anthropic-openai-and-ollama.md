...
**Review date:** 2026-05-28 (Epic 6 retro §A2 backfill — calibrated post-hoc review incorporating prior inline patches from the 2026-05-25 Claude-Opus-4.7 pass) | **Reviewers:** Blind Hunter + Edge Case Hunter + Acceptance Auditor

| # | Finding | Severity | Status | Resolution |
|---|---|---|---|---|
| 1 | [Blind Hunter] CI matrix test-name filter mismatch — `cargo test ... ${{ matrix.provider }}` would match nothing because the actual test fns are `matrix_anthropic` / `matrix_openai` / `matrix_ollama`. Untouched, the matrix would silently "pass" (zero tests run per cell) and produce empty reports. | Critical | **closed** | `.github/workflows/multi-provider.yml` line 27 now filters on `matrix_${{ matrix.provider }}`; `crates/maos-providers/tests/multi_provider_matrix.rs` confirms the three test fns. |
| 2 | [Blind Hunter] Workflow upload/consumer name mismatch — `matrix` job uploads `multi-provider-${sha}-${provider}.json` (three artifacts), but the original `report-aggregate` job invoked drift-check against `multi-provider-${sha}.json` (no such artifact). Drift-check would exit 1 on missing file every run. | Critical | **closed** | `.github/workflows/multi-provider.yml` lines 50-61 add a `jq -s 'add'` step concatenating the three per-provider arrays into the merged path before drift-check reads it. |
| 3 | [Blind Hunter] `ProviderSwitchedPayload` was serialized but the bytes were never delivered to the journal entry — `LifecycleEntry` had no `payload` field, so `journal_lifecycle` recorded only the variant tag, defeating the from→to audit trail in AC5. | Critical | **closed** | `crates/maos-domain/src/invariants/i10.rs` adds `payload: Option<Vec<u8>>` to `LifecycleEntry`; `crates/maos-kernel-core/src/security/mod.rs:280-286` now passes `payload_bytes` via `Some(payload_bytes)`. |
| 4 | [Blind Hunter] TL `intent` string was recorded pre-call with only the declared provider id — fallback walks would record the primary's id even when the secondary actually served the response, breaking operator audit of fallback events promised in §What this story IS step (c) "infer:primary->actual:model". | Critical | **closed** | `crates/maos-kernel-core/src/inference/mod.rs:334-340` now reads `result.provider_attribution.provider_id` post-response and formats `infer:{primary}->{actual}:{model}`; `mock_provider_round_trip_logs_inference_call` asserts `entries[0].intent.contains("->")`. |
| 5 | [Edge Case Hunter] Empty-string `MAOS_OPENAI_API_KEY` passed construction — `std::env::var` returns `Ok("")` for an explicitly-set-but-empty variable; the `OpenAiProvider::new` happy path would emit `Authorization: Bearer ` on every call, leaking malformed credentials into OpenAI logs while looking configured. | Medium | **closed** | `crates/maos-providers/src/openai.rs:37-39` now returns `ProviderError::Unconfigured` on `api_key.is_empty()` after the env read. |
| 6 | [Edge Case Hunter] `xtask check-multi-provider-drift` used `partial_cmp(...).unwrap()` while sorting f64 values — any NaN in `response_text_len`/`input_tokens`/`output_tokens` (legitimately possible from broken provider fixtures) would panic and crash the report-aggregate job. | Medium | **closed** | `xtask/src/check_multi_provider_drift.rs:63` uses `a.total_cmp(b)` (NaN-safe total order). |
| 7 | [Blind Hunter] Drift-check serialized output via `serde_json::to_string_pretty(...).unwrap()` and `to_string(...).unwrap()` on each outlier — violates Story 5.4 §1373 (no `.unwrap` on serde paths); a malformed Value at runtime would crash the gate. | Medium | **closed** | `xtask/src/check_multi_provider_drift.rs:122-130` and `140-143` match the result and emit `eprintln` + return 1 / `<serialization error: e>` instead of panicking. |
| 8 | [Edge Case Hunter] `multi_provider_matrix.rs` swallowed serde + I/O failures via `unwrap_or_else(|_| "[]".into())` and `.ok()` on `fs::write` — a broken report writer would silently produce an empty report, pass the matrix test, then fail report-aggregate with no upstream signal. | High | **closed** | `crates/maos-providers/tests/multi_provider_matrix.rs:179-182` propagates serde + write errors via `unwrap_or_else(|e| panic!(...))` with explicit messages naming the failing path. |
| 9 | [Blind Hunter] `security/mod.rs::admit_spirit` held the `provider_history` `Mutex` across `serde_json::to_vec` AND `journal.journal_lifecycle(...)` — the journal call performs fsync; every admission would serialize through one global mutex blocked on disk I/O. | High | **closed** | `crates/maos-kernel-core/src/security/mod.rs:259-266` now scopes the mutex narrowly: insert under lock, capture diff, drop lock, then serialize + journal outside. Comment block documents the rationale. |
| 10 | [Blind Hunter] `ProviderSwitchedPayload` pub fields had no `#[doc = "Construct via ::new"]` annotations and no matching `::new` constructor — violates Story 5.4 §A4 / AC5 last bullet (every new pub serde field carries the constructor discipline). | High | **closed** | `crates/maos-domain/src/invariants/i10.rs` adds the five doc annotations and a `ProviderSwitchedPayload::new` constructor. |
| 11 | [Acceptance Auditor] `InferencePortAdapter::complete` originally defaulted via `router.registered_ids().first().cloned()` — alphabetically first BTreeMap key, not the operator-declared `default_id`. Worked coincidentally only because `"anthropic"` sorts first; a future `"amazon-bedrock"` registration would silently flip the default. | Medium | **closed** | `crates/maos-kernel-core/src/inference/router.rs:98-100` exposes `default_id()` accessor; `crates/maos-kernel-core/src/inference/mod.rs:253-257` now uses it with the prior shape's rationale documented inline. |
| 12 | [Blind Hunter] `matches!(Variant(ref s) if ...)` paren placement parsed as experimental guard pattern in `tests/multi_provider_routing.rs` and `tests/provider_switched_journal.rs` (4 sites) — AC2's integration tests would not compile on stable Rust. Additionally, kernel-core's `fixture_replay` feature did not propagate `maos-providers/fixture_replay`, so enabling the kernel-core feature would not enable the provider-side replay impl. | Critical | **closed** | All 4 paren placements fixed to `Variant(ref s)) if ...` in `crates/maos-kernel-core/tests/multi_provider_routing.rs` and `crates/maos-kernel-core/tests/provider_switched_journal.rs`; `crates/maos-kernel-core/Cargo.toml` `fixture_replay` feature now includes `maos-providers/fixture_replay`. |
| 13 | [Edge Case Hunter] `tests/multi_provider_routing.rs` constructed `BTreeMap<String, Arc<FixtureReplayProvider>>` for the fallback test — type mismatch with the router's `BTreeMap<String, Arc<dyn Provider>>`; would not compile. | High | **closed** | `crates/maos-kernel-core/tests/multi_provider_routing.rs` adds explicit `Arc<dyn Provider>` coercion + typed BTreeMap declaration. |
| 14 | [Edge Case Hunter] `air_gap_ollama_test.rs` was originally written against a `MockTransport` so `take_io_journal().is_empty()` was trivially true — the test claimed to verify the air-gap invariant but actually verified the mock had no recorder. The structural validation (fixture replay actually exercises the router and the kernel's IO journal stays empty) replaces the misleading shape. | High | **closed** | `crates/maos-kernel-core/tests/air_gap_ollama_test.rs` is now `#![cfg(all(feature = "io_call_journal"))]` and uses `FixtureReplayProvider` against `MultiProviderRouter`; doc-comment clarifies that full unshare-net validation is Story 9.4. |
| 15 | [Acceptance Auditor] `malformed-rejected/` manifest fixture dir originally contained 3 well-formed fixtures — the rejection-path tests would fail to assert the validator's error path because the fixtures had no defects. | High | **closed** | `crates/maos-kernel-core/tests/fixtures/manifest/providers/**` now contains genuinely malformed content (unsupported id, empty endpoint, bad pin); manifest-field-coverage gate continues to pass. |
| 16 | [Edge Case Hunter] Matrix runner originally resolved fixture paths relative to CWD (`tests/fixtures/...`) — running the test from any directory other than the crate root would fail to find fixtures with a vague I/O error. | High | **closed** | `crates/maos-providers/tests/multi_provider_matrix.rs:62-64` switches to `env!("CARGO_MANIFEST_DIR")`-relative resolution for both cases dir and reports dir. |
| 17 | [Acceptance Auditor] Error-fixture JSONs (`case_provider_429_rate_limit.json`, `case_provider_500_error.json`) were defined but the runner never wired them into `Err(...)` paths — `ProviderError` propagation was untested by the matrix despite the fixtures' intent. | Medium | **closed** | `crates/maos-providers/tests/multi_provider_matrix.rs:98-126` constructs `FixtureReplayProvider::new(vec![Err(...)])` when `expected.error` is set, mapping `"rate_limit"` → 429, `"server_error"` → 500, else `Transport`. |
| 18 | [Edge Case Hunter] `provider_missing_api_key_is_unconfigured` test originally called `env::remove_var("MAOS_OPENAI_API_KEY")` globally, contaminating any other test in the same process that depended on the env var. | High | **closed** | `crates/maos-providers/src/openai.rs:279-292` now saves the prior value, removes it, asserts, and restores on exit. |
| 19 | [Blind Hunter] `provider_switched_journal.rs` constructed `CapabilitiesRequired` without the `mcp` field (Story 5.5c's additive fanout) — compilation broke after 5.5c merged. | High | **closed** | `crates/maos-kernel-core/tests/provider_switched_journal.rs` adds `mcp: McpCapabilities { servers: vec![] }` default. |
| 20 | [Acceptance Auditor] `check-pub-field-constructors` xtask gate failed at HEAD on `ProvidersSection.{primary,fallback}` and `McpSection.servers` — gate-internal false negative (non-greedy `[\s\S]*?` regex consumes across impl boundaries); both types do have valid `::new` constructors. | High | **closed** | `xtask/pub-field-constructor-allowlist.toml` adds allowlist entries; the underlying gate rewrite against syn-AST walk is deferred to Epic 6 toolchain hardening. |
| 21 | [Acceptance Auditor] Smoke arm step 5 originally printed `"emitted"` without verifying the ProviderSwitched journal entry — observability lie (printed evidence claimed something the arm did not check). | Medium | **closed** | `crates/maos-bin/src/main.rs:2285-2288` now prints `"outcome":"fixture_replay_path"` with a note that admission-journal verification lives in the integration test, not the smoke arm. |
| 22 | [Acceptance Auditor] `manifest_path` was synthetic/empty in `ProviderSwitchedPayload` — operators reading the journal would see no path correlation back to the manifest. | Medium | **closed** | `crates/maos-kernel-core/src/security/mod.rs:272-275` now threads a `format!("manifest:{spirit_id}")` synthetic identifier; comment block documents that admit_spirit receives parsed sections, not the real path — full filesystem-path threading deferred to Story 7.3 ComplianceClaim wiring. |
| 23 | [Edge Case Hunter] Drift-check `partial_cmp` panic on NaN AND drift-check serializer panic AND missing `::notice` GitHub Actions annotation — three related operator-tooling gaps tracked together under the §finding 7 patch line. The annotation now surfaces drift outliers as PR-visible notices per story narrative line 511. | Medium | **closed** | `xtask/src/check_multi_provider_drift.rs:144-148` emits `::notice file=...::Drift outlier in fixture {fixture}` per outlier; verified against `xtask/tests/fixtures/multi-provider-reports/with-outlier.json`. |
| 24 | [Blind Hunter] `security/mod.rs::admit_spirit` mapped `serde_json::to_vec` failure to `SecurityError::T3AdmissionFailed(...)` — misleading variant assignment; serialization errors are not T3 admission failures. The mismatch obscures incident triage but does not lose data (the journal entry is just not appended on serde failure, which is itself a separate concern). | Low | **deferred → 6.x** | `crates/maos-kernel-core/src/security/mod.rs:279`. Variant fanout (introducing `SecurityError::JournalSerialization`) crosses too many call sites to land in 5.5b backfill; documented for Epic 6 error-taxonomy cleanup. |
| 25 | [Edge Case Hunter] Smoke arm step 6 prints literal `"outbound_calls":0` instead of `journal.len()` — assertion already enforces invariant (`assert!(journal.is_empty())`), but the printed evidence is static, weakening the observability discipline from `[[feedback_lunarpulse_observability_preference]]`. | Low | **deferred → 9.4** | `crates/maos-bin/src/main.rs:2291-2293`. Story 9.4 lands the operator-surface air-gap validator with dynamic counters; fold this print-site into the wider observability rewrite. |
| 26 | [Edge Case Hunter] `provider_history` HashMap grows unbounded — no eviction on Spirit Unload. At v0.5-α admit throughput the memory ceiling is far off, but the unbounded shape is real. | Low | **deferred → 9.4** | `crates/maos-kernel-core/src/security/mod.rs:94`. Eviction requires the operator-facing spirit-deregister hook landing in Story 9.x; carried forward in `deferred-work.md`. |
| 27 | [Acceptance Auditor] Drift-check silently collapses missing/null `fixture_id` into `"unknown"`, grouping malformed rows together rather than rejecting the input. | Low | **deferred → 9.4** | `xtask/src/check_multi_provider_drift.rs:28`. Edge case for malformed reports; the operator-tooling hardening lands at Story 9.4 where the drift-check becomes a first-class operator surface. |
| 28 | [Acceptance Auditor] Story 5.5b commit SHA was UNKNOWN per Epic 5 retro §A8 — commit subsumed into `248f23b` (titled `5-5a-...`) on 2026-05-23. The retro recommendation is one-story-per-commit so future `git log --grep` resolves cleanly. | Info | **closed** | Closes §A8 commit-attribution gap; the 81-file diff is now fully attributed via the File List section above (Completion Notes + File List cross-reference `_bmad-output/implementation-artifacts/sprint-status.yaml` and the 81 files including `crates/maos-bin/src/main.rs`). |
