...
| 21 | Stale test name `sandbox_config_t3_parseable_but_rejected_at_admission` described an obsolete invariant while its body proved parsing only. | Low | **closed** | Renamed to `sandbox_config_t3_parseable` at `crates/maos-manifest/src/manifest.rs:2391`; `t3_effective_tier_admitted` separately proves signed-lock-backed T3 admission. |
| 22 | Runtime auto-detection discarded the Podman error and returned only the Docker failure on a dual miss. | Low | **closed** | `crates/maos-kernel-core/src/security/sandbox/t3/runtime_detect.rs` returns `T3Error::RuntimeUnavailableDiagnostics { podman, docker }` when both probes fail, while forced modes retain their single-runtime error; `auto_mode_retains_both_runtime_probe_diagnostics` proves both causes survive. |
...
| 24 | Used admission parameter `_manifest` retained an intentionally-unused prefix. | Low | **closed** | `crates/maos-kernel-core/src/security/mod.rs` names the parameter `manifest` and consumes `manifest.image_pin` when selecting an entry from the verified lock. |
...
| 33 | `spirit-inspect` emitted hardcoded PID/runtime/image/tier values instead of live SCB state. | High | **closed** | The fabricated one-shot arm is removed. `crates/maos-control/src/lib.rs` serves an authenticated loopback endpoint backed by the daemon's live scheduler `Arc`; `crates/maos-cli/src/subcommands.rs` requires endpoint/token configuration and maps 401/404 to typed failures. Successful T3 spawn identity is committed through the live SCB report path. `cargo test -p maos-control` proves bearer rejection, missing reports, and exact report serialization. |
...
| 35 | Image verification compared an attested registry manifest digest with runtime `.Id` output and parsed the entire stdout buffer as one SHA. | Medium | **closed** | `crates/maos-kernel-core/src/security/sandbox/t3/image_verify.rs` queries `image inspect --format '{{json .RepoDigests}}'`, normalizes the attested and runtime repositories, selects the matching `@sha256:` manifest digest, and preserves distinct malformed/missing versus mismatch errors. Unit tests cover registry ports, tags/digests, repository selection, malformed JSON, and missing repositories. |
| 36 | The shipped default lock contained a test signer and was accepted by admission/smoke without trust-anchor verification. | High | **closed** | `crates/maos-kernel-core/src/security/sandbox/t3/image_lock.rs` produces verified lock/attestation types only after trust-anchor and signature validation and rejects the shipped placeholder as `T3Error::PlaceholderImageLock`; `crates/maos-kernel-core/src/security/sandbox/t3/spawn.rs` accepts only the verified type. Sandbox admission and image-verification tests cover the fail-closed path. |
...
**2026-08-13 closure disposition:** all six open current-HEAD rows are closed with production behavior and focused executable evidence; zero rows remain open or decision-needed.
