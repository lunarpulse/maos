...
|---|---|---|---|
| `image_pin` verification not performed during T3 admission | Critical | **closed** | Added resolve_pin/default_attestation check in `security/mod.rs:182-194` |
| `inspect_image_sha` is a stub — every T3 spawn fails | Critical | **closed** | Implemented real runtime inspect call in `image_verify.rs:102-126` |
| 25 escape scenario files not authored | High | **closed** | Authored 25 scenarios across 5 categories in `t3-escape-corpus-v0/` |
| `T3EscapeCorpus::load` and test driver do not exist | High | **closed** | Created `t3_escape_corpus.rs` loader + 4 test assertions (25/25 load, categories, unique IDs) |
| T3 CI jobs not in aggregate `needs` list | High | **closed** | Added 3 T3 jobs to aggregate needs in `discipline.yml:882` |
| `SandboxApplied` journal not emitted at T3 admission time | High | **closed** | Added conditional `SandboxApplied` journal in `security/mod.rs:228-234` |
| `CapAuditEvent::SandboxBlock` not emitted by `quarantine_spirit` | High | **closed** | Added `audit_sender` parameter + emit in `quarantine.rs:33-64` |
| Adversarial subcommand + escape-block missing from smoke arm | High | **closed** | Added Step 4 adversarial spawn in `main.rs` smoke arm |
| Trust anchor env-var reader (`MAOS_T3_IMAGE_TRUST_ANCHOR_PUB_HEX`) absent | High | **closed** | Added `read_trust_anchor_pub()` in `image_verify.rs:17-28` |
| Revocation quarantine fallback matches all T3Error variants | High | **closed** | Split match: `QuarantineRequiresSubprocessForm` → fallback; other `Err(e)` → `RevocationError::QuarantineFailed` |
| `inspect_container_host_pid` races with container startup | Medium | **closed** | Added 10-attempt retry with progressive backoff in `spawn.rs:130-160` |
| Container name collision on rapid respawn | Medium | **closed** | Extended name suffix with XOR-mixed upper/lower nonce bits in `argv.rs:46-51` |
| `probe_runtime` timeout computed but never enforced | Medium | **closed** | Removed dead `timeout` variable in `runtime_detect.rs` |
| `T3ImageAttestation` Deserialize bypasses `new()` validation | Low | deferred → Story 5.5e | Partially mitigated by `parse_signed_image_attestation` re-checks + crypto verify |
| `OnceLock` caches runtime detection failure permanently | Low | deferred → v0.7+ | Transient boot failure permanently disables T3; acceptable for v0.5-α |
| **Backfill 2026-05-25** — `build_runtime_argv` synthesizes its own container name (`maos-{spirit_id}-{lower}-{xor}`) but `spawn_t3` uses `parent.container_name` from `T3SpawnContext` for `inspect` / Drop cleanup — names diverge; inspect fails → 10 retries × 50ms backoff → `T3Error::Inspect` after ~5s; Drop's `stop`/`rm` target the wrong name and leak the actual container | Critical | **closed** | Pass `container_name: &str` through `build_runtime_argv` (argv.rs:21-29), call sites in `spawn.rs:94-103` and the smoke arm now share one identity; argv test updated. Caller (`spawn_t3`) owns collision-resistance via `boot_nonce`-derived suffixing on the `T3SpawnContext.container_name` it constructs |
| **Backfill 2026-05-25** — `spirit-inspect` arm writes JSON report to stderr via `eprintln!` instead of stdout per AC5 + Dev Note ("prints as JSON to stdout"); `maosctl spirit inspect ... \| jq` returns empty | Medium | **closed** | Changed `eprintln!` → `println!` in `crates/maos-bin/src/main.rs:1968-1976` and documented the contract inline |
| **Backfill 2026-05-25** — Test driver `crates/maos-eval/tests/t3_escape_corpus.rs` is schema-only (loads 25 scenarios, asserts shape/categories/uniqueness) — does NOT actually spawn the synthetic Spirit in T3 container, execute the attack command, query TL for `FrameKind::SandboxBlock` frames, or assert ≥1 block per scenario (AC4 §test driver §1-3). The "full 25-scenario execution deferred" note in Tasks 6 covered the corpus authoring; execution remains aspirational | High | deferred → Story 7.x (NFR-Sec-14 audit) | Driver expansion to spawn+exec+TL-query against pinned image needs a Linux runner with Podman pre-installed; same gating as the `t3-escape-corpus` CI job already declared in `discipline.yml`. Documented as a v0.5 ship-gate observability gap (corpus exists, execution path proven through smoke arm Step 4 single-scenario, scaled execution arrives at NFR-Sec-14 audit) |
| **Backfill 2026-05-25** — `build_runtime_argv` maps `spec.resolved_caps.fd_max` to `--pids-limit`; FD count (RLIMIT_NOFILE) and process count (cgroup `pids.max`) are semantically distinct kernel limits. Sharing the slot risks under-allocating processes when the operator caps FDs aggressively | Low | deferred → Story 9.x (operator-policy resource-floor) | Documented inline (argv.rs:35-44) as DR-5.5a-9 placeholder. Full `ResolvedCaps.pids_max` split lands with the operator-policy resource-floor expansion |
| **Backfill 2026-05-25** — Smoke arm passes `[--, echo, hello-from-t3]` as command suffix; the in-container entrypoint is `/maos/spirit` which (for the busybox-as-Spirit smoke case) is busybox itself. Busybox does not treat `--` as "end of options" the way POSIX shells do — it tries to invoke an applet named `--` and may fail | Medium | deferred → Epic 6 (subprocess Spirit ABI) | Smoke arm is observability, not gating (graceful-degrade on this system). Real Spirit binary at Epic 6 owns its argv parsing and will honor `--`. Reference: argv.rs:87 |
| **Backfill 2026-05-25** — Stale test name `sandbox_config_t3_parseable_but_rejected_at_admission` at `crates/maos-kernel-core/src/security/manifest.rs:1750` — AC1 spec called for REPLACING this test with `sandbox_config_t3_parseable_and_admitted_at_admission`. Body is still correct (only asserts parse, not admission), but the misleading name implies an obsolete v0.3-β invariant | Low | open | Test still passes and is semantically benign; rename in the next sandbox-test edit pass |
| **Backfill 2026-05-25** — `runtime_detect` "auto" mode probes Podman → falls back to Docker → returns the **Docker** error on dual-miss; operator sees the wrong runtime name in the error message, masking the fact that Podman was tried first | Low | open | Aggregate the two probe errors into a single `T3Error::RuntimeUnavailable` variant carrying both reasons, or extend the variant to list the attempted runtimes. Reference: `runtime_detect.rs:46-51` |
| **Backfill 2026-05-25** — Duplicate commit `248f23b` carries the message `5-5a-sandbox-tier-t3-container-isolation-via-docker-podman` but its diff is exclusively Story 5.5b content (multi-provider routing: `inference/router.rs`, `maos-providers` Ollama/OpenAI/fixture-replay, multi-provider-v0 fixtures, provider-switched journal). The real Story 5.5a commit is `2916b84` (Sat May 23 08:56:27); the mislabeled commit is `248f23b` (Sat May 23 12:03:39). Cosmetic — code is correct in both, only the commit message is wrong | Low | open | Note in Story 5.5b's backfill review; consider an `git filter-repo` rewrite if the retro decides commit-message hygiene matters more than rewriting history |
| **Backfill 2026-05-25** — `_manifest: &SandboxConfig` parameter at `security/mod.rs:128` has an underscore prefix (Rust convention "intentionally unused") yet the body at line 194 reads `_manifest.image_pin`. Stylistically misleading; downstream readers may assume the parameter is unused and remove the new image-pin admission logic during refactoring | Low | open | Rename to `manifest: &SandboxConfig` in the next admission-path edit pass |
| **Backfill 2026-05-25 — TOCTOU on image SHA** verified mitigated: `argv.rs:54-58` pins `image_uri@sha256:<hex>` so the container runtime itself enforces the SHA at pull/use time. No additional mitigation required at v0.5-α | Critical (verified) | **closed** | Validation only; no code change. Reference: argv.rs:54-58 |
| **Backfill 2026-05-25 — RAII Drop on T3 container** verified correct: `SandboxedContainerChild::Drop` kills runtime parent, waits, then best-effort `stop --time=2` + `rm -f` the container. Drop swallows errors per RAII contract (no propagation). On panic, Drop still fires per Rust unwinding contract | Critical (verified) | **closed** | Validation only; no code change. Reference: child.rs:49-72 |
| **Backfill 2026-05-25 — `LifecycleEvent::SandboxApplied = 17` vs `FrameKind::SpiritRevoked = 17`** discriminant overlap verified harmless: distinct enums in distinct modules (`maos-domain::invariants::i10::LifecycleEvent` vs `maos-kernel-core::iac::transparency_log::FrameKind`); no cross-wire serialization shares the byte | Low (verified) | **closed** | Validation only; no code change |
| **Backfill 2026-05-25 — `wall_clock_now_ns()` vs `monotonic_now_ns()`** verified: all 5.5a-introduced journal/TL emit sites use `monotonic_now_ns()` (security/mod.rs:232, 241, quarantine.rs:35). No `wall_clock_now_ns` regression introduced. Pre-existing seconds-as-u64 site at `applier.rs:341` is from Story 5.4 (commit 3d751b4) per §1380 carryover, NOT introduced by 5.5a | Low (verified) | **closed** | Validation only |
| **Backfill 2026-05-25 — Vendor SDK additions** verified: no new container-runtime crates added (no `bollard`, no `shiplift`, no `podman-api`). Implementation uses `std::process::Command` shelling out to system Podman/Docker per DR-5.5a-3. Confirms zero-new-dep posture | Low (verified) | **closed** | Validation only |
| **Backfill 2026-05-25 — cap-audit first-production-caller wiring** verified: `emit_t3_escape_block` at `cap_audit_bridge.rs:14-28` uses `sender.try_send(event).is_err()` → `cap_audit::record_drop()` per ADR-030 (never `.await`). Wired into `quarantine.rs:43-52`. This IS the first production caller of `emit_sandbox_block` per inspection report finding §10. Probe-side fallback `emit_t3_escape_block_probe` is `eprintln`-only at v0.5-α | Critical (verified) | **closed** | Validation only |
| **Backfill 2026-05-25 — Revocation quarantine fallback** verified correct: applier.rs:251-305 splits the `quarantine_spirit` result into three arms — `Ok(_) → 0 receipts`, `Err(QuarantineRequiresSubprocessForm) → drain-then-terminate fallback` (with `quarantine_requested` TL marker emitted unconditionally at 316-333), `Err(other) → RevocationError::QuarantineFailed`. The fallback path DOES fire on the documented v0.5-α deferred-quarantine return. JoinHandle pruning via `drains.remove(&pid)` on task completion (mirrors Story 5.4 §1368 pattern) | High (verified) | **closed** | Validation only |
| **Backfill 2026-05-25 — Smoke arm graceful-degrade** verified: on this Linux system with no Podman/Docker installed, `MAOS_ONE_SHOT=smoke-t3-sandbox-5 cargo run -p maos-bin --features fixture_replay` prints exactly one informative JSON line `{"step":1,"surface":"runtime_detect","outcome":"unavailable","reason":"..."}` and exits 0. Not a silent skip — operator can observe the unavailability | High (verified) | **closed** | Validation only |
