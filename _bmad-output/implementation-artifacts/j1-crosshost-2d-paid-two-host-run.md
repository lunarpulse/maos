...
kloc_grant: "**NONE, and taking one would be a violation.** `kloc_check` counts `--types Rust` only (`xtask/src/kloc_check.rs:167-190`); markdown and `_bmad-output/` are not counted. ⚠ `kloc-check` is **BLOCKING and RED at HEAD** on four keys, executed 2026-08-19: `aggregate 151124 >= 147057` (D17), `maos-kernel-core 18933 > 18248` (D13), `maos-domain 8695 > 8644` (D14), and **`xtask 39966 > 39960` — undisclosed at authoring, ROUTED at the round-table to `2e` (AC5.1)**. ⚠ **CORRECTED 2026-08-25 by the §A6 Evidence-Infra Auditor: this four-key list is now a THREE-key list.** `2e` did not merely take the `xtask` grant — it **retired** the breach (39966 → 39927, back under the untouched 39960 ceiling), so live measurement at HEAD shows only `aggregate`, `maos-domain` and `maos-kernel-core` over. The substantive claim is unchanged and still verified: `kloc-check` is Blocking and RED, so **AC6.2 holds and `all gates green` is not available**. Filed rather than overwritten — and noted as the FOURTH instance of this story's own recurring shape (*a residual that was TRUE and EXPIRED*), the three others having been corrected earlier the same day. Correcting expired claims is itself a place expired claims accumulate. This story must NOT absorb the `+6`: D17's ruling is that a grant requires *a measured delta to justify it*, and this story's Rust delta is zero. See AC6.2 — **`all gates green` is not available as a done criterion.**"
...
receipt are landed; `paid_run_capture_present` has flipped to `true` and the `two-host-signed-run`
demo beat has moved **ABSENT → INDETERMINATE**, retiring the widowed control this row was created
to prevent.

> ⚠ **CORRECTION 2026-08-25 — the gating clause above expired.** This file said AC8 was gated on
> `j1-crosshost-2e` (`backlog`, no story file). **`j1-crosshost-2e` is `done`** — dev pass 2026-08-22,
> §A6 closed 2026-08-24 (`zai/glm-5.3`), landed in `c549bbc0`. All six code blockers (F1-F5, F7) are
> closed. **AC8 is no longer blocked by mechanism; it is blocked by operator substrate** — two
> processes, a clean sandbox home, a funded metered API key, and a decision to spend. Every
> "⛔ 2e" marker in the findings below is therefore *history*, not a live blocker. Kept rather than
> deleted because this lane files corrections instead of silently overwriting them; this is the shape
> the lane already named — *a residual that was TRUE and EXPIRED*.

> **What a dev can do without spending: AC1-AC7, all of it, code-free.** AC8 — the paid run itself —
> needs the substrate above. `runbook-j1-t8-two-host-paid-run.md` is the procedure; `PUBLISHED-FINGERPRINTS.md`
> pre-commits the keys the run must use.
>
...
### F6 — the judge cannot tell a real run from a forgery ~~and a committed test proves it~~ **(citation corrected 2026-08-25)**

> ⚠ The forgery is REAL and was re-proven live by the §A6 Blind Hunter on 2026-08-25 (single-key halves AND garbage-signature halves, both `passed:true findings:[]`).
> **The claim corrected is the evidence claim, not the defect.** No committed test asserts that the two halves must carry different keys.
> `j1_crosshost_2c_proven_red.rs:386-414` asserts that the *published template is admissible*; its single-root material comes from the shared
> `good_bundle()` helper (`:270-295`) and is incidental. Fixing the gate would RED that test as *'the template broke'*. A test that exhibits a
> weakness while asserting something else is not an enrolled control — the very shape this finding exists to name.
...
- [x] **T8 (AC8, AC4.2-4.6)** — the paid run and its stranger-checkable evidence. EXECUTED 2026-08-25 (`claude-haiku-4-5-20251001`, $0.0168 total against a $3 cap). `2e` unblocked it.
...
**Do not treat a green gate as evidence** (F6). Read `paid_run_capture_present` and
`two_host_signed_run_claimed` — both `false`, both published as true facts (R1).

> ⚠ **CORRECTION 2026-08-25.** This paragraph originally named a **third** field,
> `capture_signature_verified`, and told the reviewer to read it. **`j1-crosshost-2e` DELETED that
> field** (`check_j1_two_host_signed_run.rs:1291-1292`; `verify_capture_signature` deleted at `:1233`)
> because it reported on a value that was unreachable by construction. Verified against the live gate
> on 2026-08-25: the JSON emits `paid_run_capture_present`, `two_host_signed_run_claimed` and
> `claim_scope`, and no capture-signature field. A reviewer following the old text would have looked
> for a field that no longer exists and had to guess what its absence meant.

#### What remains — T8 / AC8. ~~It is IMPOSSIBLE, not merely inadvisable.~~ **SUPERSEDED 2026-08-25**

> ⚠ **This subsection is a dated measurement, not the current state.** Everything below was true on
> **2026-08-22** and stopped being true on **2026-08-24**, when `j1-crosshost-2e` closed F1 and shipped
> `maosctl cohort sign`. Re-measured 2026-08-25 at `c549bbc0`:
>
> ```
> $ ./target/release/maosctl cohort sign --help
> Sign a cohort manifest with an authority key so a cohort daemon will boot.
>       --authority-key <AUTHORITY_KEY>   Path to the 32-byte Ed25519 authority seed.
>                                         REQUIRED, with no environment fallback and no default.
> ```
>
> **AC8 is no longer blocked by mechanism.** The remaining blockers are operator substrate, and the
> money-after-failure modes (F5, F7) are closed — `verify.py` now returns `OK — signature verified`
> (exit 0) on the real T6 bundle, re-executed 2026-08-25. The record below is retained because it is
> what the attempt actually measured, and because the finding it produced (do not build an
> out-of-process signer) is what shaped 2e's design.
...
and **nothing in the shipped product can supply those 64 bytes** *[as of 2026-08-22 only — `2e` shipped
`maosctl cohort sign`; see the SUPERSEDED banner above]*. `CohortManifest::signed_with` has
...
| Date | Change |
|---|---|
| 2026-08-25 | **§A6 REVIEW — 4 layers + runtime, reviewer model `sonnet` (≠ dev `anthropic/claude-opus-5`; a CONVENTION, per AC7.5 — no machine-readable reviewer field exists anywhere in this repo). 12 findings: 3 HIGH, 6 MED, 3 LOW. Nine patched here code-free; three routed to `deferred-work.md` because closing them needs Rust and this row may not write it.** *** **HIGH-1, AND IT HAS NO CODE-FREE REPAIR — DISCLOSED, NOT CLOSED.** The Acceptance Auditor re-derived the zero-Rust invariant *from the landed git archive* instead of from the working tree and it fails there: the ONLY commit between baseline `dd4cf959` and HEAD is **`c549bbc0`, titled `j1-crosshost-2d-paid-two-host-run`**, and it carries 35 files including `crates/maos-bin/src/main.rs` (+203) and `crates/maos-cli/src/subcommands.rs` (+230). `git log --all` contains **no commit naming `j1-crosshost-2e` at all** — that story's entire Rust delta lives inside a commit bearing THIS story's key. The dev-pass method (`git status --porcelain` on the live tree) is true and was true, but it cannot survive the standard THIS STORY'S OWN AC7.1 sets: *re-derive it from the landed artifacts alone*. A stranger auditing *'did 2d write Rust'* from the commit that bears its name concludes **yes, extensively**; the correction is prose with no commit boundary behind it. ⚠ **RESOLVED THE SAME DAY BY SPLITTING THE COMMIT — and the first disposition rested on a FALSE PREMISE I stated without checking.** I recommended disclosure-over-rewrite on the ground that *'`c549bbc0` is pushed work'*. **It was not pushed**: `main` was 18 commits ahead of `origin/main` and the offending commit was inside that window. The operator was asked to choose between disclosure and a rewrite whose cost I had overstated; when the premise was corrected they chose the rewrite. `c549bbc0` is now split into **`30a3794d`** (`j1-crosshost-2e-two-host-run-enablement`, all 29 code and 2e-owned files) and **`3c638fad`** (this story: 6 files, **zero** `.rs`/`.toml`/`.lock`/workflow). The split is proven lossless: the tree after re-applying every commit is **byte-identical** to the pre-split backup (`backup-pre-split-2026-08-25`), verified by `git rev-parse HEAD^{tree}` equality, and the intermediate tree matched `c549bbc0` exactly before AC8 was re-applied. **MEASURED EFFECT:** `j1-crosshost-2e: could not locate commit via git log --grep` is GONE — the gate now resolves `30a3794d` for that story and, for the first time, actually compares its File List against a diff. That comparison surfaced two items (296 → 298 total): (a) `File List entry 'omitted.**'` — a **checker parser artifact**, scraped from prose at `j1-crosshost-2e-…md:767`, the same class as the 73 brace/glob shorthand entries AC6.3 already recorded; and (b) `sprint-status.yaml not present in git diff` — the **irreducible residue of a file-level split**: both stories edited that one file inside the one commit, so no file-level boundary can satisfy both File Lists. Assigning it to 2e would simply move the violation onto this row. Disclosed rather than shuffled. **This is a better state than the one it replaced**: a gate that can compare and finds two things beats a gate that reports it cannot check at all. Note the flag is still off in CI (`discipline.yml:1745`), the hole AC6.3 filed — none of this is enforced today. The one thing the rewrite could NOT fix is that a file-level split cannot separate two stories' edits inside a shared file; that limit is stated here rather than papered over. *** **HIGH-2 (PATCHED):** `traceability-matrix.md` still read *'⚠ ABSENT until the operator run'* and *'⛔ GATED ON MONEY AND HARDWARE'* hours after the run. The document whose stated purpose (AC5.2) is that *a matrix listing only what passed is a marketing document* had gone stale in the opposite direction — **understating** a run that had happened. Both rows rewritten with the measured evidence. *** **HIGH-3 (ALREADY FILED AS F6, BUT ITS CITATION WAS OVERSTATED — PATCHED).** The Blind Hunter, working blind and never reading this file, re-derived F6 independently and forged TWO gate-admissible artifact sets in a sandbox mirror: one key in both halves, and non-hex garbage signatures — both `passed:true`, `findings:[]`, exit 0. The gate weakness itself is known, disclosed and priced (RELEASE-HOLDS, and the gate's own output says the signatures are operator-verified). **What was wrong is F6's evidence claim.** F6 said `j1_crosshost_2c_proven_red.rs:386-414` *'commits the proof'*. It does not: that test asserts *the published template is admissible so an operator is not refused after billing*, and the single-root material is incidental scaffolding from the shared `good_bundle()` helper. **No test anywhere asserts the two halves must carry different keys** — and if the gate were fixed, that test would RED and read as *'the template broke'*. **A test that exhibits a weakness while asserting something else is not an enrolled control**, which is precisely the failure shape F6 was written to expose. Corrected in the traceability matrix; the test-suite blind spot is filed. *** **MED, ALL PATCHED — every one is the same class: two evidentiary tiers presented as one.** `transport:"cross-host-tcp-mtls"` and `host_b_delegation_served` are `println!` output absent from both bundles, quoted beside a `frame_id` that IS signed; the effect claim (`DELEGATED-PROOF.md`) lives outside git while its own sibling clause (the `not_completed` verdict) is a signed structured row, so one sentence spanned two tiers; `redaction_result:"verified"` was called *backed by an executed scan* when the scan left no artifact; the `$0.002113` ping was stated as precisely as the `$0.014644` that a signature covers; and `j1-two-host-evidence/README.md` — the NORMATIVE field contract — still declared its own directory *empty of real artifacts* while holding three. *** **LOW (PATCHED):** the kloc breach list said four keys; live measurement says three, because `2e` RETIRED the `xtask` breach rather than taking a grant for it. *** **WHAT THE REVIEW DID NOT FIND, WHICH IS THE LOAD-BEARING RESULT: the Edge Case Hunter proved all 7 named edges by execution and found ZERO defects in the landed artifact.** The `sk-` landmine does not reach this gate; `claim_scope` is byte-exact and independently forbidden elsewhere; the booleans reject `"true"`, `1` and absence; `preceded_by_not` binds per-occurrence and per-field. Its two real bugs (`detect_credential` substring matching; `two_host_shape:""` skipping all four two-host checks) live in `record-capture`'s sibling validator, and NEITHER was exercised in producing this evidence. The Evidence-Infra Auditor re-executed all ten commands on its own machine and reproduced every machine-checkable claim exactly — including the **zero-Rust delta**, which is true of the uncommitted work and false only of the commit boundary above. **And the one control the gate structurally cannot provide, the operator did provide**: the two halves really are signed by two independent roots, proven because `reconcile-hosts` REFUSED `SharedAttesterRoot` when the same root was deliberately used for both. |
| 2026-08-25 | **💰 AC8 — THE PAID TWO-HOST RUN WAS PERFORMED. `in-progress` → `review`; all eight ACs complete.** Operator: Lunarpulse; cap $3; **actual spend $0.016757** (key-validation ping $0.002113 + the run $0.014644). Model `claude-haiku-4-5-20251001`, operator-chosen for cost and **provable from the artifact** — the run's own `modelUsage` is inside the `cli.subprocess.output` row that both bundles sign, so the `ANTHROPIC_MODEL` environment choice is covered by the signature rather than being an invisible influence. `spirits/worker/manifest-claude.toml` was used **unmodified** for exactly that reason: adding `--model` there would have changed `argv_prefix_hash` and made the signed artifact cite a manifest not in git. `--live` NOT passed (AC8.2) — confirmed by zero `inference.call` rows. **THE CROSSING:** host A published `boot_nonce 5642945490710666976`, held the bounded barrier, dialled on release; **the `frame_id 0100000000000000e09eb3b406c54f4e` is journaled on BOTH sides and is re-derivable from the signed bundles** (it appears in each half's `task.assign` row and in both `run.capture` `audit_refs`); host A `rc=0`; worker spawned as real child `362808`. ⚠ **EVIDENTIARY TIER, corrected 2026-08-25 by the §A6 Acceptance Auditor:** the strings `transport:"cross-host-tcp-mtls"` (`crates/maos-bin/src/main.rs:3511`) and `host_b_delegation_served` (`crates/maos-bin/src/delegation.rs:744`) are **`println!` console output and appear in NEITHER bundle** (`grep -c` = 0 in both). They are real code paths and the substance is separately corroborated by the bundles' own `worker.completion-verdict` row, but as quoted they were re-derivable only from a terminal transcript that was never captured into any artifact. This is the same defect already filed from the `2a` review — *the oracle verdict itself is println-only, never journaled* — reaching the record one layer up: **a story quoting console output alongside signed rows as if they were one tier.** **EVIDENCE:** `record-capture` ran BEFORE `sealed-export` on both hosts, so the CaptureDoc is journaled as `run.capture` and sits INSIDE both signatures (verified: both bundles contain `run.capture` and the artifact name). `sealed-export --host` under each host's OWN root produced 15 and 11 entries whose pubkeys are byte-identical to `PUBLISHED-FINGERPRINTS.md`, committed at `dd4cf959` before the run existed. The stranger's path — `verify.py` against the COMMITTED fingerprints, not the artifacts' self-claims — returned `OK — signature verified` on both halves. `reconcile-hosts` OK under the third (operator) key, 1 shared frame_id, ratified claim scope verbatim. `redaction_result:"verified"` — ⚠ **tier corrected 2026-08-25 by the §A6 Acceptance Auditor.** A scan WAS executed (the metered key was grepped for in both `transparency.sqlite` files and both host logs and found in none), but **its output is not preserved in any artifact**, so the field is epistemically identical to `stranger_verification`, which this directory's own README correctly labels *a sworn statement, not a verified result*. The earlier wording here — *'backed by an executed scan'* — claimed a stronger status than the artifact set supports. Recorded as an operator attestation, and the underlying gap (`record-capture` accepts caller-asserted `fs_jail`/`redaction_result` with no run evidence) is already filed from the `2a` review. **GATE:** `check-j1-two-host-signed-run` 0 findings with **`paid_run_capture_present: true`** (was `false`) and `two_host_signed_run_claimed: false` — the latter is CORRECT and published as a true fact under R1: the gate cannot verify the two signatures, the operator did. `demo-j1`'s `two-host-signed-run` beat moved **ABSENT → INDETERMINATE** (*'capture present and validated; the run is NOT claimed'*), which retires the widowed-control failure shape — an ABSENT beat owned by a `done` story — that this row exists to prevent. ⚠ **THE RUN'S REAL FINDING, AND IT IS THE INVERSE OF THE ONE THIS STORY FILED.** The worker WROTE its artifact (`DELEGATED-PROOF.md`, 1111 bytes, sha256 `f5dadd16ef56220ef39a2e47f6fa772c7899633e8af7755aad3de3bae6cd516c`, at `~/.maos/j1-t8-lab/paid/work-b/`) and the adapter still scored `not_completed:permission_denied`, because the model made ONE incidental `Bash` call — `find` over the operator home, i.e. the directory holding the audit signing keys — which the `acceptEdits` posture correctly denied, and `claude_result_object_oracle` treats any non-empty `permission_denials` as non-completion. The result object's own fields dissent: `subtype:"success"`, `is_error:false`, `stop_reason:"end_turn"`. `2a` filed this adapter's asymmetry as a FALSE POSITIVE (`completed=true` over an unwritten file, reproduced free at HEAD earlier the same day); the paid run proves the SAME missing effect oracle yields a FALSE NEGATIVE too. **Both directions are one absent control**, and the fix is not 'ignore `permission_denials`' — that field is the only thing separating a refusal from a success on this adapter. Filed to `deferred-work.md`, lane *worker-adapter completion semantics*, TO SCHEDULE John. **The operator ratified signing THIS run rather than re-running with a goal tuned to avoid the denial**, on the ground that engineering the task until the verdict is pretty is what this lane calls canning a control. The capture records the effect and the verdict as two separate facts and reconciles neither. **A/B CONTROL, run after the artifact was already signed and deliberately EXCLUDED from it** ($0.009105, `~/.maos/j1-t8-lab/investigate`): everything held constant — same binaries, signed manifest, certs, model, topology — with ONE change, a goal forbidding shell use. Result `permission_denials: []`, `completed:true`, 2 turns not 4, file written. **The variable is not the mechanism, the transport, the model or the task; it is only whether the model happened to attempt something unrelated that the posture denied.** A verdict that flips on that is not measuring completion. The control run is evidence in `deferred-work.md`; it is NOT in `j1-two-host-evidence/`, because the signed set holds the run that actually happened first. AC6.2 still honoured: **`all gates green` is NOT claimed** — `kloc-check` remains Blocking and RED at HEAD on inherited keys. **Total spend across all three live calls: $0.025862 of a $3 cap** — of which only the signed run's `$0.014644` is re-derivable from an artifact (`total_cost_usd` inside the signed `cli.subprocess.output` row). The key-validation ping (`$0.002113`) and the A/B control (`$0.009105`) were measured from console output and are **operator-attested only**; flagged 2026-08-25 by the §A6 Acceptance Auditor for stating an unbacked figure with the same precision as a signed one. |
| 2026-08-25 | **T8 DRESS REHEARSAL — runbook Chapters 2-4 EXECUTED END-TO-END WITH A FIXTURE WORKER, ZERO SPEND. Every step of the paid run except the `claude` spawn is now proven at `c549bbc0`.** Substrate at `~/.maos/j1-t8-lab/` (moved off `/tmp` so it survives a reboot). **Ch2:** mTLS leaves minted with an explicit `leaf.ext` — `CA:FALSE` confirmed `critical` in the emitted cert, the extension whose absence makes rustls reject with `CaUsedAsEndEntity`; leaves distinct (`e20dc72c…` / `25cc969d…`). Cohort authority seed minted SEPARATE from every audit root; **`maosctl cohort sign` produced a real 128-hex/64-byte signature**, discharging F1 — the `EInvalidSignature("expected 64 bytes, got 0")` that made host B unbootable is gone. Signer's own refusals PROVEN, not assumed: signing a manifest whose `authority.keys` omits the signer is **refused, exit 2** (*'signature and claimed signer disagree'* — without this it would be a forgery tool), and omitting `--authority-key` with `MAOS_AUDIT_KEY` set does **not** fall back to the audit root. Daemon configs cross-checked against the three namespaces the runbook calls the most common error: `local_host` resolves to a cohort-manifest `host_id`, `peer_id` to the topology's `host` value, and each side's `cert_fingerprint` is the OTHER side's leaf. **Ch3 — THE CROSSING HAPPENED.** Host A published `boot_nonce 4702206881397115580` under its own `cohort:crosshost-started` intent and HELD on the bounded barrier; host B pinned it and listened on `127.0.0.1:19555`; `touch $READY` released the dial. Host A journaled `transport:"cross-host-tcp-mtls", frame_id:"0100000000000000bc8a12ca3e984141"` and host B journaled `host_b_delegation_served` with **the identical frame_id** and `worker_outcome:"completed"`, its worker spawned as a real child process. Host A exited `rc=0`. F4's pairing — the procedure that was IMPOSSIBLE before 2e — executed on the first attempt. **Ch4 — the evidence pipeline closes.** `sealed-export --host` on each side under its OWN root emitted bundles whose pubkeys are **byte-identical to the fingerprints committed in `PUBLISHED-FINGERPRINTS.md` before any of this existed** (14 entries under `4bbc1187…`, 8 under `843dc5a8…`); `verify.py` returned `OK — signature verified` on BOTH halves **against the committed values, not the artifacts' self-claims**; `reconcile-hosts` returned OK with 1 shared frame_id and printed the ratified claim scope verbatim. **Both refusals that the two-host claim rests on were PROVEN to fire, exit 1:** re-sealing host B under host A's key ⇒ *'both halves were attested by the SAME root — one seed holder cannot attest two identities'*; sealing without `--host` ⇒ *'bundle carries no host claim, so it cannot be half of a two-host run'*. ⚠ **AND THE REHEARSAL SHOWS WHAT A FAILED APPENDIX-B9 CHECK LOOKS LIKE:** the worker reported `completed=true` while `work-b/` stayed **empty**. Benign for the fixture, which writes nothing by design — but it is the exact silhouette of the `claude` failure mode reproduced free earlier the same day. The operator now has a measured baseline for both outcomes, so on the paid run the B-9 check is a comparison, not a judgement call. |
| 2026-08-25 | **CORRECTION — three expired claims filed, not silently overwritten. Zero Rust, zero AC movement; status stays `in-progress`.** `j1-crosshost-2e` reached `done` (dev 2026-08-22, §A6 closed 2026-08-24 `zai/glm-5.3`, landed `c549bbc0`), which invalidated three statements this file made: (1) the header called AC8 *gated on `j1-crosshost-2e` (`backlog`, no story file)* — 2e is `done` and AC8 is now blocked by **operator substrate**, not mechanism; (2) the AC7 reviewer instruction named **`capture_signature_verified`**, a field 2e DELETED (`check_j1_two_host_signed_run.rs:1291-1292`, `verify_capture_signature` deleted `:1233`) — re-measured live 2026-08-25, the gate JSON emits only `paid_run_capture_present`, `two_host_signed_run_claimed`, `claim_scope`, so a reviewer following the old text would hunt a field that no longer exists; (3) *What remains — T8/AC8. It is IMPOSSIBLE* is SUPERSEDED — `maosctl cohort sign` exists and `verify.py` returns `OK — signature verified` exit 0 on the real T6 bundle. All three retained with dated banners per this lane's convention (*a residual that was TRUE and EXPIRED*). **T8 substrate prepared the same day (free, no spend):** release binaries rebuilt and PROVEN real at `c549bbc0` — release double-boot returned distinct nonces `4190754155658946787` / `7933111923296394421` under `MAOS_TEST_BOOT_NONCE=424242`, and the **debug control returned `424242` twice**, so AC1's falsifier is proven to discriminate rather than to pass for free; both release runs now `rc=0` after building `worker-cli-fixture` into `target/release/` (its absence, not any 2e defect, was the earlier nonzero exit); 2e's F4/F7 surfaces verified PRESENT in the shipped binary (`cohort:crosshost-started`, `MAOS_DELEGATED_GOAL`, `MAOS_CROSSHOST_PAIRING_READY_FILE`, `MAOS_CROSSHOST_PAIRING_TIMEOUT_SECS`); all three PUBLISHED-FINGERPRINTS private keys verified to derive to their committed public values (`4bbc1187…`, `843dc5a8…`, `433b27c1…`) so the pre-run commitment is intact; operator-local `~/.maos/host-grants.toml` gained the `claude`/`Anthropic` T3 grant (backed up to `.bak-c549bbc0`) matching `manifest-claude.toml` exactly. **Runbook Chapter 1 (free rehearsal) then EXECUTED IN FULL at `c549bbc0`, each check with BOTH halves proven:** 1.3 `verify.py` on the real T6 bundle = `OK — signature verified` rc 0, and against a deliberately wrong pubkey = `FAIL` rc 1, so the mandatory-abort check discriminates. 1.4 run three ways against a fake `claude` on a prepended PATH (no network, no spend), driving host B's REAL live path: (A) clean sandbox home -> proceeds, and `host_grant_disposition` confirms the new grant live (`attested_image:"claude"`, `signing_key_id:"Anthropic"`, `SandboxTier(3)`, `permitted_egress:["api.anthropic.com"]`); (B) a planted `$HOME/.claude/.credentials.json` -> **REFUSED before spawn** with the ambient-auth error (worker never ran); (C) `MAOS_LIVE_AGENT` unset -> **refused to spawn a real agent CLI on the hermetic path**. ⚠ **Appendix B-9 REPRODUCED LIVE at HEAD, free:** the fake `claude` wrote NO file and returned only a clean result object saying *"I have written the file"* — the adapter journaled `worker_completion completed=true` and the run exited `delegation_completed`. The asymmetry 2d filed against `claude` is not theoretical and is not fixed: **on the paid run, `completed=true` must be checked by hand against the worktree before anything is signed.** Also measured: the ambient guard reads `HOME` (not `MAOS_HOME`), is `live_agent`-gated, and its own doc admits it is a FILENAME control that does **not** cover the inherited environment channel — which is the channel the metered `ANTHROPIC_API_KEY` travels on. |
