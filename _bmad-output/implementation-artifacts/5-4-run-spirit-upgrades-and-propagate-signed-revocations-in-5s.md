...
| `smoke-upgrade-revoke-5` still asserts post-revocation capability denial with a fixed print, not a token verification. | 2026-08-12 current-HEAD audit | **Critical** | open | Patch required — `main.rs:6870-6871` prints `denied_after_revocation`; `revocation_verify_denial.rs:1-12` compares error strings only. Exercise issue → CRL apply → `verify_token`/`verify_and_audit` and assert the revoked-token error. |
| All 50 revocation/upgrade corpus descriptors still reference CRL, anchor, and manifest assets that do not exist. | 2026-08-12 current-HEAD audit | **High** | open | Patch required — the corpus drivers validate descriptor metadata but never open or execute `crl_blob_path`, `trust_anchor_pub_path`, or predecessor/successor manifests. Commit every referenced artifact and compare real execution outcomes to descriptors. |
| CRL verification still reconstructs signed bytes with non-canonical `serde_json::to_vec(&crl.entries)`. | 2026-08-12 current-HEAD audit | **High** | open | Decision needed — choose one producer-independent contract—RFC 8785 JSON, deterministic CBOR, or an explicitly preserved signed payload—and migrate every signer/verifier together. |
...
| A classless SCB can still match a malformed CRL entry whose deserialized `spirit_class` is empty. | 2026-08-12 current-HEAD audit | Medium | open | Patch required — `applier.rs:103-118` maps missing class to `""`; `parser.rs:54-64` does not re-run the constructor's class validation. Reject classless SCBs and malformed wire classes before matching. |
| A Spirit loaded after `apply_crl` snapshots the SCB map can escape a CRL that is then permanently marked applied. | 2026-08-12 current-HEAD audit | **High** | open | Decision needed — the applier retains only CRL IDs and `scheduler.load` has no revocation check. Choose persistent validated revocation state checked at admission, or synchronize application and loading with a complete rescan. |
| Future `#[non_exhaustive] RevocationAction` variants still fall through to a warning and zero receipts. | 2026-08-12 current-HEAD audit | Low | open | Patch required — replace `applier.rs:304-313` fallback with structured fail-closed behavior or termination; require every new action to have an explicit branch. |
| Both drain paths still double `progress_threshold_ms` in `u32` before converting to `u64`. | 2026-08-12 current-HEAD audit | Low | open | Patch required — widen before multiplying with `u64::from(...).saturating_mul(2)`, or validate an upper bound in the manifest. |
| `LocalFileRegistryClient` still reads its trust anchor from process-global environment state. | 2026-08-12 current-HEAD audit | Medium | open | Decision needed — the client stores only `crl_dir` and cannot consume corpus-local `trust_anchor_pub_path`. Select constructor injection, a provider abstraction, or registry-scoped key resolution. |
| The composition root still abandons the revocation poller's cancellation token and `JoinHandle`. | 2026-08-12 current-HEAD audit | Low | open | Patch required — `main.rs:3257-3258` creates both and never uses them again. Retain them in service lifecycle state, cancel on shutdown, and await completion. |
| HotSwap and Migrator still receive the predecessor's `spirit_obj` as the alleged successor. | 2026-08-12 current-HEAD audit | **Critical** | open | Decision needed — `upgrade.rs:86-123,180-186` constructs no successor instance. Define a manifest-to-Spirit factory/loading boundary and require a newly constructed successor for every upgrade policy. |
| `revocation_applier_pipeline.rs` remains a type-level stub and never invokes `RevocationApplier`. | 2026-08-12 current-HEAD audit | **Critical** | open | Patch required — replace its enum/serde checks with a real applier fixture covering signature parsing, matching, token revocation, action/receipt, journal/frame emission, version matching, and idempotency. |
| Five named upgrade-policy/revocation integration tests remain enum, serde, report, or error-display tests. | 2026-08-12 current-HEAD audit | **High** | open | Patch required — instantiate the orchestrator/applier and assert policy/action state transitions, migrated state, receipts, journal/frame output, and post-revocation capability denial. |
| Revocation cadence still inherits `MAOS_SUPERVISION_FAST` when `MAOS_REVOCATION_FAST` is absent. | 2026-08-12 current-HEAD audit | Low | open | Decision needed — decide whether this coupling is intentional; either document/test `revocation > supervision > default` precedence or isolate revocation from the supervision override. |
| Case-sensitive class matching still consumes wire classes that the parser has not revalidated as lowercase. | 2026-08-12 current-HEAD audit | Low | open | Patch required — prefer rejecting non-lowercase deserialized classes; otherwise define and apply canonical normalization before matching. |
...
| `parse_signed_crl` still bypasses structural invariants from `SignedRevocationList::new` and `RevocationEntry::new`. | 2026-08-12 current-HEAD audit | Low | open | Patch required — centralize validation and invoke it after deserialization; zero key/signature and class format currently differ between constructors and the wire parser. |
