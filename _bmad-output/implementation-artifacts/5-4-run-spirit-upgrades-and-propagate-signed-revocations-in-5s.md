...
| `cargo test --workspace --all-features` times out at 300s | Low | open (pre-existing) | Known issue from Stories 5.2/5.3; individual crate tests pass |
...
| `parse_rejects_signature_invalid` test is non-functional (mock limitation) | Low | deferred | Documented; real crypto provider needed for functional test |
...
|---|---|---|---|---|
| `revocation_propagation_p99.rs` bench is a **complete placeholder** — does NOT spawn 10⁴ concurrent `verify()` calls, does NOT measure propagation latency from `apply_crl` return to first `Err(CapError::Revoked)`, does NOT emit the JSON p99 report that `assert-revocation-p99-floor` consumes, AND does NOT mechanically enforce the NFR-Rel-9 ≤5s p99 contract. The benchmark body is one synchronous `ring.revoke_all(7)` call in a Criterion loop. The `tokens` and `SandboxTier` imports were dead. AC6 "structurally closed by bench + CI gate" is **NOT MET** — the CI gate at `.github/workflows/discipline.yml::nfr-rel-9-revocation-5s-p99` provides only "does it run without crashing" assurance, not the spec-claimed mechanical p99 floor. | Acceptance Auditor | **Critical** | deferred (documented in-bench) | Added LIMITATION doc-comment to bench; fixed unused-variable warnings; tracked for follow-up rewrite. The bench-emitted JSON the `assert-revocation-p99-floor` binary reads is never produced by THIS bench — the binary would fail to find the report under real CI conditions. |
| `smoke-upgrade-revoke-5` Step 1 (`hot-swap`) was a bare `println!("…hot-swap…completed")` — never invoked `upgrade_orchestrator.upgrade(.., HotSwap)`. Identical pattern to Story 5.5d Finding #7 ("stage-show prints"). AC7 "evaluator OBSERVES the three upgrade policies executing" was structurally false: only 2/3 policies (cold-swap + revocation propagation) were actually exercised; hot-swap was simulated by a fixed string emit. | Acceptance Auditor | **Critical** | fixed inline | Step 1 now invokes `upgrade_orchestrator.upgrade(.., UpgradePolicy::HotSwap)` and emits the real outcome string. If HotSwap fails due to minimal-composition-root limitations, the failure is reported (not hidden). |
| `smoke-upgrade-revoke-5` Step 4 (capability denial) is a bare `println!("…denied_after_revocation")` — never invokes `capability.verify_token()`. AC6 "subsequent token uses against the revoked Spirit fail with `ECapabilityRevoked` (AC6's bench verifies the round-trip)" is structurally false in the smoke arm: the denial is asserted by a string emit, not by exercising the verify path. The revocation_verify_denial.rs test is also a stub (asserts only that error variants are distinct strings). | Acceptance Auditor | **Critical** | open | Smoke arm Step 4 should issue a token for the revoked Spirit's PID, call `capability.verify_token(token, posture, T2)`, and assert `Err(CapError::Revoked)`. Deferred — the smoke arm's composition root does not currently retain the issued token handle past the revocation step. |
| The 30 revocation corpus fixtures + 20 upgrade-policy fixtures reference `crl_blob_path` / `predecessor_manifest_path` / `successor_manifest_path` files (`*_crl_001.signed.json`, `*_anchor_001.pub`, `*_pred_001.toml`, `*_succ_001.toml`) that **do not exist on disk** — only the scenario-descriptor JSONs are committed. The eval test driver at `crates/maos-eval/tests/revocation_corpus.rs` only asserts the descriptors load + category distribution (5 per category × 6 categories = 30) — it never opens the referenced blob/anchor/manifest files. So the corpus structurally counts as "30 unique scenarios" by category but the underlying CRL bytes / trust anchors / manifests are not present, and the "signature-rejection scenarios accept (false positive) OR signature-accept scenarios reject (false negative)" guarantee the discipline.yml `revocation-corpus` job claims to enforce is **vacuous**. | Acceptance Auditor | **High** | open | Corpus generators (`xtask gen-revocation-corpus` / `xtask gen-upgrade-policy-corpus`) need to ship + generate the referenced blobs. Until then the 30/20 scenario counts are descriptor-only. Deferred to a follow-up "Story 5.4-bis corpus blob generation" task. |
| `parse_signed_crl` verifies the Ed25519 signature over `serde_json::to_vec(&crl.entries)` — JSON serialization is **non-canonical across producers**. Within Rust+serde the field order is stable (struct-derive order), but a Python or JS CRL signer that emits alphabetical-key JSON would produce a different byte stream than the verifier reconstructs, breaking signature verification even for legitimately-signed CRLs. The trust-anchor pin (line 39-42) catches some of this but only when `signer_pub_key` mismatches; an attacker who controls the signing identity could exploit ambiguity in canonicalization. The spec says "canonical-serialized entries blob" but the implementation does plain `serde_json::to_vec` with no canonicalization step. | Blind Hunter | **High** | open | Either (a) document that v0.3-β CRLs MUST be Rust+serde_json produced (operator-issued only, single producer); (b) switch to a canonical-bytes scheme (CBOR with deterministic encoding per RFC 8949 §4.2, or a custom sorted-key JSON serializer); (c) sign the entire envelope bytes received from registry rather than re-serializing entries. Option (c) is the safest air-gapped story. Deferred. |
...
| `parse_rejects_signature_invalid` test (parser.rs:144-155) is **non-functional** — the test acknowledges its own non-functionality in a comment: "MockCrypto always returns Ok, so the signature check passes even on mutated bytes. … For the mock, we skip the assert and document the limitation." The test runs and passes with no assertions. This means **there is no test in the codebase exercising the failure path of `CryptoProvider::verify_signature` returning `Err`** for CRL parsing. The dev's existing finding "documented; real crypto provider needed for functional test" is acknowledged, but this leaves a real coverage gap on the signature-rejection contract. | Blind Hunter | High | open (already noted by dev, re-flagging severity) | The fix is straightforward: wire `RingCryptoProvider` into the parser test (it already exists in `maos-kernel-core::security::crypto`), sign a real CRL with a known keypair, then mutate the bytes and assert rejection. Estimated 30 LOC. Deferred to follow-up. |
| `RevocationApplier::apply_crl` matches by `class_name != entry.spirit_class` (exact string equality, applier.rs:118) — spec says "spirit_class: [a-z0-9-]+; must match SCB.manifest.class.name" but the spec example uses `"hello-spirit"`. **Edge case**: if SCB's `manifest.class.name` happens to be empty (`.unwrap_or_default()` at line 109) AND a CRL entry has empty `spirit_class` (rejected by `RevocationEntry::new` validation, so unreachable from validated entries), the match would succeed. The applier reaches the `.unwrap_or_default()` path when an SCB has no class section. This is a defense-in-depth gap: SCBs without a class should never match any CRL entry, but the current code path WOULD match if a malformed CRL bypasses `RevocationEntry::new` (e.g. via raw struct construction in tests). | Edge Case Hunter | Medium | open | Add explicit guard: `if class_name.is_empty() { continue; }` before the entry loop. Deferred. |
| `RevocationApplier::apply_crl` iterates `spirits_snapshot` (`.values().cloned()` at applier.rs:100), then iterates `crl.entries` per spirit. The `break` after first matching entry (applier.rs:361) prevents same-spirit duplicate revocation — good. But: if a **new Spirit is spawned BETWEEN the snapshot and the per-spirit revocation work** (race against `scheduler.load(...)` while `apply_crl` is in flight), the new Spirit is missed by this CRL application. The applied_crls set then marks the CRL as applied — so the new Spirit will never be checked against this CRL UNLESS the poller fetches it again with a fresh ID. For CRL-import via `maosctl revocations import` (FR60), this is a real correctness gap: import-then-spawn-malicious-spirit succeeds. | Edge Case Hunter | High | open | Need a "re-scan on Spirit spawn" hook: on `scheduler.load`, iterate `applied_crls` and check each entry against the new SCB. Or: store CRL entries (not just ids) in `applied_crls` so `scheduler.load` can do post-hoc enforcement. Deferred to follow-up. |
| `RevocationAction::Quarantine` arm calls `crate::security::sandbox::t3::quarantine::quarantine_spirit(...)` which (per Story 5.5a) returns `Err(T3Error::QuarantineRequiresSubprocessForm)` for in-process spirits. The arm correctly falls back to `DrainThenTerminate`. However, **the wildcard arm** `_ => { eprintln!(...); 0 }` (applier.rs:307-313) on the `#[non_exhaustive]` enum is unreachable today (all 3 variants are handled) but **silently produces 0 receipts + skips termination** if a future variant is added without updating this match. The dev's finding "fixed: added eprintln warning for unknown actions" addresses the silence but the unreachable arm masks the additive-enum risk. | Edge Case Hunter | Low | open | Replace `_ =>` with an explicit panic or a structured error return for the unreachable arm (e.g. `RevocationAction::Quarantine` is the last documented variant; an extension story adding a variant should update this match — better to fail loudly via `unreachable!()` documented as "add new variant here when extending RevocationAction"). Deferred. |
| `DrainThenTerminate` arm's drain deadline is read from `manifest.supervision.progress_threshold_ms * 2` (default 60s). **Edge case**: `progress_threshold_ms` is a u32; multiplying by 2 could overflow if `progress_threshold_ms > u32::MAX/2` (~2.1B ms = ~24 days). The cast `as u64` happens AFTER multiplication, so overflow happens at u32 first. Unlikely in practice (operator config) but no validation in `SupervisionSection::from_toml_str`. | Edge Case Hunter | Low | open | Use `progress_threshold_ms.checked_mul(2).unwrap_or(60_000) as u64`. Deferred. |
| `LocalFileRegistryClient::trust_anchor_pub` reads `MAOS_CRL_TRUST_ANCHOR_PUB_HEX` (revocation.rs:374) — this is process-global env-var state. **Edge case**: a test that sets the env-var then concurrently runs `cargo test --test-threads=N` could observe a stale value. More importantly: `trust_anchor_pub_path` field on the 30 corpus scenarios suggests they expect per-scenario trust anchors, but `LocalFileRegistryClient` only reads the env-var. So the corpus loader cannot validate per-scenario trust-anchor-mismatch rejection without a separate trust-anchor injection path. | Edge Case Hunter | Medium | open | Allow `LocalFileRegistryClient::new` to accept an explicit `trust_anchor_pub: Vec<u8>` override, or add a `TrustAnchorProvider` trait. Deferred. |
| `RevocationPoller::spawn` returns a `JoinHandle<()>` — caller is expected to retain it. The composition root in `main.rs` spawns the poller but does it retain the handle for shutdown drain? Per the existing Story 5.4 finding "active_drains map grows unboundedly — JoinHandles never removed (fixed)", but the poller-level handle is separate. | Edge Case Hunter | Low | open | Verify in composition root that the poller JoinHandle is retained and awaited during shutdown. Deferred to integration review. |
| `UpgradeOrchestrator::upgrade` HotSwap and Migrator arms call `self.hot_swap.initiate_swap(spirit_id, &successor_manifest, Arc::clone(&successor_spirit_obj))` — but `successor_spirit_obj` is the **PREDECESSOR's** spirit_obj (cloned from `scb.spirit_obj` at upgrade.rs:101). The `initiate_swap` expects the NEW spirit instance. This means hot-swap and migrator are actually swapping the predecessor with… itself. **Critical**: the spec's "load the successor Spirit instance from the manifest" step is missing — there's no `Spirit::load_from_manifest(...)` call. The same predecessor object is reused with the new manifest, breaking the FR49 contract for any state-sensitive Spirit. | Blind Hunter | **Critical** | open | The successor spirit_obj must be constructed from the successor manifest (via the registry's Spirit-load path or a test-double for v0.3-β). Currently this is a fundamental fake — hot-swap "succeeds" only because the predecessor is reused as the successor. Documented as a v0.3-β limitation alongside the cold-swap admission bypass (existing Finding line 1370). |
| `revocation_applier_pipeline.rs` integration test (2 tests, ~28 LOC) only asserts `RevocationAction::default()` and `RevocationOrigin` serde roundtrip — **does NOT exercise the propagation pipeline at all**. The file's own comment acknowledges this: "v0.3-β stub — full signature-accept/reject/idempotency/version-range coverage lands with Task 15 completion." But Task 15 IS marked complete in the sprint-status. The discipline.yml gate `revocation-corpus` runs `cargo test -p maos-kernel-core --test revocation_applier_pipeline --release` which therefore passes vacuously. | Acceptance Auditor | **Critical** | open | The propagation pipeline integration test must (a) build a `RevocationApplier` with all 8 Arc handles, (b) load 1+ SCBs, (c) apply a valid CRL, (d) assert SpiritRevoked frame emitted + halt-receipt produced + token verify fails. Estimated 100-150 LOC. The CI gate is currently vacuous. |
| `upgrade_orchestrator_three_policies.rs`, `upgrade_cold_swap_with_inflight_tasks.rs`, `upgrade_migrator_cross_major.rs`, `on_revocation_three_actions.rs`, `revocation_verify_denial.rs` are all **enum/error-display tests only** — they verify the type-level surface (`UpgradePolicy::as_str()`, error Display impls, serde roundtrips) but DO NOT instantiate any orchestrator/applier, DO NOT exercise any actual upgrade or revocation flow. The story's spec for AC3 ("end-to-end cold-swap with predecessor halt-receipt") and AC4 ("end-to-end migrator with state migration") are reduced to "the policy variant exists in the enum" assertions. | Acceptance Auditor | **High** | open | The "v0.3-β stub" rationale (file comments) cites composition-root complexity, but `smoke-upgrade-revoke-5` proves the composition root is buildable for these flows. Tests should be promoted to actual integration coverage. Deferred. |
| `MAOS_REVOCATION_FAST=1` collapses cadence to 100ms (poller.rs:74-81) — `pick_poll_cadence()` reads `watchdog_common::pick_poll_cadence()` then overrides. **Edge case**: `watchdog_common::pick_poll_cadence` may itself be overridden by `MAOS_SUPERVISION_FAST=1`. If only `MAOS_SUPERVISION_FAST=1` is set (no `MAOS_REVOCATION_FAST`), the revocation poller will use the supervision-fast cadence (likely also 100ms), conflating two independent test channels. The spec at story line 55 says "same `pick_poll_cadence` discipline reused" but the override semantics are not documented. | Edge Case Hunter | Low | open | Document the cadence-resolution order: revocation_fast > supervision_fast > default. Or have `RevocationPoller::pick_poll_cadence` ignore the supervision env-var entirely. Deferred. |
| `applier.rs::apply_crl` matches via `class_name != entry.spirit_class` (exact string equality) — does NOT handle case-insensitivity. If a CRL entry says `"Hello-Spirit"` but the manifest class is `"hello-spirit"`, no match. `RevocationEntry::new` validates `[a-z0-9-]+` so this is mostly defended-in-depth, but a CRL constructed via raw struct (bypassing `::new`) could carry mixed-case `spirit_class` and silently no-op. | Edge Case Hunter | Low | open | Add `.to_ascii_lowercase()` normalization on both sides before comparison, OR strengthen the parser's per-entry validation. Deferred. |
| `LifecycleEntry.timestamp` (u64) is documented as no unit; existing journaling sites store seconds (`now_ns / 1_000_000_000`). Story 5.4's `Upgrade` and `Revoked` arms follow this convention correctly. **However**, `monotonic_now_ns()` is monotonic-clock-relative and produces values offset from boot; dividing by 1e9 gives "seconds since monotonic base" (the init at `init_monotonic_base()` time), NOT wall-clock seconds. The journal column will diverge from wall-clock by the boot offset. Consumers reading this field expecting Unix epoch seconds will misinterpret. | Blind Hunter | Medium | open | Either (a) document the column as "monotonic seconds since boot"; (b) switch to `wall_clock_now_ns() / 1e9` for journal timestamps (acceptable for journal-only since we don't compare it to monotonic latencies); (c) widen the schema. The existing dev finding "Journal timestamp uses SystemTime::now() instead of monotonic_now_ns()" went the OTHER direction — switched TO monotonic — which actually broke wall-clock semantics for journal-archival use. Pre-existing class issue (see Finding line 1380). |
| `UpgradeOrchestrator::upgrade` calls `self.scheduler.unload(predecessor_pid)` in the ColdSwap arm BEFORE constructing the successor SCB — if `unload` succeeds but the successor SCB construction fails (e.g. manifest parse retry, allocate_pid race), the spirit is unloaded with no successor. There is NO rollback path. This violates the FR49 implicit transactional contract ("policy: cold-swap with re-init"). Cold-swap should be unload-then-load atomically; if load fails, predecessor should be re-loaded (or the operator notified). | Blind Hunter | High | open | Add try-load-first / sequence-revert pattern, or document that ColdSwap failures leave the substrate in a Spirit-absent state. The existing finding at line 1370 ("ColdSwap bypasses scheduler.load()") notes the admission gap but not the rollback gap. |
| `parse_signed_crl` skips structural validation that `SignedRevocationList::new` would enforce. The dev's existing finding "fixed: Added empty-entries check after deser in parser.rs" only covers ONE invariant. Other `::new` invariants — `schema_version == 1`, zero-signature rejection, zero-pubkey rejection — are NOT re-checked after deserialization. Specifically: `signature == [0u8; 64]` is checked in `::new` but a CRL deserialized from JSON could carry all-zero signature bytes, which would then fail at `verify_signature` (good) but ALSO bypass the structural `SignatureInvalid` early-return. Not a security bug (`verify_signature` catches it) but a defense-in-depth gap. | Blind Hunter | Low | open | Call `SignedRevocationList::new(...)` on the deserialized values (which would also re-validate schema_version, etc.) instead of returning the deserialized struct directly. Deferred. |
| The `MAOS_ONE_SHOT=revocations-import` arm requires `MAOS_CRL_TRUST_ANCHOR_PUB_HEX` env-var (main.rs:1753) — but the CLI dispatch from `maosctl revocations import` (subcommands.rs `dispatch_revocations`) is NOT verified to set this env-var. If the operator runs `maosctl revocations import foo.json` without first exporting the trust anchor, the dispatched maos-bin process errors with a non-obvious message. UX gap; per FR60 the import should fail-safe but with operator-actionable guidance. | Edge Case Hunter | Low | open | Have the CLI subcommand check for `MAOS_CRL_TRUST_ANCHOR_PUB_HEX` before dispatching and emit a clear "trust anchor not configured; export MAOS_CRL_TRUST_ANCHOR_PUB_HEX or place a trust-anchor file at <documented path>" error. Deferred. |
...
| `cargo test -p maos-kernel-core --lib --release` **fails to compile** with `error[E0432]: unresolved import maos_mcp::fixture_replay` at `crates/maos-kernel-core/src/mcp/mod.rs:137`. The reference is `#[cfg(any(test, feature = "fixture_replay"))]`-gated in `maos-mcp/src/lib.rs:17` but the kernel-core import does NOT gate. **Pre-existing** — this failure was introduced by Story 5.5d (verified by checkout-3d751b4 which passes). Not a Story 5.4 regression but blocks the verification command listed in the review brief. | Test Infra | (pre-existing) | open | Story 5.5d retro item — the `use maos_mcp::fixture_replay::FixtureReplayMcpServer` site at kernel-core needs the same feature gate. Out-of-scope for this 5.4 backfill review. |
| `cargo run -p xtask --bin xtask -- check-pub-field-constructors` **fails with 3 violations** on `ProvidersSection` / `McpSection` types. Verified that at Story 5.4's commit `3d751b4` this gate PASSED — the violations are Story 5.5c/5.5d additions. Not a Story 5.4 regression. | Test Infra | (pre-existing) | open | Story 5.5c/5.5d follow-up. |
| `_ => { eprintln!(...); 0 }` arm on `RevocationAction` match in applier.rs:307-313 has a stray trailing comma after `0` (line 313: `},`). Cosmetic — rustfmt would normalize. | Style | trivial | open | Run `cargo fmt`. |
