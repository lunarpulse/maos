...
> **Restored 2026-09-12 by the §A6 code review of 15-4:** the working tree had this blockquote and the two bullets below deleted, with no disposition recorded anywhere — removal without a disposition is not a disposition. Both bullets are restored verbatim; the Epic-11 bullet is **closed in place** below rather than left open, because its successor story was dropped with the deferral recorded in the epic.
...
- **Epic 11 must carry REAL ja/zh-Hans + a language-identity gate.** AC5 i18n is honestly DESCOPED to v2.0/Epic 11 (Korean placeholders + DO-NOT-SHIP markers; coverage+glossary gates report-only because they tautologically pass on wrong-language content). Epic-11 Story 11.6 must add real translations **and** a language-identity gate that detects wrong-language content (Hangul-in-ja, etc.) — coverage/glossary-lock provably cannot. **Ownerless and open.** *Dispositioned by Story 13.6 / AC5, 2026-08-08 (mechanical stale-owner sweep).* Epic 11 and its retrospective are both `done`; the language-identity gate was never claimed by a successor. **CLOSED 2026-09-12 — obsolete as filed:** Story 11.6 is recorded **DROPPED 2026-06-29** at `epic-11-v20-technical-phase.md:50` — ja/zh-Hans *deferred indefinitely*, the Korean-placeholder scaffold removed, en+ko the supported locales per `LOCALES.md`'s Deferral Policy — so no successor can ever claim it. The entry stays on the ledger, honestly closed, rather than deleted silently. *Dispositioned by the §A6 code review of 15-4, 2026-09-12 (restored verbatim, then closed, after 15-4 removed it without a disposition).*
...
- **FKCS frozen-tag leg uses CWD-relative kernel-baseline paths** [xtask/src/check_fkcs.rs:94-106] — `validate_live_triple` → `check_kernel_baseline::check()` opens `xtask/kernel-core-baseline.toml` and `crates/maos-kernel-core/src` relative to the CWD, so running the gate from a workspace subdirectory (e.g. `crates/maos-bin`) fails with `No such file or directory`. 12.6's new `workspace_root()` helper hardened only the `git()` invocation, not this path resolution. Not triggered in CI (jobs run from the workspace root). Pre-existing. **CLOSED 2026-09-13 by `16-0-kernel-pin-content-hash`** (rule 10(a), at origin — the path-injectable seam AC2 required is the same seam this row asked for): `check_kernel_baseline::check_at(src, baseline)` takes both inputs as paths and `check()` delegates through workspace-root resolution, so the gate resolves from any subdirectory. Proven by `xtask/tests/kernel_pin_content_hash_16_0.rs`, which calls `check_at` in-process with `cargo test -p xtask`'s CWD = `xtask/` — the exact condition that used to fail.
## Deferred from: code review of 13-5d-production-spirit-collective-route (2026-07-19)

- **Fallible `record_invocation` (audit-channel drop returns `Ok`)** [crates/maos-kernel-core/src/capability/mod.rs:332-351] — `CapabilityRegistryAdapter::record_invocation` treats a full-channel `try_send` failure as a dropped event and still returns `Ok(())`; a saturated audit channel silently loses the route-level `CapabilityInvocation` that AC5's correlation join reconciles against. The only real fix is a fallible invocation path, which is kernel-core work outside the L10 FLAG-Winston grant (fence item 2: do not spend the grant beyond the pid check). *Reason deferred:* requires a second FLAG-Winston escalation; **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D3, RULED by `14-0` AC4.1: repaired OUT of the kernel and CLASS-WIDE. `record_drop()` lives in `maos-capability`, so the observable-drop repair costs ZERO kernel-core lines and zero re-pin, and covers all EIGHT drop sites rather than this one — the eighth being `cap_tokens/mod.rs:280-282`, the same defect UNCOUNTED on the operator revoke path. No second FLAG-Winston escalation is required, because the ruling is ZERO kernel-Δ. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.* *Dispositioned by Story 13.6 / AC5, 2026-08-08 (mechanical stale-owner sweep).* “the next kernel-touching story” has been overtaken FOUR times (13.5h, 13.5i, 13.5j, 13.6c all shipped kernel deltas without taking it), so the phrase now names nobody; the retro decides who carries kernel-core debt. Party-mode consensus 2026-07-19. The story's interim mitigations: audit-first ordering at the composition root + a drop-counter assertion on the `mediated-operation-correlation` gate leg + a may/may-not-table honesty line.
  **CLOSED 2026-09-17 by `16-5`:** `record_invocation` now returns `AuditSinkUnavailable`; the three production callers preserve that cause instead of reporting policy denial.
...
- **Legal-hold check-then-act race with deletion** [crates/maos-kernel-core/src/memory/mod.rs:470-498,571-573] — `forget_with_reason` consults `is_under_legal_hold` and releases the authority lock before any mutation; the private/index deletes happen ~100 lines later with no spanning transaction or recheck. A `place_legal_hold` that lands in that window is ignored and the principal erases despite an active hold. Each hold operation takes its own SQLite lock (`crates/maos-iac/src/adapter/transparency_log.rs:1155-1187`). *Reason deferred:* the check/mutate ordering predates this diff; 13.5b changed the orchestrator's handling of `Suspended`, not the kernel's hold-then-delete sequence. Closing it needs a hold-scoped transaction or an optimistic recheck in kernel-core — kernel-core lines, outside the ratified ZERO fence. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D5.1, decomposed by `14-0` AC4.2. Kernel-touching, and routed through D3's ZERO-Δ ruling rather than a second escalation: same crate, same fence, same moment. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.* *Dispositioned by Story 13.6 / AC5, 2026-08-08 (mechanical stale-owner sweep).* The `13-5h` candidacy lapsed: that story is `done` and did not take the hold-scoped transaction.
  **CLOSED 2026-09-17 by `16-5`:** `forget_with_reason` now serializes the legal-hold check and all per-principal mutation under one guard; the race and mixed-cascade vectors pin the boundary.
- **`revoke_all_for_pid(...).unwrap_or(0)` feeds a signed `CategoryStatus::Removed` count** [crates/maos-bin/src/main.rs:8168-8169,8217-8221] — a failed revocation is indistinguishable from "zero tokens existed", and the difference is then signed into the proof bundle as `capability_tokens: Removed { count }`. Exactly the D-1/D-4 genre this story exists to close, on a line the diff did not touch. *Reason deferred:* pre-existing; the honest fix is a `CoverageGap` on revocation failure, which reopens the AC1 vocabulary decision now pending on the region-pinned terminal. **Owner: `20-3a-gate-honesty`** — D5.2, decomposed by Story `14-0` AC4.2 under one control: *no signed erasure attestation may assert a completion it did not observe.* ⚠ The *Reason deferred* above is WRONG at HEAD and is corrected here: `CategoryStatus::CoverageGap` **already exists** (`crates/maos-audit/src/erasure/proof.rs:22-27`), so the honest fix reopens no AC1 vocabulary decision. This row had no owner cue at all and was invisible to `check-dev-record-completeness` until `14-0` AC2.3.
- **`decommission_region_key` hardcodes `completed: true`** [crates/maos-audit/src/erasure/regional_teardown.rs:116-124] — it derives a public key from the still-supplied `base_seed` and asserts completion; no key material is revoked or destroyed. The signed regional receipt therefore attests a decommission that never happened. *Reason deferred:* pre-existing (Story 9.4b); currently unreachable in practice because phase (a) fails first on the `shared` coverage gap. Becomes live the moment the pending AC1 decision restores a `completed`-capable forget attestation — re-open then. **Owner: `20-3a-gate-honesty`** — D5.3, folded with D5.2 under the same control by Story `14-0` AC4.2. Verified INTACT at HEAD. This row had no owner cue at all and was invisible to the owner sweep until `14-0` AC2.3; it is also the item the `14-0` preflight brief could not find, and it does exist — here.
- **Crash between durable mutation and audit append leaves no reconciliation record** [crates/maos-kernel-core/src/memory/mod.rs:571-589; crates/maos-loom-lite/src/store.rs:1772-1823 → crates/maos-bin/src/main.rs:4862-4877] — both the forget cascade and the operator collective erase commit destructive state before their audit frame, and `insert_kernel_event_returning_id` panics on write failure by design (I2 binding). No startup repair or persisted reconciliation job exists; the one-sided-erase plant in `xtask/tests/story_10_4a_ac1_proven_red.rs:438-447` is a pure boolean helper that never observes production stores. *Reason deferred:* 13.5b took an explicit position (Trap 4 fail-fast, ADR-059:58-62) and registered mutation-to-audit crash atomicity as open with no vehicle. Recorded here so it does not disappear. **Owner: `v25-erasure-crash-reconciliation`** — D5.4, DEFERRED WITH A NAMED KEY by Story `14-0` AC4.2 rather than to a phase label. It is the largest of D5's four (≥4 crates, a persisted intent record and a startup reader) and shares its design with **ADR-059 residual #4** — scope them together or neither. It had TWO open homes (this row and ADR-059 #7); both now point at the one key so they cannot drift.
- **Skipped `AdvisorySubstrate` legs still emit `passed: true`** [xtask/src/check_reza_production_path.rs:474-545] — when the two-datname Postgres environment is absent the live legs are not attempted, yet `passed` is `blockers.is_empty()` and the gate exits zero with a WOULD-HAVE-BLOCKED warning. The story's own testing standard says absent substrate must be UNMEASURED, never green. *Reason deferred:* this is the shared `gate_common` house pattern across every substrate-bound gate, not a 13.5b regression; CI provisions pgvector/pg16 for this job so the legs do run and do block there. Fixing it is a cross-gate change. **CLOSED 2026-08-07 by `c45df0be` (Story 13.6e, judge machinery / evidence ledger).** Recorded by Story `14-0` AC4.6. ⚠ The citation above has ROTTED: `check_reza_production_path.rs:474-545` now holds `TestLeg` definitions, not the advisory-substrate branch. The stale-owner sweep ran 2026-08-08, walked this exact file, annotated the rows on either side and SKIPPED this one; the stale row then propagated verbatim into retro C3, `sprint-status.yaml` and decision **D2** — *which was therefore authored describing a code site that had not existed for four days.* The surviving `passed` / `product_claim` split is RATIFIED as the explicit claim boundary: `evidence_ledger.rs:1538` emits `"passed": blockers.is_empty()` while the same run emits `product_claim: NOT_PROVEN`, per-leg `ABSENT` and a WOULD-HAVE-BLOCKED banner. The real surviving residual is **D20** (E12-B1 at six of eight), filed against its own ID.
- **`MAOS_ONE_SHOT` legal-hold-release and collective-erase carry no operator authorization boundary** [crates/maos-bin/src/main.rs:4799-4828,4831-4877] — both accept a principal id / pid+namespace+key from environment variables and act, with no operator identity, capability, or approval check; `team_guard` is tenant placement, not authority. *Reason deferred:* AC4(a) explicitly mandated reusing the `MAOS_ONE_SHOT` dispatch idiom, and every other one-shot verb has the same shape — exec rights on the host binary already imply operator authority. Revisit if operator-axis work (NFR-Ops-11) ever lands. **DOOR HALF CLOSED 2026-09-14 by Story `16-1-daemon-post-surface-and-verb-retarget`**: when a daemon is live, `legal-hold-release` now travels over the bearer-authenticated operator door (authenticated before route — no bearer, no action), and its offline one-shot arm runs only when the child acquires the 16-1 store lock set exclusively. The collective-erase half of this row stays open.
...
- **Eight Family-B gates are outside the evidence ledger** [`xtask/src/check_scale_churn.rs`, `check_cohort_mesh.rs`, `check_enterprise_pdp.rs`, `check_enterprise_identity.rs`, `check_escape_detector.rs`, `check_trial_attestation.rs`, `check_vetting_attestation.rs`, `check_wasm_form_equiv.rs`] — each carries its own `{label, passed, failed, ran, [attempted,] green}` leg struct with no `binding`, no `substrate_present` and no evidence state, so none of them emits a `product_claim` and none of them publishes a ledger artifact. *Reason deferred:* 13.6e's ledger set is **derived** from the four `CONTRACTS` entries precisely so it cannot quietly grow; widening it is scope, not budget. **Owner: `21-4-one-instrument-and-env-registry`** — D1, RESTATED by `14-0` AC4.5. The ledger does NOT expand: `check_rto_gate.rs:77-84`'s *"No evidence — SKIPPED (not a silent PASS)"* pattern is ratified as the separate out-of-ledger evidence contract. Note the row's own second clause is close to inverted — `check_vetting_attestation.rs:220-231` and `check_wasm_form_equiv.rs:244-256` HARD-FAIL on any unmeasured leg and are the strictest gates in the repo; the real subject set is the seven gates that report `passed: true` over absent evidence, none of which this row names. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.* (alongside the `maos-a2a-core` third-consecutive-unratified grant and the `xtask` decomposition question) — the retro should decide whether the ledger is a four-gate journey instrument or a workspace-wide one before anyone extends it.
- **Two of those eight cannot express `ABSENT` at all** [`xtask/src/check_vetting_attestation.rs:31`, `xtask/src/check_wasm_form_equiv.rs:104`] — their `LegResult` has **no `attempted` field**, so "the leg never ran" and "the leg ran and produced nothing" are the same value. A projection cannot distinguish `ABSENT` from `INDETERMINATE` there without a struct change. Both gates are hermetic today, which is why the hole has not bitten; it becomes real the moment either grows a substrate-bound leg. **Owner: `21-4-one-instrument-and-env-registry`** — D1, RESTATED by `14-0` AC4.5. The ledger does NOT expand: `check_rto_gate.rs:77-84`'s *"No evidence — SKIPPED (not a silent PASS)"* pattern is ratified as the separate out-of-ledger evidence contract. Note the row's own second clause is close to inverted — `check_vetting_attestation.rs:220-231` and `check_wasm_form_equiv.rs:244-256` HARD-FAIL on any unmeasured leg and are the strictest gates in the repo; the real subject set is the seven gates that report `passed: true` over absent evidence, none of which this row names. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*, same decision.
- **`check-kernel-baseline` prints its PASSED line to stdout** [`xtask/src/check_kernel_baseline.rs`] — every gate that reuses it as a leg (`check-cross-region-consensus`, `check-multi-region-slo`) therefore emits one non-JSON line before its `--json` payload. Pre-existing and harmless in CI (both jobs `tee` rather than parse), and 13.6e's ledger artifact is written to a file so it is unaffected. *Reason deferred:* fixing it changes a shared gate's output contract. **Ownerless and open.** *Dispositioned by Story 13.6 / AC5, 2026-08-08 (mechanical stale-owner sweep).* `xtask gate-infrastructure maintainers` is not a role that exists in sprint-status. **Close when `check-kernel-baseline --json` is machine-clean and both composite callers consume a quiet structured result without transcript parsing.** **CLOSED 2026-09-13 by `16-0-kernel-pin-content-hash`** (rule 10(b) — the story already touching this file was asked first). ⚠ The obvious fix was NOT the fix: this gate's own `--json` was already clean, because `check_kernel_baseline::run` prints the human PASSED line only in the non-json `else if` branch. The pollution came from **six** legs calling `run(false)` — which prints that line to STDOUT on the green path — from inside their own `--json` payload: `check_cross_region_consensus.rs:151`, `check_multi_region_slo.rs:525`, `check_enterprise_identity.rs:329`, `check_enterprise_pdp.rs:320`, `check_escape_detector.rs:450`, `check_scale_churn.rs:411`. All six now call `check()` (`check_fkcs.rs:95-97` shape) and all six were measured emitting parseable JSON. Separately, `check_cohort_mesh.rs:696` returned `Err` BEFORE its JSON println, so a kernel red produced zero bytes of stdout — it now emits its own payload first and forwards the per-file detail. Falsifier kept as a test: `a_composite_gates_json_stdout_is_machine_clean`.
...
- **A second, independent cause collapse on a SHIPPED operator surface.**
  ...
  **`WrongDirection`**, `ConsentStateStale`, `ConsentStateUnavailable`,
  ...
  (`crates/maos-bin/src/main.rs:2357-2362`), preserving the cause only inside a
  ...
  one is in the CLI, not the kernel. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D7, unchanged target, re-homed off the epic key by `14-0` AC2.2. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
  **CLOSED 2026-09-17 by `16-5`:** all six refusal variants now publish distinct machine-readable outcomes in both the recall audit row and traceback response.
...
- Nothing in production compares the environment's `MAOS_REGION_HOME` with the
  ...
  **Owner: `21-4-one-instrument-and-env-registry`** — D4a, SPLIT out by `14-0` AC4.4. Neither named vehicle could host it: 14-7 and 14-8 are static-scan stories and cannot host a runtime boot check, so D4's target AND its stated fallback were both wrong. Registration stays D4b/`21-4-one-instrument-and-env-registry`; classification+crypto is D4c/`21-4-one-instrument-and-env-registry`. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
...
- **No gate reconciles the kernel pin with its own HISTORY.** `grep -rn "HISTORY" xtask/src/` → **0**. `xtask/kernel-core-baseline.toml:438` reads *"23596 → 23517"* while its own prose two lines down says *"+116 physical lines from the 23401 pin"* (23401 + 116 = 23517). The `from` value could be arbitrary and nothing would red. Fixing it needs a NEW gate, which Story 13.6 is forbidden to add, so the gap is recorded rather than closed.
  **Owner: `21-4-one-instrument-and-env-registry`** — handed by the Epic-13 retrospective §4 (2026-08-11): 14.6 owns the ceiling instrument and retro-residual discipline.
- **In-`src` kernel test modules are budget-charged but CI-unexecuted.** Re-measured: **41** files under `crates/maos-kernel-core/src` declare a `mod tests`, every one of them counted by `kloc-check` (only `spill_test_faults` is excluded, `xtask/src/kloc_check.rs:189`), and all 58 `--test` invocations in `.github/workflows/discipline.yml` name an integration target, so **zero** CI invocations run the kernel lib target. The counting rule matters: "files declaring `mod tests`" = 41, which is why the earlier "42 in 42" and "44 in 43" numbers disagreed — they counted different things.
  **Owner: `21-4-one-instrument-and-env-registry`** — handed by the Epic-13 retrospective §4 (2026-08-11): 14.6 owns the ceiling instrument and retro-residual discipline.
- **CLOSED by Story `14-0` (2026-08-26).** Only
  `crates/maos-bench/benches/audit_query_latency.rs:235` emitted the invalid
  `"capability.invoke"` frame kind; line 24 is free-text intent. The kind is now
  `"capability.invocation"`, the bench asserts a non-empty result, and
  `.github/workflows/discipline.yml` executes the benchmark in CI.
...
- **BOUNDED by `j1-crosshost-2c` (AC3.5), not fixed.** Ctrl-C/graceful-shutdown can wait forever on a never-exiting remote worker: `spawn_blocking` enters `run_cli_wrapper_manifest` and the shutdown await has no deadline or abort path. 2c bounded every SENDER-side wire operation (`TcpStream::connect` and `framed.send`, both previously unbounded) and wired the operator-configured `partition_timeout_secs` to the TCP path, so no A2A call can hang the loop. The remaining unbounded wait is the LOCAL `spawn_blocking` join, which is a lifecycle/shutdown concern rather than a fault-injection one: closing it means giving `run_cli_wrapper_manifest` an abort path inside `maos-kernel-core`, which this story is pinned to leave byte-identical (24472). ROUTED 2026-08-18 round-table (was `Ownerless and open`). LANE: Epic-14 kernel lifecycle, FLAG-Winston, because the fix is an abort path inside `run_cli_wrapper_manifest` = a kernel-core delta against the 24472 pin. TO SCHEDULE: Winston. Routing a lane is NOT the same as having a story; it must acquire one before Epic-14 closes — this is kernel lifecycle-lane debt under FLAG-Winston, bounded as RELEASE-HOLDS claim boundary row 11. Evidence: crates/maos-bin/src/delegation.rs:601-633; crates/maos-bin/src/main.rs:9561-9571.
- **BOUNDED by `j1-crosshost-2c` (AC3.5) and recorded as a RELEASE-HOLDS claim boundary (row 10).** Crash window between journal(`Written`) and worker spawn makes a delegated task durably look processed (every replay returns `Duplicate`, no execution record). 2c fixed the SAME durability lie on the digest-reply path (`:819` below) by making the durable hand-off the commit guard, but the delegation path's window is a CRASH window, not an ordering window: the journal write and the spawn are separate operations with no shared transaction, so closing it needs a reconciliation pass over `Written`-without-execution rows at boot. That is a recovery mechanism, not a fault-semantics repair. ROUTED 2026-08-18 round-table (was `Ownerless and open`). LANE: Epic-14 recovery/reconciliation - closing it needs a boot-time pass over `Written`-without-execution rows, which is a mechanism, not a fault-semantics repair. TO SCHEDULE: John. Routing a lane is NOT the same as having a story; it must acquire one before Epic-14 closes, and it is bounded as RELEASE-HOLDS claim boundary row 10. Evidence: crates/maos-bin/src/delegation.rs:442-450.
- **RESOLVED by `j1-crosshost-2c` (AC3.5). Fixed, not bounded.** Digest-reply path ACKed a retry as `Duplicate` after a dropped-receiver NACK, because `observe_reply` recorded the dedup before `push_to_intake_sink` ran. The invariant now implemented is **nothing is `Duplicate` until something is durable**: `DigestReadPort::observe_reply_guarded` runs the caller's commit guard after every authorization check and BEFORE any dedup state is published, and the router passes the intake `try_send` in as that guard. A failed hand-off publishes no dedup record and no receipt audit row, so the reply stays RETRYABLE. Proven by `crates/maos-cohort/src/state.rs` (`a_failed_commit_leaves_the_digest_reply_retryable_never_duplicate`) and `crates/maos-a2a-core/tests/digest_reply_durability_2c.rs` (dropped receiver, full channel, and the drain-then-retry path); machine-checked by the `duplicate-after-durable` leg of `check-j1-two-host-signed-run`. Evidence: crates/maos-a2a-core/src/cohort.rs `observe_reply_guarded`; crates/maos-a2a-core/src/router.rs digest-reply arm; crates/maos-cohort/src/state.rs.
- `parse_host_grants_toml` defaults an omitted `permitted_tier` to T3 (highest), contradicting its own "missing required field is an error" doc — pre-existing moved code (worker_spawn.rs relocation). ROUTED 2026-08-18 round-table (was `Ownerless and open`). **FAIL-OPEN, treat as security-relevant**: a parser defaulting an omitted `permitted_tier` to T3 (highest privilege) while its own doc says a missing required field is an error is not a cosmetic defect. LANE: worker-grant hardening. TO SCHEDULE: John. Routing a lane is NOT the same as having a story; it must acquire one before Epic-14 closes — worker-grant hardening-lane debt. (Pre-existing RED on `check-dev-record-completeness` at HEAD `7aa07ee3`; the phrasing asserted an owner that resolves to no sprint-status key. Restated in the gate's own vocabulary by `j1-crosshost-2c` so the gate is GREEN again. The item itself is unchanged and still open.) Evidence: crates/maos-bin/src/worker_spawn.rs:170-202.
...
## Deferred from: code review of j1-crosshost-2c-two-host-signed-run (2026-08-18)

- Read-phase `awaiting response` bounded by `idle` rather than the operator partition window (`crates/maos-a2a-tcp/src/transport.rs:602-603`) — a peer that completes handshake and send-ACK then goes silent renders as untyped `TransportFailed("awaiting response")` with no frame id, the exact conflation AC3.3's typed mint was built to prevent, one phase later. Ratified AC3.1 scope was connect/send only.
  RE-ASSIGNED 2026-08-21 round-table (Mary · Paige · John · Sally · Winston · Amelia · Murat), ratified as R8 of `j1-crosshost-2d-paid-two-host-run` and discharged by that story's AC5.3. The prior assignment was to 2d itself, which is **code-free by twice-ratified construction** (R2): an assignee that is forbidden to write Rust is not an assignee, and the assertion would additionally have turned `check-dev-record-completeness` RED the instant 2d reached `done` — `xtask/src/check_dev_record_completeness.rs:182-183` buckets an assignee whose sprint status is `done` as `Stale`, and stale rows are unconditional violations at `:546-555` (reproduced in a sandbox).
  **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — fault typing on a cross-host OPERATOR surface belongs to the v2.0 operational-surface sweep, which already holds the `audit_query_latency` bench defect above and carries D7 and D18. LANE: v2.0 operational-surface sweep. TO SCHEDULE: John. Routing a lane is NOT the same as having a story; it must acquire one before Epic-14 closes — v2.0 operational-surface-sweep debt.
  Until it does, the defect is **operator-visible and documented rather than typed**: `_bmad-output/test-artifacts/runbook-j1-tier-2-signed-live-run.md` Phase 7 now states that a stalled paid run renders as `TransportFailed("awaiting response")` with no frame id, and that this rendering does **not** distinguish a network partition from a slow-but-live agent. Documenting an untyped fault is not typing it. Evidence: crates/maos-a2a-tcp/src/transport.rs:602-603.
  **CLOSED 2026-09-17 by `16-5`:** the response wait now uses the partition window and maps to typed `CrossHostPartitionTimeout` rather than a generic transport string.
- Intake-sink mutex held across the durable audit append (`crates/maos-a2a-core/src/router.rs:1490-1501`) — `observe_reply_guarded` (before_commit + audit append + dedup publish) executes under `intake_sink.lock()`, so concurrent delegation pushes and digest replies serialize behind audit-DB write latency. Lock scope is load-bearing for the nothing-Duplicate-until-durable invariant; narrowing needs a redesign plus load evidence. Perf follow-up.
- `CODE_INTERNAL`/`CODE_TIMEOUT` collapse into `IacBusError::CrossHostRouteFailure` with hand-duplicated display strings (`crates/maos-a2a-core/src/error.rs:200,206` vs `router.rs:1955-1964`) — two sources of truth for the same rendered text; a wording tweak to either desynchronizes them with no compile error and no sync test. Design acknowledged in-code ("the distinction survives in the rendered text").
- Write-path redaction blind to uppercase hex — `is_hex_byte` accepts only `a-f` (`crates/maos-iac/src/adapter/redaction.rs:135-137`), so a 32+-char A-F secret is neither scrubbed pre-write nor detectable by the pre-write tripwire. Pre-existing; the read-path scan half was patched in 2c review.

## Deferred from: the paid two-host run of j1-crosshost-2d-paid-two-host-run (2026-08-25)

- **The `claude` completion oracle scores a COMPLETED task as `not_completed:permission_denied` when any unrelated tool call was denied — a FALSE NEGATIVE, and the exact inverse of the false positive this lane already filed.** Measured on the paid run itself (`~/.maos/j1-t8-lab/paid`, model `claude-haiku-4-5-20251001`): the delegated goal was *"create a file named DELEGATED-PROOF.md …"*; the worker **wrote that file**, 1111 bytes, correct content, via the `acceptEdits` posture. It **also** made one incidental `Bash` call — `find /home/lunarpulse/.maos -type f -name "*.md"` — which the posture correctly denied. `permission_denials` therefore came back with exactly one entry, and `claude_result_object_oracle` (`crates/maos-bin/src/worker_cli.rs:498-537`) treats ANY non-empty `permission_denials` as non-completion, so `worker_completion` journaled `completed:false` and `host_b_delegation_served` carried `worker_outcome:"not_completed:permission_denied"` over a task that had in fact been performed. The result object's own fields contradict the verdict: `subtype:"success"`, `is_error:false`, `stop_reason:"end_turn"`, `num_turns:4`.
  **Why this matters beyond tidiness.** `j1-crosshost-2a` filed the `claude` asymmetry in ONE direction — no effect oracle, so a refusal-with-prose scores `completed=true` (Appendix B-9 of `runbook-j1-t8-two-host-paid-run.md`, reproduced free at HEAD 2026-08-25 with a fake adapter). The paid run shows the SAME missing effect oracle produces the opposite error too: with no way to ask *"did the goal's effect occur?"*, the adapter substitutes *"was anything denied?"*, which is neither necessary nor sufficient. A run can be signed as failed while its artifact sits on disk, and — per B-9 — signed as succeeded while nothing was written. **Both directions are the same absent control.**
  A fix must NOT be "ignore `permission_denials`": that field is the ONLY thing separating a refusal from a success on this adapter (`:490-497`), and dropping it re-opens the ship-blocker `2a` closed. The shape of a real fix is an **effect oracle** — the goal names an artifact, and completion is judged against that artifact — which is what `codex` gets natively from `item.completed`/`file_change` (`:462-465`) and `claude` does not get at all. Note `j1-crosshost-2e` (F7) already made the goal operator-supplied via `MAOS_DELEGATED_GOAL`, so the goal CAN name a checkable artifact today; nothing consumes it as an oracle.
  Until it is fixed the compensating control is manual and is written into the runbook as a **blocking** pre-signature step (Appendix B-9): read the worktree, do not read the verdict. This paid run's capture records the effect and the verdict as two separate facts rather than reconciling them.
  **CONTROLLED COMPARISON, run the same day and deliberately NOT signed.** An investigation-only second run (`~/.maos/j1-t8-lab/investigate`, $0.009105) held EVERYTHING constant — same binaries, same signed cohort manifest, same certs, same model `claude-haiku-4-5-20251001`, same two-process topology — and changed ONE thing: the goal added *"Use only file writing tools. Do not run shell commands and do not explore the filesystem."* Result: `DELEGATED-PROOF-B.md` written, `permission_denials: []`, `worker_completion completed:true`, `worker_outcome:"completed"`, 2 turns instead of 4. **So the variable is not the mechanism, the transport, the model or the task — it is solely whether the model happened to attempt something unrelated that the posture denied.** A completion verdict that flips on that is not measuring completion. The investigation run is recorded here as evidence and is NOT part of the signed artifact set: `_bmad-output/test-artifacts/j1-two-host-evidence/` holds only the ratified run, and tuning a goal until the verdict is green is exactly the canning this lane refuses.
  LANE: worker-adapter completion semantics. TO SCHEDULE: John. Routing a lane is NOT the same as having a story; it must acquire one before Epic-14 closes. Evidence: crates/maos-bin/src/worker_cli.rs:490-537 and :462-465; the signed halves at `_bmad-output/test-artifacts/j1-two-host-evidence/host-{a,b}-bundle.json` (the `cli.subprocess.output` row carries the full result object, `permission_denials` included, under Ed25519).

## Deferred from: §A6 review of j1-crosshost-2d-paid-two-host-run (2026-08-25)

*Reviewer model `sonnet` (≠ dev `anthropic/claude-opus-5`). All three need Rust; `j1-crosshost-2d` is code-free by twice-ratified construction (R2), so an assignee that may not write Rust is not an assignee — the same reasoning that re-assigned the `2c` fault-typing row above.*

- **Every green fixture in the J1 proven-red suite is already an instance of the forgery it fails to catch.** `xtask/tests/j1_crosshost_2c_proven_red.rs`'s `good_bundle()` helper (`:270-295`) hardcodes `"attester_pubkey": "aa".repeat(32)` and `"signature": "bb".repeat(64)` **identically for both the `"host-a"` and `"host-b"` calls**, and it backs all 43 tests in the file. So the suite has **never once exercised a genuinely two-key artifact through this gate**, which is why F6 went unenrolled: the shape is present in every baseline but asserted by none. Proven 2026-08-25 by the §A6 Blind Hunter, which forged two gate-admissible artifact sets in a sandbox mirror (one key in both halves; and non-hex garbage signatures) — both `passed:true`, `findings:[]`, exit 0 against the real gate binary. Note the trap for whoever fixes this: adding an `attester_pubkey`-inequality check to leg 9 will RED `published_capture_template_is_admissible_by_the_real_gate`, and that RED reads as *"the template broke"* unless the fixture is fixed in the same commit. Also note `reading_attester_pubkey_inside_reconciliation_reds` (`:644`, rule R-RG1) **forbids** reconciliation from trusting the artifact's self-declared pubkey — so the fix is not "make leg 9 read the pubkeys and trust them"; it is either an out-of-process verification the gate deliberately does not do (module doc `:54-60`: a shelled `cargo` vacuums fixtures) or an honest acceptance that this control is operator-held. Whatever is chosen, the fixture must stop shipping a forgery as its definition of good. LANE: J1 gate + test-infra. TO SCHEDULE: John. Evidence: xtask/tests/j1_crosshost_2c_proven_red.rs:270-295 and :386-414; xtask/src/check_j1_two_host_signed_run.rs leg `paid-run-capture`.

- **`detect_credential` does substring matching while its helper is named `contains_prefix`, so ordinary English words containing `sk-` are flagged as `api_key_generic`.** `crates/maos-iac/src/adapter/redaction.rs:81-83,309-321` runs `haystack.windows(needle.len()).any(|w| w == needle)` against `b"sk-"`. Proven by execution 2026-08-25: `"task-file"`, `"risk-report"` and `"disk-image"` all return `Some("api_key_generic")`; the un-hyphenated `"the task"` / `"risk of failure"` correctly return `None`. This is a **fail-closed availability bug on a paid path**: the scanner runs in `maosctl audit record-capture`'s `CaptureDoc` path (`crates/maos-cli/src/subcommands.rs:2830`) over the whole canonicalized JSON blob, so an operator who writes *"reviewed the task-file"* in a capture note is refused **after the agent is billed**. It does not reach `check-j1-two-host-signed-run` (grepped: zero references), and the landed evidence contains no `sk-` occurrence. LANE: redaction/credential-scanner correctness. TO SCHEDULE: John. Evidence: crates/maos-iac/src/adapter/redaction.rs:81-83,309-321; crates/maos-cli/src/subcommands.rs:2830.

- **An empty `two_host_shape` silently disables all four two-host controls at signing time.** `CaptureDoc::validate_two_host()` (`crates/maos-cli/src/subcommands.rs`) returns `Ok(())` immediately when `two_host_shape.trim().is_empty()`. Proven 2026-08-25 by two temporary unit tests (added, executed, reverted): with `two_host_shape: ""`, a CaptureDoc carrying `two_host_trust_anchor: "negotiated automatically by the protocol, no human involved"`, `two_host_host_b_audit_key: "same key as host A, welded"` and an EMPTY `two_host_stranger_verification` validates **Ok** and would be journaled and signed; setting `two_host_shape` to any non-empty value re-enables the block and the same document is refused. The skip is deliberate (it keeps every pre-2c single-host capture valid) but it is implemented as *absence disables the checks* rather than *absence forbids the claims*, so the three fields it guards can be signed as free prose. Not exploited here — both landed bundles carry `two_host_shape: "two-processes-one-box"`, so the full block ran. LANE: capture-validation correctness. TO SCHEDULE: John. Evidence: crates/maos-cli/src/subcommands.rs `CaptureDoc::validate_two_host`.

## Deferred from: code review of 14-1-100-host-churn-scale-envelope (2026-08-27)

- **`check-scale-churn`'s summary omits the percentile sample counts AC3.4.a names as its third publication surface.** `xtask/src/check_scale_churn.rs:425-438` discards successful `cargo test` output and `legs_json` serializes only the statically-required host markers, so neither the stderr summary nor the JSON carries `detection_latency_median_secs`, `detection_latency_p99_secs` or their `n=3` population. AC3.4.a's substantive control (a reader cannot mistake a max-of-three for a characterised tail) IS met on the other two surfaces — the test output prints both percentiles with `n=`, and `report_to_markdown` names `n` inline. Parsing the percentile lines into the leg result needs new `xtask/src` lines against **3 lines of remaining ceiling headroom** (40610/40613), so it is sequenced behind the marker-exactness and enrollment-binding patches from the same review, which should reclaim first. Owner: `21-4-one-instrument-and-env-registry` (instrument work; re-homed from 14-6 by correct-course 2026-09-04). Found by the Acceptance Auditor layer (confidence 0.99).

## Deferred from: code review of 14-1-100-host-churn-scale-envelope (2026-08-28)

- **[CLOSED by 14-2, 2026-08-29 — the same commit as 14-2's status flip]** `tests/coverage-matrix.yaml` NFR-Rel-9 row notes were re-based onto the PRD's real text (`≤ 5s p99 under 10⁴ concurrent capability-token validations`) and its false rotation-gate mapping was removed; the 30s/90s floors were re-attributed to NFR-Sec-13's rotation-chaos scope; NFR-Sec-12 and NFR-Sec-13 now name the gate legs that enforce them. Mislabel surfaces were corrected in `gate-registry.toml`, `discipline.yml`, and `docs/release/v2.2-capacity-envelope.md`. The leg-structured `check-rotation-real-timing` gate is GREEN with 5 legs, 11 enrolled rotation tests, and 11 exact oracle invocations.

## Deferred from: code review of 14-2a-production-mtls-rotation-trigger (2026-08-30)

- **Operator HTTP serializes every connection on one accept thread** (`crates/maos-control/src/lib.rs:127-175`) — `handle_connection` runs inline in the accept loop, and each socket may consume the two-second read timeout before the next authenticated operator request is accepted. Pre-existing to Story 14-2a; the rotation read surface makes the availability cost more visible but does not introduce the server architecture. A bounded per-connection worker design belongs to the operator-surface owner rather than this trust-rotation patch. **CLOSED 2026-09-14 by Story `16-1-daemon-post-surface-and-verb-retarget`** (rule 10(a) — the operator-surface owner rewrote its own loop): each accepted connection is handed to a bounded worker pool (cap 16; over-cap ⇒ immediate fixed 503 busy body), the per-`read` 2 s timeout is replaced by a whole-request 5 s read deadline, non-`WouldBlock` accept errors `continue` instead of breaking the listener, and handler panics are contained by `catch_unwind` ⇒ fixed 500 body.

## Deferred from: code review of 14-2c-plane-c-local-leaf-declaration (2026-09-04)

- **`manifest()`/`peer_convergence()` read under two separate locks in `CohortSelfIdentity::self_identity`** (`crates/maos-bin/src/cert_rotation.rs`) — a manifest reissue committing between the two reads can render one operator request with an old `declared` fingerprint and a fresh convergence table (transient false `agree` verdict; self-corrects on the next poll). Two-lock composition is pre-existing; fixing it needs a single-snapshot read API in `maos-cohort` (new API surface + budget). Owner: `21-3-durable-tofu-and-self-leaf-rotation`, which owns the self-identity surface.

## Deferred from: code review of 15-2-kloc-ceiling-rebase (2026-09-06)

- **Pre-existing stale line-number citations into `xtask/kloc.toml`** — `xtask/src/check_decision_register.rs:12` names `xtask/kloc.toml:208` as one of the two prose mentions of `epic-14-preflight-decisions` in the tree, but those mentions sat at `:195`/`:251`/`:268` before Story 15-2 and at `:199`/`:258`/`:275` after it; the citation was already wrong when written. `xtask/kloc.toml:232` cites `` `:551` ``, a line that did not exist at authoring time (the file was 516 lines). Neither is one of the four citations Story 15-2's F2/T10 declared in scope, and both are the class of defect that only stops recurring once grants and policy anchors are machine-readable. Owner: `21-4-one-instrument-and-env-registry` (D13(b)), which unifies the instrument and creates the data a citation checker could validate.

## Deferred from: code review of 15-5-decision-adrs-and-provisioning-checklist (2026-09-08)

- **Oracle frontmatter/index parser edges** — duplicate frontmatter keys last-win; UTF-8 BOM reds well-formed frontmatter with a misleading message; `## Context` binds to the first occurrence anywhere in the body (not a heading line); forbidden-marker scan is plain substring over the whole ADR incl. frontmatter ("Todorov" matches `todo`); indented index rows misdiagnose; `waived-by` dates unvalidated. All dormant (real corpus clean) and mostly fail-closed. Evidence: `xtask/tests/decision_adrs_and_provisioning.rs:107-127,:156-163,:414,:480-487`.
...
- **`EnvStability` declarations have no user-facing reader.** `MAOS_INFERENCE_MODE` and
  ...
  `check-env-contract` nor a runtime/docs surface renders that classification. **Owner: `16-4-maos-uninstall-and-keyring`** — add the `maos --help` / `docs/maos.dev/run-maos.md`
...
## Deferred from: 15-4-release-repair-and-first-signed-tag (2026-09-12)

- **Cut line 3 — the committed dev *seed* is not removed.** `crates/maos-audit/src/release_verify.rs:254` is one of **33 `dev_seed` occurrences across six files** (`release_verify.rs`, `crates/maos-cli/src/subcommands.rs`, `crates/maos-eval/src/trial_attestation.rs`, `xtask/src/check_trial_attestation.rs`, `xtask/src/check_third_party_trial.rs`, `xtask/tests/trial_attestation_proven_red.rs`; **25 `dev_seed(` call sites** once `producer_dev_seed` is excluded). The bundled `RELEASE_PUBKEY` it derives is consumed on a **production** path — `verify_release(…, &RELEASE_PUBKEY, …)` at `crates/maos-eval/src/trial_attestation.rs:379`, with seed fallbacks at `:67`/`:76`/`:86`. Whether the bundled default should exist at all is a security-design decision with a multi-crate blast radius, not a release-repair edit. The non-shippable half **is** repaired in 15-4: `maosctl release-pubkey` plus the `sign-and-publish` refusal step hard-fail a release whose binary embeds the development key. **Ownerless and open.** *Dispositioned by the §A6 code review of 15-4, 2026-09-12: ratified as a DECLARED CUT LINE — the story's own `## Declared cut lines` section names it a boundary, not an omission, and the review confirmed no owner exists to assign; it is filed so a future reader can tell the boundary from a gap.*
- **Cut line 6 — `docs/release/windows-deferral.md:23` claims `x86_64-pc-windows-msvc` is in the release matrix; it is not.** The `.github/workflows/release.yml` matrix is the three targets `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`, `aarch64-apple-darwin`. A false posture statement in another story's document; repairing it was out of 15-4's scope. **Ownerless and open.** *Dispositioned by the §A6 code review of 15-4, 2026-09-12: ratified as a DECLARED CUT LINE — the story's own `## Declared cut lines` section names it a boundary, not an omission, and the review confirmed no owner exists to assign; it is filed so a future reader can tell the boundary from a gap.*
- **Cut line 8 — no `strip` is added to the release profile.** Measured: the published binary is 33,195,488 bytes unstripped against 26,503,592 stripped, and `onb-nfr2-timing` strips before it measures (`tests/integration/onb_nfr2_timing.sh:97-98`), so that gate has never measured the artifact the release publishes. Changing what bytes get signed is a release-content decision. **Ownerless and open.** *Dispositioned by the §A6 code review of 15-4, 2026-09-12: ratified as a DECLARED CUT LINE — the story's own `## Declared cut lines` section names it a boundary, not an omission, and the review confirmed no owner exists to assign; it is filed so a future reader can tell the boundary from a gap.*

## Deferred from: Story 16-0 — the kernel pin gains a file set and a per-file content hash (2026-09-13)

> Each row names ONE owner by charter match, never a default bucket (rule 10). Filed, not swallowed.

- **The D11 `xtask` ceiling ratchet is a null control for the very raise it is supposed to police** [`xtask/tests/d11_xtask_ceiling_ratchet.rs:41-85,142-179,195-213`] — `authorize_xtask_raise` (with `Refusal::{MissingStory,UnknownStory,DoneStory,Frozen}`) is called ONLY from synthetic fixtures (`:88-131`) and never sees the real `kloc.toml` row. What binds against the tree is one weaker assertion (`:195-213`) that (a) checks only that a cited key is **present** in `development_status`, never that it is OPEN — `DoneStory` is fixture-only — and (b) scans every comment line in the whole file rather than the `xtask` row's block, because the `#` branch of `real_xtask_row` (`:142-159`) sits outside the `xtask` branch. Replaying its tokenizer against the real file, **24 citations already resolve** (`14-0`…`21-5`), so the assertion is green no matter what any story writes. Measured again at 16-0: the row's comment tokenizes to `['15-4', '16-0']` and the test passes — but it would also have passed with no 16-0 citation at all. *Reason deferred:* repairing it here would make 16-0 the story that fixes the control funding its own raise — the Story 13.6e shape (*a control whose owner has already shipped*) this story exists to avoid. **Owner: `20-3a-gate-honesty`** — exact charter match (correctness of existing gates). D-16-0-J. ⚠ Tokenizer trap, measured and worth carrying: `story_keys` (`:164-179`) splits on any char that is neither an ASCII digit nor `-` and then requires EXACTLY TWO all-digit parts, so a comment citing the **full** sprint key `16-0-kernel-pin-content-hash` yields the token `16-0-` (three parts, third empty) and matches **nothing**. A citation must carry the bare `16-0` followed by a non-digit, non-dash character.
- **Four independent readers of `src_lines` and four independent line-counting implementations** [`xtask/src/check_kernel_baseline.rs:86,99`, `crates/maos-a2a-tcp/tests/t11_t12_chaos_absence.rs:196-203,216`, `xtask/tests/fkcs_oracle.rs:170,180`, `xtask/src/check_fkcs.rs:836`] — the gate's doc comment claimed the a2a-tcp guard *"reads the SAME toml (no second literal)"*, which was false. **The false doc comment is corrected by 16-0** (it now names all four readers and says why they stay separate); the duplication itself is NOT removed, because `t11_t12_chaos_absence.rs` lives in a kernel-adjacent crate that must not grow an `xtask` dependency and `check-service-boundary` is the control that forbids it. 16-0 pins the live hazard instead, in both directions: no new key carries an `src_lines` prefix, and `[kernel_src]` lands strictly after the assignment (`no_key_before_src_lines_begins_with_src_lines`). *Reason deferred:* unification needs a shared crate that does not exist. **Owner: `21-4-one-instrument-and-env-registry`**, which already holds the one-instrument charter. D-16-0-I.
- **`t11_t12_chaos_absence.rs:199-202` breaks on any `=` inside a value** [`crates/maos-a2a-tcp/tests/t11_t12_chaos_absence.rs:199-202`] — it resolves the pin with `.rsplit('=').next()` and then `.expect()`s, so a value containing `=` yields the wrong parse and a near-miss key **panics** the test process rather than failing a gate. Not triggered by 16-0's keys (they carry no `src_lines` prefix and land after `:481`, and the shape is regression-tested from the xtask side by `the_three_other_readers_still_resolve_the_pin`), but it is a live fragility in a kernel-adjacent guard. **Owner: `21-4-one-instrument-and-env-registry`** — same reader-unification charter.

## Deferred from: code review of 16-0-kernel-pin-content-hash (2026-09-13)

- **No permanent regression vector for the three-`?`-caller red-path emission** [`xtask/src/check_cohort_mesh.rs:702-714`, `check_multi_tenant_loom.rs:1889`, `check_epic_close_coherence.rs:124`] — AC5's planted-red falsifier ("each `?` caller still emits its own report") was executed manually by the dev (Debug Log) and re-executed by the 2026-09-13 §A6 runtime layer (PASS), but nothing commits it: deleting cohort-mesh's `if json` emit block at `:704-709` — the exact zero-byte-`--json` defect this story found and fixed — passes all 20 vectors. Committing the vector needs a seam that does not mutate the tracked baseline mid-suite (in-binary tests run in parallel threads); candidate: a serialized `CARGO_BIN_EXE_xtask`-driven vector behind a suite-wide mutex, or a `check_at`-style seam on the composite gates. **Ownerless and open — needs a charter match (candidate: `20-3a-gate-honesty`, correctness-of-existing-gates).** *Dispositioned by the §A6 code review of 16-0, 2026-09-13: candidate owner named by charter match and the assignment ratified at that story's creation; filed so a future reader can tell a queued owner from a gap.*
- **CI wiring for the kernel-pin suite is unpinned by any local test** [`.github/workflows/discipline.yml:2089,2106` (fetch-depth, named step, `components: rustfmt`)] — deleting any of the three lines reds loudly in CI (proven in a faithful depth-1 simulation by the §A6 runtime layer, named message included) but nothing catches it pre-push; the repo's own doctrine (`discipline.yml:424` — a test not named in its own step is a test nothing runs) suggests the wiring deserves a pin. **Ownerless and open — candidate owner: `20-3a-gate-honesty` (gate enrolment correctness) or the Epic-16 hermetic-exit owner.** *Dispositioned by the §A6 code review of 16-0, 2026-09-13: candidate owner named by charter match and the assignment ratified at that story's creation; filed so a future reader can tell a queued owner from a gap.*
- **The AC4 reflow vector depends on a floating-`stable` rustfmt** [`xtask/tests/kernel_pin_content_hash_16_0.rs:391,464`; `discipline.yml` job installs `toolchain: stable`] — a stable-roll rustfmt whose formatting differs would red the kernel job as "rustfmt did not restore the tree", misattributing toolchain drift as kernel-pin drift (check-fmt would redden too, so it is loud, not silent). A date/toolchain pin or a `rustfmt --version` assert in the vector would attribute it correctly. **Ownerless and open — toolchain-pinning is an ops/workflow decision; candidate owner: `ops-*` lane or 21-4's instrument charter.** *Dispositioned by the §A6 code review of 16-0, 2026-09-13: candidate owner named by charter match and the assignment ratified at that story's creation; filed so a future reader can tell a queued owner from a gap.*

## Deferred from: Story 16-1 — the daemon post surface and verb retarget (2026-09-14)

> Each row names ONE owner by charter match, never a bucket (rule 10). Filed from the story's declared cut lines (§11), not swallowed.

- **Lifecycle Journal cross-process overwrite** [`crates/maos-kernel-core/src/journal/mod.rs:98-102,178-181`] — `JournalAdapter::open` opens the journal with `.write(true)` and no `.append(true)`, and `append_transition` writes at the handle's cursor, so a second process on the same home writes from the cursor the first left and can overwrite its lines mid-record (replayed during 16-1's spike: a one-shot's line overwritten, torn fragment left). The append flag is a kernel-core byte and 16-1 is pinned out of kernel-core. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — the only Epic-16 story holding a FLAG-Winston kernel grant.
  **CLOSED 2026-09-17 by `16-5`:** journal handles use `O_APPEND`, each NDJSON record is one pre-built `write_all`, and independent-adapter concurrent writes recover without loss.
- **Home split-brain beyond `control.json`** [`crates/maos-audit/src/lib.rs:1455-1476,1555-1575`] — the private memory root resolves from `MAOS_MEMORY_ROOT`/`XDG_DATA_HOME`/`HOME` and the spirit-archive root likewise; neither reads `MAOS_HOME`, so a root with `MAOS_HOME=/a` and an offline child with `MAOS_HOME=/b` share stores their home rules believe are separate. Meanwhile the 16-1 store lock set (D-16-1-D) holds the race closed: roots take it shared and offline durable children exclusively, on the store directories' own handles, whatever home resolved them. **Owner: `16-4-maos-uninstall-and-keyring`** — its AC1 already enumerates `MAOS_HOME` and the XDG data root, and its `maos purge` deletes those directories, so purge must take the same lock set exclusively.
- **`resolve_spirit_name`'s `hello-spirit → pid 0` wildcard** [`crates/maos-audit/src/lib.rs:1624-1665`] — the v0.1-β fallback answered the name `hello-spirit` with `SELECT DISTINCT boot_nonce, spirit_pid … WHERE spirit_pid = 0`, which matched EVERY pid-0 kernel row regardless of which spirit wrote it. **CLOSED 2026-09-14 by `16-2-shell-halt-registry-and-j0-scene` (D-16-2-F)**: BOTH pid-0 wildcards retired (maos-audit's branch deleted; maos-shell's private `resolve_spirit_pid` deleted, `run_audit_query` now resolves through the identity rows with a boot+pid filter); the `MAOS_ONE_SHOT=hello-spirit` evaluator run writes the kind-19 identity row the resolution reads; the 13.5b erasure fixtures reseed it; name resolution now fails closed (`unknown spirit '<name>' — no admission or load row names it in this Transparency Log`), with legacy pre-16-2 rows reachable by `--boot <nonce>` and erasable by 16-4's `maos purge`.

## Deferred from: code review of 16-2-shell-halt-registry-and-j0-scene (2026-09-15)

> §A6 net, 5 layers (Blind + Edge Case + Acceptance + Test-Infra + runtime re-execution of obligations (a)–(o)). Each row names ONE owner by charter match, never a bucket (rule 10). 16 `patch` findings stayed in the story as action items; only genuinely pre-existing or latent items are filed here.

- **Registry flips to `Resumed` BEFORE the resolver's context write; a failed `memory.write` strands the halt terminal-Resumed forever** [`crates/maos-kernel-core/src/halt/resolver.rs:143-175`; REPL side `crates/maos-shell/src/lib.rs:603-613`] — `registry.resolve(halt_id, terminal)` commits the state transition, then `memory.write(… "halt_context::<id>" …)` runs and on failure returns `ResolveError::Internal` with no rollback. Story 16-2's Trap 5 documents the benign ordering gap and bounds the REPL's re-read at 50 ticks (5 s), but the FAILURE branch is unhandled: the halt is now terminal-`Resumed` so a retried `maosctl halt resolve` answers `halt_already_resolved`, the operator's clarification text is unrecoverable, and the REPL abandons the directive printing only `halt <id> resolved: provided_context (context not delivered)` — less feedback than the `Terminated` arm's `turn abandoned (accepted_halt)`. The reordering (or a compensating rollback) is a kernel-core byte and 16-2 is pinned out of kernel-core (Trap 12). **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — the only Epic-16 story holding a FLAG-Winston kernel grant, and its D3 charter is audit durability/truth.
  **CLOSED 2026-09-17 by `16-5`:** context is written before the registry transition; write failure leaves the halt pending and retryable.
- **`terminate_spirit` receipts skipped for a Spirit loaded-but-never-started, and on `on_unload` hook failure** [`crates/maos-kernel-core/src/scheduler/control_block.rs:446-454`; `scheduler_loop.rs:497-512`] — `is_transition_allowed` has no `(Loaded, Unloaded)` arm (explicit test `loaded_to_unloaded_rejected`), and `fire_on_unload` runs BEFORE `terminate_spirit`, so both paths skip the NFR-Rel-11 `PlannedUnload` receipts. **Already filed by Story 16-2 itself** in the `epic-16-retrospective` sprint row; re-filed here because 16-2's new `finish_shell_session` is the first caller to reach it on a shell path (where it also masks the causal admission error — that half stayed in the story as a `patch`). Latent only for hello-spirit, whose manifest declares no hooks. **Owner: `epic-16-retrospective`** (kernel bytes, no story charter) — as 16-2 ratified.
- **`classify_halt_record` labels the kernel's serde-failure receipt payload `raised`** [`crates/maos-cli/src/subcommands.rs:2003-2004` against `crates/maos-kernel-core/src/halt/termination.rs:105-110`] — when `serde_json::to_vec(&receipt)` fails, `terminate_spirit` writes `{"error":"…","halt_id":"<ULID>"}`; that payload fails `HaltReceipt` deserialization (no `timestamp_ns`/`spirit_pid`/`boot_nonce`/`frame_id`) but parses as `EpistemicHaltPayload`, whose only required field is `halt_id` — so a termination row renders as a raised halt in `maosctl halt list`. Unreachable today (every receipt field serializes), but D-16-2-E's five-value `record` contract is the audit surface's truth and has this corner. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — charter match (audit durability/truth); the clean fix is kernel-side (a distinct `FrameKind` or a typed error payload), which needs 16-5's grant.
  **CLOSED 2026-09-17 by `16-5`:** `classify_halt_record` recognizes a termination receipt before the permissive raised-halt payload.
- **Journal-write failure after resolver commit yields a resolution with no `approval_decision_log` row** [`crates/maos-bin/src/operator_door.rs:954-981` over `crates/maos-director-surface/src/halt_ui.rs:80-84`] — `submit_resolution` runs `resolver.resolve` (which flips the registry AND writes the context) and only THEN `journal_halt_resolution`. If the journal write fails the door answers `Failed{code:"halt_resolution_failed"}` and the REPL prints the refusal, but the next tick sees `Resumed`, `take_context` succeeds, and the Spirit PROCEEDS with the operator's context while the approval log holds nothing. Story 16-2's "exactly one approval row" obligation (d) holds for RACING surfaces (the registry CAS makes the loser `halt_already_resolved` — measured live) but not for this post-commit boundary. Pre-existing ordering; fail-closed in the intended direction (no journal row without resolver success), unhandled in reverse. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D3 audit durability is exactly "an audit event dropped while the operation returns".
  **CLOSED 2026-09-17 by `16-5`:** an approval-journal failure rolls the registry resolution back to its pending state and preserves retryability.
- **REPL clarification has no size cap while the HTTP surface caps at 64 KiB** [`crates/maos-shell/src/lib.rs:431` vs `crates/maos-control/src/lib.rs:215`] — D-16-2-C's "one mechanism, two surfaces" is true of the resolution path but not of input bounding: an HTTP `halt resolve` body over `MAX_BODY_BYTES` is refused unread by `post_guard`, while the same resolution typed at the REPL carries an arbitrarily long stdin line verbatim into `Resolution::ProvidedContext`, the private-tier memory write and the journal `reasoning` string. Unbounded memory/journal growth from one pasted line. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — it owns the journal-durability surface the oversized `reasoning` string lands in; the cap belongs beside the existing `MAX_BODY_BYTES` rule.
  **CLOSED 2026-09-17 by `16-5`:** REPL clarification input is capped at the shared 64 KiB boundary and oversized input is explicitly refused.
- **`take_context` reads but never removes `halt_context::<id>`** [`crates/maos-bin/src/shell_host.rs:144-154`] — the trait method's name promises consumption; the impl is a pure `MemoryManagerPort::read`, and no `delete`/`forget` of the key exists on the shell or resolver path. The bounded re-read loop (Trap 5) legitimately needs idempotent reads, but after `proceed_with` the key persists in the Private tier for the process lifetime, and on disk under `retention = "persist"`, one row per halt. Safe today because ULID halt ids never repeat, so no stale context can be served to a later turn. Owed either a post-proceed delete or a contract rename. **Owner: `16-4-maos-uninstall-and-keyring`** — its `maos purge` already enumerates and erases the private-tier root, so it owns private-tier residue by charter (same routing 16-2 §11 row 4 used for legacy TL rows).
- **`run_bounded` can deadlock a chatty child on full pipes** [`crates/maos-journey-test/src/lib.rs:742-761`] — the bounded loop is `child.try_wait()` + 50 ms sleep while `stdout`/`stderr` stay `piped()` and are drained only AFTER exit via `wait_with_output()`. A child writing past the OS pipe buffer (~64 KiB) blocks on write forever, `try_wait` never yields `Some`, the deadline fires, the child is killed and the helper panics `"{label}: did not exit within {timeout_secs}s"` — a hang misreported as a timeout. Latent: every current call site (`maos init`, `maosctl …`) is small-output. The generic helper's timeout contract is unsound for any future verbose child. **Owner: `16-3-subprocess-crash-to-handle-crash`** — its charter is subprocess supervision and it already owns the crash/hang corpora (100-kill/50-hang), so the concurrent-drain fix belongs with it. **CLOSED 2026-09-15 by `16-3-subprocess-crash-to-handle-crash` (D-16-3-O)**: `run_bounded` now drains stdout and stderr on two threads WHILE the bounded `try_wait` loop runs and joins them for the capture (`crates/maos-journey-test/src/lib.rs:726-796`); the regression vector `crates/maos-journey-test/tests/run_bounded_full_pipe.rs` spawns a child writing 1 MiB to each stream and it exits inside the bound with both captured whole — red at the old shape (`full-pipe-drain: did not exit within 10s`, failed at 10.02 s), green at the fix (0.07 s).
- **`cargo test -p maos-journey-test` can silently test a STALE `target/debug/maos`** [`crates/maos-journey-test/tests/journey_j0.rs:23-41`] — the journey tests exec a PREBUILT binary path, and `cargo test -p maos-journey-test` does not rebuild `maos-bin`. This bit the 2026-09-15 review's own runtime layer: the obligation-(e) falsifier (`invoke_halt` → `println!`) initially PASSED because `target/debug/maos`'s mtime predated the source edit; the red appeared only after an explicit `cargo build -p maos-bin`. CI is safe — `journey-hermetic-tier-1` (`discipline.yml:2180`) and the new `j0-scene` job (`:3610`) both build the binaries first — but locally any journey falsifier can produce a false green, which is exactly how a null control is born (E15-A6). Owed: a build-freshness assertion in the harness (compare binary mtime against the crate sources, or assert a build-stamp string) so a stale binary reds instead of passing. **Owner: `20-3a-gate-honesty`** — its charter is "absent/indeterminate evidence is never a silent PASS", and a stale-binary green is precisely that.
- **`v01_evaluator_path.sh` exit-capture repair landed in Story 16-2's sweep rather than being routed (rule-10 waiver)** [`tests/integration/v01_evaluator_path.sh:132-165`] — `assert_refused_ansi_free` was rewritten to capture `$?` directly (temp files, `set +e`, explicit code compare) because the old shape "failed even against a correct binary — measured red at `bb9f0657` in a clean worktree". AC5 authorised only "step 3 passes unchanged"; the helper backing steps 4a-4c is a pre-existing defect fixed outside any AC, decision or §11 cut line, and rule 10 wants such items routed with an owner rather than swept. Reverting a real fix is worse than recording it, so this row IS the waiver: the repair stands, the process deviation is on the record, and it changes what a CI gate can fail on. **No further work owed** — filed for audit traceability only.

## Deferred from: Story 16-3 — subprocess crash → handle crash (2026-09-15)

> Story 16-3 §11 cut lines filed at T7 (rows 7, 9, 10, 12, 13, 14). Each row names ONE owner by charter match, never a bucket (rule 10). The section's other rows are handled where the story routed them — the retro/owner-story §12 notes, the recorded 16-5 AC1 decision, and `20-3b-gate-retirement-and-coverage-generator` — not as rows here.

- **TL redaction scrubs any `sk-` substring, e.g. every `task-…` id** [`crates/maos-iac/src/adapter/redaction.rs:81`; probe in 16-3 §11] — the generic redaction rule carries the bare prefix `sk-`, so the probe measured the TL's rendered surface masking not only API keys but every id containing `sk-` — every `task-…` id included — and the audit surface stops naming the very rows it records. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D3's audit-truth charter owns what the TL shows; a boundary-aware rule belongs beside the kind/token vocabulary 16-5 is adding. Note in 16-5 AC1 (§12).
  **CLOSED 2026-09-17 by `16-5`:** generic `sk-` redaction now requires a token boundary; task IDs remain intact while real key-shaped values remain redacted.
- **Worker SCB `sandbox_tier` shows T2 and the process runs bare** [`crates/maos-kernel-core/src/scheduler/control_block.rs:251,362`; `crates/maos-kernel-core/src/lifecycle/cli_wrapper/runtime.rs:461`] — the Worker's SCB records the T2 tier while the spawn is a bare `Command::new` with no sandbox applied (`runtime.rs:461`), so the audit surface claims a confinement the OS never enforced. **Owner: `17-1-worker-egress-allowlist-and-scoped-credential`** — its AC4 makes `spawn_t3` the single Worker spawn path; the tier row becomes true when the sandbox actually lands. Note in 17-1 AC4 (§12).
- **Tier-denial message prints `permitted == requested` when no grant exists** [`crates/maos-domain/src/host_grant.rs:134-138`] — the `NoMatchingGrant` arm fills `permitted` with `requested` itself, so the refusal an operator reads states a permitted tier equal to the requested one and cannot be told apart from a grant that exists and matches. **Owner: `17-1-worker-egress-allowlist-and-scoped-credential`** — rule 10(b): 17-1's AC4 reworks the tier-denial surface the message lives in. Note in 17-1 AC4 (§12).
- **Kind-1 kernel rows the FR4 doorbell cannot see or match**: `insert_kernel_event_returning_id` raw rows at a real pid (already red at HEAD) and the non-JSON `task.orphaned` at pid 0 [`crates/maos-iac/src/adapter/transparency_log.rs:1364-1405`; callers `crates/maos-bin/src/main.rs:5796,10411,10513`; `crates/maos-kernel-core/src/halt/resolver.rs:223`] — the kernel writes kind-1 rows the FR4 read-back cannot see or match: the raw rows at a real pid are already red at HEAD, and the `task.orphaned` disposition lands at pid 0 in a non-JSON shape. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — the same recorded kernel-rows decision its AC1 holds (§12 note).
  **CLOSED 2026-09-17 by `16-5`:** caller-supplied kind/token values reach the TL, task-orphan payloads are typed JSON, and the FR4 doorbell covers kind-1 and lifecycle DecisionDispatch writers.
- **`MAOS_ONE_SHOT` arms whose audit drains time out** (`smoke-epic-4`, `smoke-spirit-5`, `smoke-supervision-5`, `hello-spirit`) [`crates/maos-bin/src/main.rs:6113-6121`; measured ~5.06 s each at `af96c907`] — the one-shot arms bound their audit-writer drain at 5 s (`tokio::time::timeout`, `:6117`) and all four named arms reach the bound, printing `audit writer drain timed out after 5s` on every invocation. **Owner: `epic-16-retrospective`** — no open story's charter covers one-shot teardown; `hello-spirit` is the FR58 evaluator path (and `onb_nfr2_timing.sh`'s subject), so the retro must decide a vehicle.
- **A hot swap drains butler's pending halt with no receipt** [`crates/maos-kernel-core/src/halt/mod.rs:440`] — `validate_swap_halt_continuity` drains the predecessor's pending halts (`drain_for_spirit`, `:440`) and its `SafeDrained` verdict carries only a count; no receipt is written for a drained halt, so the approval log holds a halt that ended without a recorded decision. **Owner: `epic-16-retrospective`** — kernel byte, no charter.

## Deferred from: Story 16-4 — MAOS purge and keyring (2026-09-16)

> Story 16-4 §11 rows 3, 5, 7, and 10. Each row names one owner by charter match; purge handles the existing state without silently changing its producer.

- **`run_uninstall_cascade` proves emptiness from shared-tier rows only** [`crates/maos-bin/src/main.rs`, `run_uninstall_cascade`] — the cascade can report that nothing remains while private-tier state still exists. Story 16-4 deletes the private-tier root but does not change the cascade's proof. **Owner: `16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — its D3 audit-truth charter owns the proof rendered to operators.
  **CLOSED 2026-09-17 by `16-5`:** both NotFound gates now require zero rows in shared and private principal tiers.
- **`cargo deny` is advisory and clippy is not executed** [`.github/workflows/discipline.yml:38,44,121`] — job-level `continue-on-error: true` swallows a deny failure, while clippy is installed but no job invokes it. Story 16-4 ran `cargo deny check bans` locally before accepting the keyring tree; that does not repair CI enforcement. **Owner: `20-3b-gate-retirement-and-coverage-generator`** — its gate-disposition charter owns whether these controls become binding or are retired.
- **`migration_plan::optional_plan_path` has no `$HOME/.maos` fallback** [`crates/maos-bin/src/migration_plan.rs:183-192`] — with `MAOS_HOME` unset, migration plans can be omitted instead of persisting under the default home. Purge enumerates the resulting roots but must not relocate live state while deleting it. **Owner: `epic-16-retrospective`** — no open implementation story owns migration-plan path resolution.
- **Three data resolvers ignore `XDG_DATA_HOME`** [`maos_registry::storage::dirs_fallback`, `maos_skill::store::dirs_fallback`, `maos_registry::yank::cursor_file_path`] — they continue to resolve legacy `$HOME/.local/share/maos` paths. Story 16-4 enumerates those paths so purge removes existing data, but changing the writers would require a migration. **Owner: `epic-16-retrospective`** — decide the migration vehicle before changing producer paths.

## Deferred from: code review of 16-4-maos-uninstall-and-keyring (2026-09-16)

- **Legacy `/tmp/maos/crl` written by pre-16-4 builds is not enumerated by purge** [`crates/maos-bin/src/purge.rs`, `maos_roots`] — the hardcoded `/tmp/maos/crl` registry client path was removed in this story, but hosts that ran earlier builds retain CRL state there; purge covers only `default_crl_dir()`/`MAOS_CRL_PATH`. Same class as the routed `/tmp` legacy-fallback row; `/tmp` is volatile. **Owner: `epic-16-retrospective`** — decide whether a legacy-CRL enumeration leg is worth the bytes.
- **rustls 0.23.40 carries RUSTSEC-2026-0285 and chacha20 0.10.0 is yanked** [`Cargo.lock`] — review round 2 reverted 16-4's unrelated dependency bumps to keep obligation (i) (`git diff Cargo.lock` non-empty exactly once, for `keyring`) intact; the revert re-exposed these pre-existing advisories. `cargo deny check advisories` FAILS at baseline 822070d7 identically, and CI's deny job is advisory (`continue-on-error`, discipline.yml:38). The upgrades (rustls → 0.23.45, rustls-webpki → 0.103.15, chacha20 → 0.10.2, socket2, itertools) must land as their own reviewed change. **Owner: `20-3b-gate-retirement-and-coverage-generator`** — its charter already owns whether `cargo deny` becomes binding.

## Deferred from: code review of 16-5-audit-drop-legal-hold-and-a2a-deny-vocabulary (2026-09-17)

- **Journal open fails hard on a torn multibyte line** [`crates/maos-kernel-core/src/journal/mod.rs:110-111`] — `open()`'s strict `read_to_string` turns a torn multibyte char (partial `write_all` under ENOSPC) into a boot-failing `JournalError::Open` instead of skipping the line, and `recover_in_flight_with_tasks` swallows the same error into an empty report. Story 16-5's single-`write_all` append sharply reduces torn writes but recovery still assumes clean UTF-8. Pre-existing shape; lossy-read + skip is the candidate repair.
- **`CapError::AuditSinkUnavailable` reaches operators as Display prose only** [`crates/maos-bin/src/main.rs:1275,1304,1332`] — the distinct cause exists in Rust but no door code, exit code, or JSON-RPC mapping distinguishes it from other refusals; an operator surface must string-match to learn "the audit sink is down". Same doctrine debt AC7 removed for D7's vocabulary (ADR-058:52, "distinguishable without string matching"); the `CrossWall*` typed-surface treatment is the in-repo precedent for the fix.

## Deferred from: code review of 16-6-maosctl-load-over-the-door (2026-09-20)

- **Two pre-existing test-environment gaps fail package runs, not this change** [`crates/maos-cli/tests/two_host_reconcile_2c.rs:511,597`; `crates/maos-bin/tests/drain_once_audit_writer.rs:27`] — `cargo test -p maos-cli` has 2 failures needing a Python Ed25519 backend (`pip install cryptography` or `PyNaCl`; verify.py reports "no Ed25519 library found"); `cargo test -p maos-bin` stops at `drain_once_audit_writer` because the `worker-cli-fixture` binary is absent from daemon-sibling/deps//PATH. Unrelated to Story 16-6: the story's own target `maosctl_load_16_6` passes 6/6 when invoked separately, and kernel-core (586) and control (39) suites are green. Repair is environment provisioning, not code.

## Deferred from: code review decisions (2026-09-20)

- **Worker `on_unload` failure window** — mechanism (x) releases the kernel state (receipt + revocation + SCB removal) before `on_unload`; for the subprocess Worker whose `on_unload` (`crates/maos-bin/src/supervision.rs:1668-1680`) is the only child-ender, a panicking/budget-exceeded hook leaves a live OS child with no SCB and a `PlannedUnload` receipt already written. Ratified residual of §18a ruling (A); owner: kernel supervision / Epic 17-1 endgame.
- **Teardown classification loss on panicking `on_start`** — the crash handler spawn is fire-and-forget; if rollback's map removal wins the race, cleanup completes but the crash record is lost (`NotLoaded` into a discarded `Result`). State stays consistent; only the crash classification is lost. Residual of the crash-handler-owns-teardown rule (review 2026-09-20).
