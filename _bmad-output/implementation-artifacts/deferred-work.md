...
- **`run_bounded` can deadlock a chatty child on full pipes** [`crates/maos-journey-test/src/lib.rs:742-761`] — the bounded loop is `child.try_wait()` + 50 ms sleep while `stdout`/`stderr` stay `piped()` and are drained only AFTER exit via `wait_with_output()`. A child writing past the OS pipe buffer (~64 KiB) blocks on write forever, `try_wait` never yields `Some`, the deadline fires, the child is killed and the helper panics `"{label}: did not exit within {timeout_secs}s"` — a hang misreported as a timeout. Latent: every current call site (`maos init`, `maosctl …`) is small-output. The generic helper's timeout contract is unsound for any future verbose child. **Owner: `16-3-subprocess-crash-to-handle-crash`** — its charter is subprocess supervision and it already owns the crash/hang corpora (100-kill/50-hang), so the concurrent-drain fix belongs with it.
