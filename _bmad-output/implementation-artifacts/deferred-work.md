...
- **Workspace grew 17 → 19 crates without arch-doc update.** `maos-audit` added in 1b.1 (Transparency Log adapter holder); `maos-attrs` added in 1b.3 (proc-macro crate for `#[i9_exempt]`). Both defensible. Architecture doc still says 17. Tracked as Epic 1b retro **D10** — must close before Story 2.1 opens.
- **Crate-level `#![forbid(unsafe_code)]` relaxed to per-module without ADR.** Story 1b.3 needed `unsafe` for `pre_exec` async-signal-safe syscalls in `security/sandbox/linux/`. Per-module relaxation shipped without an ADR documenting the governance change. Epic 2's `#[spirit]` proc-macro will touch unsafe too. Tracked as Epic 1b retro **Doc3** — must close before Story 2.1 opens.
- **Dual `SandboxTier` type hierarchy.** Two parallel types: `maos_domain::invariants::i9::SandboxTier` (kernel-side, used by admission + `cap_policy::strictest_of`) AND `maos_spirit_abi::compliance::SandboxTier` (ABI-side enum, used by ComplianceClaim envelope). Neither authoritative. Story 2.1's 11 lifecycle hook signatures will reference SandboxTier — cannot land with two competing canonicals. Tracked as Epic 1b retro **D9** — must close before Story 2.1 opens.
