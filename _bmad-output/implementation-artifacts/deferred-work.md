...
- **`resolve_spirit_name`'s `hello-spirit → pid 0` wildcard** [`crates/maos-audit/src/lib.rs:1624-1665`] — the v0.1-β fallback answers the name `hello-spirit` with `SELECT DISTINCT boot_nonce, spirit_pid … WHERE spirit_pid = 0`, which matches EVERY pid-0 kernel row regardless of which spirit wrote it, so once kernels journal pid-0 rows for other spirits the name resolves to wrong boots. **Owner: `16-2-shell-halt-registry-and-j0-scene`** — it loads hello-spirit through the scheduler with a real pid, retiring the fallback; the note is written into its AC2.
