...
- **A second, independent cause collapse on a SHIPPED operator surface.**
  ...
  **`WrongDirection`**, `ConsentStateStale`, `ConsentStateUnavailable`,
  ...
  one is in the CLI, not the kernel. **Owner: `16-6-audit-drop-legal-hold-and-a2a-deny-vocabulary`** — D7, unchanged target, re-homed off the epic key by `14-0` AC2.2. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
...
- Nothing in production compares the environment's `MAOS_REGION_HOME` with the
  ...
  **Owner: `20-5-env-contract-registry-and-secrets`** — D4a, SPLIT out by `14-0` AC4.4. Neither named vehicle could host it: 14-7 and 14-8 are static-scan stories and cannot host a runtime boot check, so D4's target AND its stated fallback were both wrong. Registration stays D4b/`20-5-env-contract-registry-and-secrets`; classification+crypto is D4c/`20-5-env-contract-registry-and-secrets`. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
...
- **No gate reconciles the kernel pin with its own HISTORY.** `grep -rn "HISTORY" xtask/src/` → **0**. `xtask/kernel-core-baseline.toml:438` reads *"23596 → 23517"* while its own prose two lines down says *"+116 physical lines from the 23401 pin"* (23401 + 116 = 23517). The `from` value could be arbitrary and nothing would red. Fixing it needs a NEW gate, which Story 13.6 is forbidden to add, so the gap is recorded rather than closed.
  **Owner: `20-4-kernel-hygiene-one-instrument`** — handed by the Epic-13 retrospective §4 (2026-08-11): 14.6 owns the ceiling instrument and retro-residual discipline.
...
- **`manifest()`/`peer_convergence()` read under two separate locks in `CohortSelfIdentity::self_identity`** (`crates/maos-bin/src/cert_rotation.rs`) — a manifest reissue committing between the two reads can render one operator request with an old `declared` fingerprint and a fresh convergence table (transient false `agree` verdict; self-corrects on the next poll). Two-lock composition is pre-existing; fixing it needs a single-snapshot read API in `maos-cohort` (new API surface + budget). Owner: `20-3-durable-tofu-and-self-leaf-rotation`, which owns the self-identity surface.
