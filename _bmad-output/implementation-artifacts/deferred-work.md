...
- **A second, independent cause collapse on a SHIPPED operator surface.**
  ...
  **`WrongDirection`**, `ConsentStateStale`, `ConsentStateUnavailable`,
  ...
  one is in the CLI, not the kernel. **Owner: `14-4-v2-0-sweep-operational-surfaces`** — D7, unchanged target, re-homed off the epic key by `14-0` AC2.2. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
...
- Nothing in production compares the environment's `MAOS_REGION_HOME` with the
  ...
  **Owner: `14-d4a-region-home-boot-reconciliation`** — D4a, SPLIT out by `14-0` AC4.4. Neither named vehicle could host it: 14-7 and 14-8 are static-scan stories and cannot host a runtime boot check, so D4's target AND its stated fallback were both wrong. Registration stays D4b/`14-8-register-classify-full-workspace-env-surface`; classification+crypto is D4c/`14-9-secret-var-governance-provider-keys`. *Re-homed by Story `14-0` (2026-08-26), which ruled that an `epic-*` key is a roll-up and not a vehicle; `check-dev-record-completeness` now reds on one.*
...
- **`manifest()`/`peer_convergence()` read under two separate locks in `CohortSelfIdentity::self_identity`** (`crates/maos-bin/src/cert_rotation.rs`) — a manifest reissue committing between the two reads can render one operator request with an old `declared` fingerprint and a fresh convergence table (transient false `agree` verdict; self-corrects on the next poll). Two-lock composition is pre-existing; fixing it needs a single-snapshot read API in `maos-cohort` (new API surface + budget). Owner: `14-2d-self-identity-rotation`, which owns the self-identity surface.
