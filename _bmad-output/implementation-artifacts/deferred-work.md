...
- `tests/coverage-matrix.yaml` NFR-Rel-9 row notes (10.4b-era, pre-existing): the row maps NFR-Rel-9 to "revocation-propagation latency under mTLS cert-rotation churn ... p50 ≤30s / p99 ≤90s ... cross-listed under NFR-Sec-13", but the PRD's NFR-Rel-9 is `≤ 5s p99 under 10⁴ concurrent capability-token validations` (`non-functional-requirements.md:28`); the 30s/90s floors are the cert-rotation chaos scope (NFR-Sec-13, PRD row: median ≤60s / p99 ≤5min). This conflation is the upstream source 14-1's D-A delta inherited when it misattributed 30s/90s to NFR-Rel-9. Owner: 14-2 (already holds the `check-rotation-real-timing` unimplemented-gate finding for the same NFR family).
