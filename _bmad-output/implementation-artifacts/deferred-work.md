...
- **Operator HTTP serializes every connection on one accept thread** (`crates/maos-control/src/lib.rs:127-175`) — `handle_connection` runs inline in the accept loop, and each socket may consume the two-second read timeout before the next authenticated operator request is accepted. Pre-existing to Story 14-2a; the rotation read surface makes the availability cost more visible but does not introduce the server architecture. A bounded per-connection worker design belongs to the operator-surface owner rather than this trust-rotation patch.
