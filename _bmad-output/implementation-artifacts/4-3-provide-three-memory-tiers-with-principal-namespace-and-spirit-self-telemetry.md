...
- [ ] **[High]** [edge] *defer* — Spirit self-telemetry exposes aggregate memory pressure but not per-tier breakdown; operators cannot diagnose tier-specific pressure
  - *(deferred to Story 8.3 at v0.5 binding window)*
- [x] **[Medium]** [auditor] *patch* — Principal namespace isolation test missing cross-spirit leakage validation; added in 4-3 commit
  - *Resolution: crates/maos-kernel-core/tests/principal_namespace_isolation.rs*
- [x] **[Low]** [test-infra] *dismissed* — Memory tier migration (hot→warm→cold) is manual-only; automatic migration deferred to v0.5
  - *Rationale: Epic 4 scope boundary*` row. Per Epic 3 retro A6 the Review Findings table is mandatory; every reviewer-raised finding gets a row with explicit `closed | open | deferred → Story X.Y | dismissed` status so retros can grep-verify without prose-archaeology.
