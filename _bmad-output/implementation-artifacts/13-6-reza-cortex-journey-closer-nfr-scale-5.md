...
**Design (confirmed 2026-07-30 judge-machinery preflight).** The projection is a pure function over fields every leg already emits (`{name, binding, attempted, substrate_present, green, detail}`) plus a new signature verification: `!attempted`→`ABSENT`; `attempted && green && BindingClass::Blocking`→`PROVEN_BLOCKING` (hermetic/reproducible — artifact = transcript ref, no signature); `attempted && green && BindingClass::AdvisorySubstrate && signature verified`→`PROVEN_LIVE_SIGNED`; everything else attempted (`!green`, or green-live-unsigned)→`INDETERMINATE`. Because it rides fields every leg already emits, a leg added without a state is structurally impossible (trap #3 closed by construction, not by a hand-maintained list). The ledger's NEW value is precisely the two cases the dev-lane exit (`blockers.is_empty()`) misses — `ABSENT` legs (D-2) and green-live-unsigned legs — both downgrade to not-proven and block the product claim even when the gate exits 0; a red `Blocking` leg already hard-fails via `blockers`, so the ledger need not re-catch it. Full mechanism (signer, verifier, key location, falsifier, plumbing) in Dev Notes.
...
### AC5 judge-machinery design — confirmed 2026-07-30 preflight (party-mode: Winston · Murat · Amelia · John)

Grounded in the actual gate code (`check_reza_production_path.rs` / `check_multi_tenant_loom.rs` are copy-pasted twins sharing `LegResult{name,binding,attempted,substrate_present,green,detail}` + `blocks(class) = !green && dev_enforced_red_blocks(class, substrate_present)`; `oracle_green = legs.all(green)` is already computed; `skipped_live = legs.filter(!attempted)` already derives absent legs at runtime). The closer does NOT invent: signing reuses `maos_audit::sealed_export::sign_bundle` + `MAOS_AUDIT_KEY_SEED` + `SignatureBlock` (FR44 / 13.2 Fork-4). All lands in `xtask` + `maos-audit` verify-side — **ZERO kernel-core**, judge's instrument not journey mechanism (trap #1 held).

- **Projection** — as in AC5; derived from existing fields, no per-leg annotation (trap #3 closed by construction).
- **Signer (trap #2 — the judge must not grade its own code).** The **live test harness signs its own transcript** via `sealed_export::sign_bundle` under `MAOS_AUDIT_KEY_SEED` (the `record-capture` idiom). The gate must NOT sign post-hoc — that would attest "the gate saw this text," not "the test produced it." `LegResult` gains `signature_block: Option<SignatureBlock>` + `artifact_ref`.
- **Verifier.** The ledger derives the expected pubkey from the **operator-pinned audit key** and verifies (R-RG1, `sealed_export.rs:87`) — never trusts a key the bundle carries. A planted lie (claimed `PROVEN_LIVE_SIGNED` with empty/bad/wrong-key signature) reds.
- **Key location (operator-ratified (a)).** `PROVEN_LIVE_SIGNED` requires an **operator-local/provisioned run**; CI (no operator key) yields live legs as `ABSENT`/`INDETERMINATE`. The product-claim is proven only when a signed artifact is published and the ledger verifies it — same posture as J1 Tier-2 ("CI can't hold the key"). CI dev-key signing is **rejected**: a dev-key-forgible artifact named PROVEN is the 13.2 "trusted registry" category error replayed.
- **Verdict plumbing.** Dev-lane exit (`blockers.is_empty()`) unchanged. New JSON field `product_claim: "PROVEN" | "NOT_PROVEN(<reasons>)"`, set PROVEN only when every required leg is PROVEN. `check_ship_gate_completeness` consumes it and reds a ship badge asserted while `NOT_PROVEN` — this closes that gate's ownerless "never validates CI→registry" gap (in-scope under AC5/T4).
- **Falsifier — three blinds, all must red:** (1) `PROVEN_LIVE_SIGNED` claimed + empty `signature_block`; (2) signature present but verification fails; (3) signature OK but the artifact is **not bound to this build** (stale reuse) → `artifact_ref` carries a commit-sha / substrate-nonce binding.
- **Two-gate reconcile.** A shared projection helper both Reza gates call; `ABSENT_SUCCESSORS` becomes **derived** from the legs that came back ABSENT this run (replacing the hand-maintained `&[&str]` const and the empty `&[]` on `check_multi_tenant_loom.rs:16`).
- **Substrate split.** The projection, signature-verify, and all three falsifier blinds are **substrate-independent and buildable now** (hermetic fixtures). Only the LIVE `PROVEN_LIVE_SIGNED` demonstration needs AC1/13-6c's 3×3 substrate + an operator run — so the mechanism can be authored and proven-red before the closer unblocks.
...
| 2026-07-30 | Judge-machinery design confirmed (party-mode preflight). AC5 mechanism fixed: projection over existing leg fields; live test harness signs (`sealed_export`, trap #2); ledger derives-pubkey-and-verifies (R-RG1); operator-local key only (CI dev-key rejected — category error); new `product_claim` JSON field + `check_ship_gate_completeness` consumer; falsifier = 3 blinds (empty sig / verify-fail / stale-binding); `ABSENT_SUCCESSORS` derived. ZERO kernel-core; substrate-independent mechanism buildable before 13-6c unblocks. AC5 + Dev Notes updated. |
